...
func runHarnessAgentRestartProbeStepWithCheck(ctx context.Context, repoRoot string, check harnessAgentRestartCheck) harnessStep {
	started := time.Now()
	step := harnessStep{Name: harnessAgentRestartProbeName, Command: []string{"go", "run", "./scripts/verify", "--repo-root", repoRoot, "--release", "--summary"}}
	var err error
	...
	if err != nil {
		...
		if len(step.Diagnostics) == 0 {
			step.Diagnostics = []checkDiagnostic{{
				...
				SuggestedAction: "Fix local-agent restart persistence or route recovery, then rerun `go run ./scripts/verify --release --summary --write`.",
			}}
		}
		return step
	}
...
func runHarnessAgentRestartProbeCheck(ctx context.Context, _ string) (map[string]any, []checkDiagnostic, error) {
	...
	session, err := client.Register(ctx, localagent.RegisterRequest{
		...
		ReportToken:   "restart-report-secret",
	...
	if registered, ok := server.GetSession(session.SessionID); !ok || registered.ReportToken != "restart-report-secret" {
		return nil, nil, fmt.Errorf("agent socket registration did not preserve the session report token")
	}
	...
	return map[string]any{
		...
		"session_report_token_registered": true,
	}, nil, nil
}
