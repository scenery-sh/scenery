...
var allowedDirectGoDeps = map[string]string{
	...
	"golang.org/x/text":                      "Unicode 17.0 NFC normalization required by the current relative_path scalar contract",
...
var legacyIdentityMigrationFiles = map[string]struct{}{
	// The release probe preserves the existing durable DNS migration boundary.
	"scripts/verify/harness_self_edge.go":            {},
...
func runHarnessArchitectureStep(repoRoot string) harnessStep {
	...
	if err != nil {
		diagnostics = append(diagnostics, checkDiagnostic{
			...
			Severity:        "error",
			Message:         err.Error(),
			SuggestedAction: "Fix the source walk error, then rerun `go run ./scripts/verify -o json`.",
		})
	} else {
...
func checkArchitectureDependencies(repoRoot string, summary *architectureSummary) []checkDiagnostic {
	...
	if err != nil {
		return []checkDiagnostic{{
			...
			Message:         err.Error(),
			SuggestedAction: "Restore go.mod, then rerun `go run ./scripts/verify -o json`.",
		}}
	}
	...
	if err != nil {
		return []checkDiagnostic{{
			...
			File:            filepath.ToSlash(filepath.Join(repoRoot, "go.mod")),
			Message:         err.Error(),
			SuggestedAction: "Fix go.mod syntax, then rerun `go run ./scripts/verify -o json`.",
		}}
	}
	var diagnostics []checkDiagnostic
