...
func runHarnessAssistantInitProbeStepWithCheck(ctx context.Context, repoRoot string, check harnessAssistantInitCheck) harnessStep {
	started := time.Now()
	step := harnessStep{Name: harnessAssistantInitProbeName, Command: []string{"go", "run", "./scripts/verify", "--repo-root", repoRoot, "--release", "--summary"}}
	var err error
	...
	if err != nil {
		...
		if len(step.Diagnostics) == 0 {
			step.Diagnostics = []checkDiagnostic{{
				...
				SuggestedAction: "Fix production assistant initialization and predicted generation checks, then rerun `go run ./scripts/verify --release --summary --write`.",
			}}
		}
		return step
	}
...
func runHarnessAssistantInitProbeCheck(parent context.Context, repoRoot string) (map[string]any, []checkDiagnostic, error) {
	...
	if err := runProduct(ctx, repoRoot, &output, "assistant", "init", "extra", "--mcp-server", "support", "--client", "public_api", "--app-root", appRoot, "-o", "json"); err != nil {
		return nil, nil, err
	}
	var response assistantInitResponse
	...
	found := false
	for _, resource := range after.Manifest.Resources {
		if resource.Address == "app/assistant/extra" && resource.Kind == "scenery.assistant" {
			found = true
		}
	}
	if !found {
