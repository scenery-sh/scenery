	...
	if err != nil {
		...
		if len(step.Diagnostics) == 0 {
			step.Diagnostics = []checkDiagnostic{{
				...
				Severity:        "error",
				Message:         step.Error,
				SuggestedAction: "Fix the Go code-task process boundary, then rerun `go run ./scripts/verify --release --summary --write`.",
			}}
		}
		return step
	}
...
func runHarnessCodeTaskProcessProbeCheck(ctx context.Context, repoRoot string) (map[string]any, []checkDiagnostic, error) {
...
`,
	...
	if err := runProduct(ctx, repoRoot, &stdout, "task", "run", "billing:reconcile", "--app-root", root, "--env", "production", "--", "--dry-run", "--limit", "100"); err != nil {
		return nil, nil, err
	}
