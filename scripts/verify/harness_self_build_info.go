...
func runHarnessBuildInfoProbeStepWithCheck(ctx context.Context, repoRoot string, check harnessBuildInfoCheck) harnessStep {
	started := time.Now()
	step := harnessStep{Name: harnessBuildInfoProbeName, Command: []string{"go", "run", "./scripts/verify", "--repo-root", repoRoot, "--release", "--summary"}}
	var err error
	...
	if err != nil {
		...
		if len(step.Diagnostics) == 0 {
			step.Diagnostics = []checkDiagnostic{{
				...
				SuggestedAction: "Fix trimpath build-info freshness detection, then rerun `go run ./scripts/verify --release --summary --write`.",
			}}
		}
		return step
	}
