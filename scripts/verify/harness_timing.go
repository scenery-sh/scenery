...
func selectHarnessTimingConfirmations(report *harnessTestTimingReport, baseline *harnessTestTimingReport) {
	...
	report.Diagnostics = append(report.Diagnostics, checkDiagnostic{
		...
		SuggestedAction: "Run `go run ./scripts/verify --release --fresh-tests -o json --write` for a full timing audit, or read `deferred_confirmations` in `.scenery/harness/test-timing-latest.json`.",
	})
}
...
func confirmHarnessTimingOutliers(ctx context.Context, repoRoot string, report *harnessTestTimingReport, run harnessTimingCommandRunner) {
	...
	started := time.Now()
	defer func() { report.ConfirmationSeconds = roundSeconds(time.Since(started).Seconds()) }()
	...
	if len(packageIndices) > 0 {
		...
		if err != nil {
			report.Diagnostics = append(report.Diagnostics, timingConfirmationFailure(report, "packages", command, err))
		} else {
			for _, index := range packageIndices {
				pkg := &report.Packages[index]
				seconds, ok := packageElapsedFromGoTestJSON(output, pkg.Package)
				if !ok {
					report.Diagnostics = append(report.Diagnostics, timingConfirmationFailure(report, "package "+pkg.Package, command, errors.New("package timing missing from JSON output")))
					continue
				}
	...
	groups := harnessTestConfirmationGroups(report.ObservedSlowTests)
	var binaryRoot string
	if len(groups) > 0 {
		var err error
		binaryRoot, err = os.MkdirTemp("", "scenery-timing-confirm-*")
		if err != nil {
			report.Diagnostics = append(report.Diagnostics, timingConfirmationFailure(report, "temporary test binaries", nil, err))
			return
		}
		defer func() {
			if err := os.RemoveAll(binaryRoot); err != nil {
				report.Diagnostics = append(report.Diagnostics, timingConfirmationFailure(report, "temporary test binary cleanup", nil, err))
			}
		}()
	}
	for groupIndex, group := range groups {
		binary := filepath.Join(binaryRoot, fmt.Sprintf("%d.test", groupIndex))
		packageDir, err := prepareHarnessTimingBinary(ctx, repoRoot, group.Package, binary, run)
		if err != nil {
			report.Diagnostics = append(report.Diagnostics, timingConfirmationFailure(report, "test binary "+group.Package,
				[]string{"go", "test", "-run", "^$", "-json", group.Package}, err))
			continue
		}
		for _, index := range group.Indices {
			observed := &report.ObservedSlowTests[index]
			command := []string{"go", "tool", "test2json", "-t", "-p", group.Package, binary,
				"-test.v=test2json", "-test.count=1", "-test.parallel=1", "-test.run=^" + regexp.QuoteMeta(observed.Name) + "$"}
			var samples []float64
			for sample := 0; sample < confirmationRuns; sample++ {
				output, err := run(ctx, packageDir, command)
				var seconds float64
				if err == nil {
					seconds, err = isolatedHarnessTimingSample(output, observed.Package, observed.Name)
				}
				if err != nil {
					reproduce := []string{"go", "test", "-count=1", "-parallel=1", "-run", "^" + regexp.QuoteMeta(observed.Name) + "$", "-json", observed.Package}
					report.Diagnostics = append(report.Diagnostics, timingConfirmationFailure(report,
						fmt.Sprintf("test %s.%s sample %d/%d", observed.Package, observed.Name, sample+1, confirmationRuns), reproduce, err))
					break
				}
			...
			report.Diagnostics = append(report.Diagnostics, checkDiagnostic{
				...
				SuggestedAction: "Reduce the test body toward the 50-60ms target without weakening its assertion boundary; move real external-boundary proof to an explicit release probe while retaining fast in-process coverage. No timing exceptions are permitted.",
			})
		}
	}
	for i := range report.Packages {
...
// Link once for this confirmation only. Each sample still starts a new process;
// neither test results nor binaries persist in another cache or scheduler.
func prepareHarnessTimingBinary(ctx context.Context, repoRoot, packageName, binary string, run harnessTimingCommandRunner) (string, error) {
	output, err := run(ctx, repoRoot, []string{"go", "list", "-json", packageName})
	if err != nil {
		return "", fmt.Errorf("resolve test package: %w", err)
	}
	var pkg struct{ Dir, ImportPath string }
	if err := json.Unmarshal(output, &pkg); err != nil {
		return "", fmt.Errorf("decode test package: %w", err)
	}
	if pkg.ImportPath != packageName || !filepath.IsAbs(pkg.Dir) {
		return "", fmt.Errorf("test package identity or directory does not match %s", packageName)
	}
	if _, err := run(ctx, repoRoot, []string{"go", "test", "-c", "-o", binary, packageName}); err != nil {
		return "", fmt.Errorf("link test package: %w", err)
	}
	return pkg.Dir, nil
}
...
func timingConfirmationFailure(report *harnessTestTimingReport, subject string, command []string, err error) checkDiagnostic {
	if len(command) == 0 {
		command = []string{"go", "run", "./scripts/verify", "--fresh-tests", "-o", "json", "--write"}
		if report.Budgets.Lane == "release" {
