...
export TEMPORAL_ADDRESS="127.0.0.1:$(free_port)"
...
refresh_generated_model_schemas() {
  local wt="$1"
  local name="$2"
  local log="$LOG_DIR/$name-generate-data.json"
  local generated target service
  if ! "$SCENERY_BIN" generate data --dry-run --json --app-root "$wt" >"$log" 2>&1; then
    printf 'onlv smoke: generated model data refresh failed for %s\n' "$name" >&2
    cat "$log" >&2 || true
    return 1
  fi
  while IFS= read -r -d '' generated; do
    service="${generated#"$wt/.scenery/gen/db/"}"
    service="${service%/schema.hcl}"
    target="$wt/$service/db/schema.hcl"
    if [[ ! -f "$target" ]]; then
      continue
    fi
    if ! head -5 "$target" | grep -Fq "Code generated by scenery generate data"; then
      printf 'onlv smoke: refusing to overwrite handwritten schema %s\n' "$target" >&2
      return 1
    fi
    cp "$generated" "$target"
  done < <(find "$wt/.scenery/gen/db" -path '*/schema.hcl' -type f -print0 2>/dev/null)
}

refresh_generated_model_schemas "$WT_A" a
refresh_generated_model_schemas "$WT_B" b
