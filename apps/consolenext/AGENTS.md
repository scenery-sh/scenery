...
- The Symphony page uses `symphony/state`, `symphony/task/*`, `symphony/statuses/update`, and `symphony/workflow/*` RPCs backed by Scenery-owned dashboard SQLite. It must not call `/__graphql`, `postGraphQL`, or any Linear/GraphQL API. Manual runner RPCs such as `symphony/run/start` stay unavailable until the dashboard runner channel has an authenticated protocol; the current runner is server-side only and claims `Todo` tasks when workflow mode is `auto`.
