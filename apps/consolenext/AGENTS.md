...
- The Symphony page uses `symphony/state`, `symphony/task/*`, `symphony/statuses/update`, `symphony/workflow/*`, and read-only `symphony/run/detail` RPCs backed by the Scenery-managed Postgres `scenery_symphony` database. GraphQL is not a dashboard transport. Manual runner RPCs such as `symphony/run/start` stay unavailable until the dashboard runner channel has an authenticated protocol. The unauthenticated dashboard RPC must not enable `auto`; the local trust path is `scenery symphony auto --on --app-root <path>`, while the page may surface that RPC error and may de-escalate existing auto workflows. The current runner is server-side only, requires saved workflow markdown or app-root `WORKFLOW.md` for `auto`, claims eligible `Todo` tasks, moves claimed tasks to `In Progress`, moves succeeded tasks to `Human Review`, moves failed/timed-out/stalled agent tasks to `Rework`, and exposes run workspace, summary, changed files, diff, and event details through the task modal.
