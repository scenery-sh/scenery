...
func sendVerificationEmail(ctx context.Context, email string, token string, redirectPath string) error {
	...
	return sendAuthEmail(ctx, email, "Verify your email", "verify your email", appRedirectURL(nil, "/verify-email?"+query.Encode()))
}

func sendPasswordResetEmail(ctx context.Context, email string, token string) error {
	query := url.Values{}
	query.Set("token", token)
	return sendAuthEmail(ctx, email, "Reset your password", "reset your password", appRedirectURL(nil, "/reset-password?"+query.Encode()))
}

func sendInviteEmail(ctx context.Context, email string, token string) error {
	query := url.Values{}
	query.Set("token", token)
	return sendAuthEmail(ctx, email, "You have been invited", "accept your invite", appRedirectURL(nil, "/accept-invite?"+query.Encode()))
}
