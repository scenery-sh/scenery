...
- The blessed Astryx surface is the concrete app-proven set exported from `index.ts`: typography, buttons, badges, icons, stacks, inputs, selection controls, dialogs/popovers, navigation controls, tables/lists, feedback/progress, layout helpers, command-palette primitives, `Banner`, `EmptyState`, `Field`/`FieldLabel`, `FormLayout`, and `Theme`. Keep plain re-exports until a concrete Scenery convention requires a wrapper; do not mirror Astryx wholesale. Do not wrap or rename a primitive merely to offer alternate prop spelling; the catalog has one current component API.
