...
import (
	...
	nativeruntime "scenery.sh/runtime"
...
func TestTraceDBQueryRecordsChildSpan(t *testing.T) {
	...
	ctx := nativeruntime.TraceDBQueryStart(withState(context.Background(), state), " \n SELECT  *  FROM tenants WHERE id = $1 \n", 1)
	nativeruntime.TraceDBQueryEnd(ctx, "SELECT 1", 1, nil)
...
func TestTraceDBQueryWithoutRequestIsNoop(t *testing.T) {
	...
	ctx := nativeruntime.TraceDBQueryStart(context.Background(), "SELECT 1", 0)
	nativeruntime.TraceDBQueryEnd(ctx, "SELECT 1", 1, nil)
...
func TestTraceDBQueryRedactsInlineLiterals(t *testing.T) {
	...
	ctx := nativeruntime.TraceDBQueryStart(withState(context.Background(), state), `SELECT * FROM users WHERE email = 'secret@example.com' AND age = 42`, 0)
	nativeruntime.TraceDBQueryEnd(ctx, "", -1, nil)
...
func TestTraceDBQueryUsesSQLCQueryNameAsOperation(t *testing.T) {
	...
	ctx := nativeruntime.TraceDBQueryStart(withState(context.Background(), state), "-- name: ListLatestJobListings :many\nSELECT * FROM job_listings", 0)
	nativeruntime.TraceDBQueryEnd(ctx, "SELECT 30", 30, nil)
