...
func TestApplicationSpanRecordsNestedChildSpans(t *testing.T) {
	...
	leave := enterState(state)
	defer leave()
	if scenery.CurrentRequest().Payload != payload {
		t.Fatal("public facade lost native request payload identity")
	}
	ctx := context.WithValue(withState(context.Background(), state), nativeContextKey{}, payload)
	generateCtx, generateSpan := scenery.StartSpan(ctx, "GetEarth.generate")
	if generateCtx.Value(nativeContextKey{}) != payload {
		t.Fatal("public facade lost native Go context value")
	}
