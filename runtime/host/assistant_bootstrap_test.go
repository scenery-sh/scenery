...
func TestRegisterAssistantInstallsBootstrapService(t *testing.T) {
	...
	registerTestAssistant(t, "app/assistant/support", "support")
	global.mu.RLock()
	ok := global.services.Has(assistantRuntimeBootstrapService)
	global.mu.RUnlock()
	if !ok {
...
func TestAssistantMCPManifestRegistrationIsStrictAndDependsOnFederation(t *testing.T) {
	...
	global.mu.RLock()
	gatewayInitializer, gatewayOK := global.services.Snapshot().Initializers[serviceAddress]
	bootstrapInitializer, bootstrapOK := global.services.Snapshot().Initializers[assistantRuntimeBootstrapService]
	global.mu.RUnlock()
...
func TestAssistantMCPGatewayInitializesAfterFederationBeforeHelperBootstrap(t *testing.T) {
	...
	global.mu.RLock()
	serverOrder := global.services.Snapshot().InitOrder["app/mcp_server/support"]
	gatewayOrder := global.services.Snapshot().InitOrder[assistantMCPGatewayServiceAddress("app/assistant/support")]
	bootstrapOrder := global.services.Snapshot().InitOrder[assistantRuntimeBootstrapService]
	global.mu.RUnlock()
