...
func ReportPubSubMessage(message any) {
	reporter := activeReporter()
	if reporter == nil {
		return
	}
...
func StartPubSubMessageTrace(service, subscription, topic, messageID string, payload []byte) *PubSubTrace {
	reporter := activeReporter()
	if reporter == nil {
		return nil
	}
			...
			Event: map[string]any{
				"span_start": map[string]any{
					"request": map[string]any{
						...
						"http_method":       "PUBSUB",
						"path":              "/pubsub/" + topic + "/" + subscription,
						"request_payload":   json.RawMessage(append([]byte(nil), payload...)),
...
func FinishPubSubMessageTrace(trace *PubSubTrace, service, subscription, topic, messageID string, duration time.Duration, err error) {
	if trace == nil {
		return
	}
	reporter := activeReporter()
	...
	endTime := trace.Started.Add(duration)
	reporter.enqueue(devdash.ReportEnvelope{
		Type:  "trace-event",
		AppID: reporter.appID,
		TraceEvent: &devdash.TraceEvent{
	...
	reporter.enqueue(devdash.ReportEnvelope{
		...
		TraceSummary: &devdash.TraceSummary{
			...
			ServiceName:      service,
			EndpointName:     optionalString(subscription),
			TopicName:        optionalString(topic),
			SubscriptionName: optionalString(subscription),
