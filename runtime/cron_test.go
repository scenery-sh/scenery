...
func TestTemporalCronScheduleSpecForEvery(t *testing.T) {
	...
	spec, err := TemporalCronScheduleSpecForJob(job)
	if err != nil {
		t.Fatalf("TemporalCronScheduleSpecForJob returned error: %v", err)
	}
	if len(spec.Intervals) != 1 || spec.Intervals[0] != 5*time.Minute {
