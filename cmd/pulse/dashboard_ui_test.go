...
func TestDashboardServesUIAssetsFromOverrideDir(t *testing.T) {
	t.Setenv("PULSE_DEV_DASHBOARD_UI_DIR", t.TempDir())
	uiDir := os.Getenv("PULSE_DEV_DASHBOARD_UI_DIR")
	...
	if err := os.WriteFile(filepath.Join(uiDir, "assets", "app.js"), []byte(`console.log("pulse-ui")`), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	if body := rec.Body.String(); body != `console.log("pulse-ui")` {
		t.Fatalf("unexpected asset body: %q", body)
	}
}

func TestDashboardFallbackWhenUIDirMissing(t *testing.T) {
	t.Setenv("PULSE_DEV_DASHBOARD_UI_DIR", filepath.Join(t.TempDir(), "missing"))
	server := newTestDashboardServer(t)
