...
func signBuiltBinaryIfNeeded(path string) error {
	...
	if err != nil {
		return fmt.Errorf("onlava: codesign not available for macOS binary signing: %w", err)
	}
	...
	if err != nil {
		msg := string(output)
		if msg == "" {
			return fmt.Errorf("onlava: failed to codesign built binary: %w", err)
		}
		return fmt.Errorf("onlava: failed to codesign built binary: %w\n%s", err, msg)
	}
	return nil
}
