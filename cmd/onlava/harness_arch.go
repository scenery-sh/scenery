...
var allowedDirectGoDeps = map[string]string{
	"github.com/fsnotify/fsnotify":       "file watching for onlava up live rebuilds",
...
func runHarnessArchitectureStep(repoRoot string) harnessStep {
	...
	step := harnessStep{
		...
		Command: []string{"onlava", "harness", "self", "internal:architecture-check", repoRoot},
	...
	if err != nil {
		diagnostics = append(diagnostics, checkDiagnostic{
			...
			Severity:        "error",
			Message:         err.Error(),
			SuggestedAction: "Fix the source walk error, then rerun `onlava harness self --json`.",
		})
	} else {
...
func checkArchitectureDependencies(repoRoot string, summary *architectureSummary) []checkDiagnostic {
	...
	if err != nil {
		return []checkDiagnostic{{
			...
			Message:         err.Error(),
			SuggestedAction: "Restore go.mod, then rerun `onlava harness self --json`.",
		}}
	}
	...
	if err != nil {
		return []checkDiagnostic{{
			...
			File:            filepath.ToSlash(filepath.Join(repoRoot, "go.mod")),
			Message:         err.Error(),
			SuggestedAction: "Fix go.mod syntax, then rerun `onlava harness self --json`.",
		}}
	}
	var diagnostics []checkDiagnostic
