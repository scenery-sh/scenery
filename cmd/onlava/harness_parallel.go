	...
	if err != nil {
		...
		if len(step.Diagnostics) == 0 {
			step.Diagnostics = []checkDiagnostic{{
				...
				Severity:        "error",
				Message:         step.Error,
				SuggestedAction: "Fix local agent session isolation, then rerun `onlava harness self --json`.",
			}}
		}
		return step
	}
...
func runHarnessParallelDevCheck(parent context.Context) (map[string]any, []checkDiagnostic, error) {
	...
	defer cancel()
	agentHome := filepath.Join(os.TempDir(), "onlava-harness-parallel-"+harnessRandomLabel())
	defer os.RemoveAll(agentHome)
	restoreEnv := patchEnv(map[string]*string{
		"ONLAVA_AGENT_HOME":            stringPtr(agentHome),
		"ONLAVA_DEV_CACHE_DIR":         nil,
		"ONLAVA_DEV_DASHBOARD_ADDR":    nil,
		"ONLAVA_AGENT_DISABLE":         nil,
		"ONLAVA_LOCAL_PROXY":           stringPtr("0"),
		"ONLAVA_DEV_VICTORIA":          stringPtr("0"),
		"ONLAVA_DEV_VICTORIA_DOWNLOAD": stringPtr("0"),
	})
	defer restoreEnv()
	...
	root := filepath.Join(os.TempDir(), "onlava-harness-apps-"+harnessRandomLabel())
	...
	summary := map[string]any{
		...
		"temporal_queues": []string{envValueFromList(supervisorA.sessionTemporalEnv(), onlavaruntime.DefaultTemporalTaskQueueEnv), envValueFromList(supervisorB.sessionTemporalEnv(), onlavaruntime.DefaultTemporalTaskQueueEnv)},
...
func validateHarnessParallelState(ctx context.Context, server *localagent.Server, client *localagent.Client, store *devdash.Store, appID string, sessionA, sessionB *localagent.Session, pgEnvA, pgEnvB []string, ensuredDBs map[string]bool, temporalEnvA, temporalEnvB []string) []checkDiagnostic {
	...
	check(envValueFromList(pgEnvA, "ONLAVA_MANAGED_DATABASE_NAME") != "" && envValueFromList(pgEnvB, "ONLAVA_MANAGED_DATABASE_NAME") != "" && envValueFromList(pgEnvA, "ONLAVA_MANAGED_DATABASE_NAME") != envValueFromList(pgEnvB, "ONLAVA_MANAGED_DATABASE_NAME"), "managed Postgres database names must be distinct")
	check(len(ensuredDBs) == 2, "managed Postgres must ensure two session databases")
	check(envValueFromList(temporalEnvA, onlavaruntime.DefaultTemporalTaskQueueEnv) != "" && envValueFromList(temporalEnvA, onlavaruntime.DefaultTemporalTaskQueueEnv) != envValueFromList(temporalEnvB, onlavaruntime.DefaultTemporalTaskQueueEnv), "Temporal task queue prefixes must be distinct")
