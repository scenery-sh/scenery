...
func runDBBranchPrune(ctx context.Context, stdout io.Writer, opts dbBranchOptions) error {
	...
	pruned, err := pruneExpiredNeonBranchLeases(current.BranchID, olderThan)
	if err != nil {
		return err
	}
	status, err := buildDBBranchStatus(ctx, appRoot, cfg)
...
func pruneExpiredNeonBranchLeases(currentBranchID string, olderThan time.Duration) (int, error) {
	root, err := neonSubstrateRoot()
	if err != nil {
		return 0, err
	}
...
func removeCurrentNeonBranchLease(appRoot string) (string, bool, error) {
	pin, ok, err := readWorktreeDBPin(worktreeDBPinPath(appRoot))
	if err != nil {
		return "", false, err
	}
	...
	if isProtectedNeonParentBranch(pin) {
		return "", false, fmt.Errorf("refusing to remove protected parent branch lease %q", pin.Branch)
	}
	root, err := neonSubstrateRoot()
	if err != nil {
		return "", false, err
	...
	registry.Leases = kept
	registry.UpdatedAt = time.Now().UTC().Format(time.RFC3339)
	return pin.Branch, true, writeNeonBranchRegistry(root, registry)
}
