...
const (
	doctorSchemaVersion         = "onlava.doctor.result.v1"
...
type doctorResponse struct {
	...
	Onlava        versionResponse   `json:"onlava"`
...
func doctorCommand(args []string) error {
	return runOnlavaDoctor(context.Background(), os.Stdout, args)
}

func runOnlavaDoctor(ctx context.Context, stdout io.Writer, args []string) error {
	return runOnlavaDoctorWithDeps(ctx, stdout, args, defaultDoctorProbeDeps())
}

func runOnlavaDoctorWithDeps(ctx context.Context, stdout io.Writer, args []string, deps doctorProbeDeps) error {
	...
	if opts.JSON {
		...
		if !resp.OK {
			return &silentCLIError{err: fmt.Errorf("onlava doctor found %d error(s)", resp.Summary.Errors)}
		}
		return nil
	}
	...
	if !resp.OK {
		return fmt.Errorf("onlava doctor found %d error(s)", resp.Summary.Errors)
	}
	return nil
}
	...
	return doctorProbeDeps{
		...
		DiscoverApp: func(start string) (doctorAppInfo, appcfg.Config, bool, error) {
			...
			return doctorAppInfo{
				Root:       root,
				ConfigPath: filepath.Join(root, ".onlava.json"),
				Name:       cfg.Name,
				ID:         cfg.ID,
			}, cfg, true, nil
		},
	...
	if strings.TrimSpace(appStart) == "" {
		if cwd, err := deps.Getwd(); err == nil {
			...
			resp.Checks = append(resp.Checks, doctorCheck{
				...
				SuggestedAction: "Run `onlava doctor` from a readable directory.",
			})
		}
	}
	if strings.TrimSpace(appStart) != "" {
		...
		if err != nil {
			if strings.TrimSpace(opts.AppRoot) != "" {
				resp.Checks = append(resp.Checks, doctorCheck{
					...
					SuggestedAction: "Pass a directory inside an app that contains `.onlava.json`.",
				})
			}
		} else if ok {
			resp.App = &app
...
func doctorRuntimeCheck(info doctorRuntimeInfo) doctorCheck {
	...
	if !doctorSupportedRuntime(info.GOOS) {
		status = doctorStatusWarn
		severity = doctorSeverityOptional
		message += " is not a routinely tested onlava development platform"
...
func doctorCPUCheck(numCPU int) doctorCheck {
	...
	if numCPU < 2 {
		...
		check.SuggestedAction = "Use at least 2 logical CPUs for smoother onlava development."
	}
	return check
}
...
func doctorMemoryCheck(memory doctorMemoryInfo) doctorCheck {
	...
	case memory.TotalBytes < doctorMemoryErrorBytes:
		...
		check.SuggestedAction = "Use a machine or container with more memory for onlava development."
	case memory.TotalBytes < doctorMemoryWarnBytes:
		...
		check.SuggestedAction = "Use at least 4 GiB RAM for smoother onlava development."
	}
	return check
}
...
func doctorDiskCheck(ctx context.Context, probe doctorResourceProbe, path doctorPathReport, env *doctorEnvironment) []doctorCheck {
	...
	case disk.FreeBytes < doctorDiskWarnBytes:
		...
		check.SuggestedAction = "Keep at least 5 GiB free for onlava build workspaces, caches, and local dev state."
	}
	...
	specs := []doctorToolSpec{
		{
			...
			Required:        true,
			Relevant:        true,
			MissingMessage:  "go not found; onlava requires Go 1.26 or newer",
		...
		},
		{
			...
			SuggestedAction: "Install Atlas if you need `onlava generate sqlc` to refresh schema SQL from Atlas definitions.",
		},
		{
			ID:              "tool.sqlc",
			Name:            "SQLC",
		...
		if spec.ID == "tool.go" {
			...
			if !ok {
				...
				if versionInfo.compare(doctorGoVersion{Major: doctorMinGoMajor, Minor: doctorMinGoMinor}) < 0 {
					check.Status = doctorStatusError
					check.Message = fmt.Sprintf("%s found at %s; onlava requires Go %d.%d or newer", versionInfo.String(), path, doctorMinGoMajor, doctorMinGoMinor)
...
func writeDoctorText(w io.Writer, resp doctorResponse) error {
	if _, err := fmt.Fprintln(w, "onlava doctor"); err != nil {
		return err
	}
