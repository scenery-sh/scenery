...
const (
	...
	edgeHelperLabel      = "dev.onlava.edge-helper"
	edgeHelperBinaryPath = "/usr/local/libexec/onlava-edge-helper"
	edgeHelperPlistPath  = "/Library/LaunchDaemons/dev.onlava.edge-helper.plist"
	edgeHelperSupportDir = "/Library/Application Support/Onlava/edge-helper"
	edgeHelperLogPath    = "/Library/Application Support/Onlava/edge-helper/edge-helper.log"
)
...
func edgeRestart(opts edgeOptions) error {
	...
	if err := ensureEdgeAgent(upstreamAddr, true); err != nil {
		...
		return fmt.Errorf("refresh onlava agent after edge start: %w", refreshErr)
	}
...
func edgeTrust(opts edgeOptions) error {
	...
	fmt.Fprintln(os.Stdout, "trusted onlava Caddy local CA")
	return nil
}
...
func trustCaddyLocalCA(caddyBin string) error {
	dir, err := os.MkdirTemp("", "onlava-caddy-trust-")
	if err != nil {
		return err
	}
...
func ensureEdgeAgent(routerAddr string, force bool) error {
	...
	if running && health.PID > 0 {
		if err := signalAgentPID(health.PID); err != nil {
			return fmt.Errorf("stop onlava agent pid %d: %w", health.PID, err)
		}
...
%s {
	...
	reverse_proxy %s {
		...
		header_up X-Onlava-Edge-Token %s
	}
}
...
func edgePrivilegedInstall() error {
	...
	fmt.Fprintln(os.Stdout, "installed onlava privileged edge listener for 127.0.0.1:443")
	return nil
}
...
func edgePrivilegedHelperInstall(opts edgeHelperOptions) error {
	...
	if err := stopStaleRootOnlavaEdgeAgent(opts.OwnerHome, opts.RouterAddr, 2*time.Second); err != nil {
		return err
	}
...
func edgePrivilegedHelperUninstall() error {
	...
	fmt.Fprintln(os.Stdout, "removed onlava privileged edge listener")
	return nil
}
...
func stopStaleRootOnlavaEdgeAgent(ownerHome, routerAddr string, timeout time.Duration) error {
	if os.Geteuid() != 0 {
