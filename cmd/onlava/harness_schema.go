...
const harnessSchemaValidationSchema = "onlava.harness.schema_validation.v1"
...
func runHarnessSchemaValidationStep(repoRoot string, resp harnessSelfResponse) (harnessStep, *harnessSchemaValidationReport) {
	...
	step := harnessStep{
		...
		Command:    []string{"onlava", "harness", "self", "internal:schema-validation", repoRoot},
...
func buildHarnessSchemaValidationReport(repoRoot string, resp harnessSelfResponse) *harnessSchemaValidationReport {
	...
	items := []struct {
		...
		{name: "harness.test_timing", schemaRel: "docs/schemas/onlava.harness.test_timing.v1.schema.json", payload: resp.TestTiming},
		{name: "harness.fixture_matrix", schemaRel: "docs/schemas/onlava.harness.fixture_matrix.v1.schema.json", payload: resp.FixtureMatrix},
		{name: "harness.schema_validation", schemaRel: "docs/schemas/onlava.harness.schema_validation.v1.schema.json", payload: report},
		{name: "agent_context", schemaRel: "docs/schemas/onlava.agent_context.v1.schema.json", payload: buildHarnessAgentContext(repoRoot, resp)},
	}
...
func harnessInspectDocsPayload(repoRoot string) (map[string]any, error) {
	...
	if err := runOnlavaInspect([]string{"docs", "--repo-root", repoRoot, "--json"}, &out); err != nil {
		return nil, err
	}
