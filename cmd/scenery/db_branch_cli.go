...
func runDBBranchRestore(ctx context.Context, stdout io.Writer, opts dbBranchOptions) error {
	...
	if strings.TrimSpace(opts.At) == "" {
		return fmt.Errorf("scenery db branch restore requires --at <timestamp-or-lsn>")
	}
	...
	point, err := (sqliteBranchProvider{cfg: cfg}).RestoreBranch(ctx, pin, opts)
	if err != nil {
		return err
	}
	if opts.JSON {
		return writeInspectJSON(stdout, map[string]any{
			"schema_version": "scenery.db.branch.restore.v1",
			"branch":         pin.Branch,
...
func runDBBranchDiff(ctx context.Context, stdout io.Writer, opts dbBranchOptions) error {
	...
	if target == "" {
		return fmt.Errorf("usage: scenery db branch diff <branch> [--app-root <path>] [--json]")
	}
	pin, ok, err := readWorktreeDBPin(worktreeDBPinPath(appRoot))
	if err != nil {
		return err
	...
	diff, err := (sqliteBranchProvider{cfg: cfg}).DiffBranch(ctx, pin, target, opts)
	if err != nil {
		return err
	}
	if opts.JSON {
		return writeInspectJSON(stdout, map[string]any{
			"schema_version": "scenery.db.branch.diff.v1",
			"branch":         pin.Branch,
			"target":         target,
			"diff":           diff,
		})
