...
func TestHarnessBuildInfoProbeStepReportsInjectedResult(t *testing.T) {
	t.Parallel()
	step := runHarnessBuildInfoProbeStepWithCheck(context.Background(), "/repo", func(_ context.Context, repoRoot string) (map[string]any, []checkDiagnostic, error) {
		if repoRoot != "/repo" {
			t.Fatalf("repo root = %q", repoRoot)
		}
		return map[string]any{"proof": "ok"}, nil, nil
	})
	if !step.OK || step.Name != harnessBuildInfoProbeName || step.Summary["proof"] != "ok" {
	...
	failed := runHarnessBuildInfoProbeStepWithCheck(context.Background(), "/repo", func(context.Context, string) (map[string]any, []checkDiagnostic, error) {
