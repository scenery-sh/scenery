...
func buildVNextInspectAppResponse(appRoot string, cfg appcfg.Config, result *vnext.Result) inspectdata.AppResponse {
	services := vnextInspectServices(result)
	endpoints, _ := vnextInspectEndpoints(result)
	...
	response := inspectdata.AppResponse{
		SchemaVersion: "scenery.inspect.app.v1", App: vnextInspectAppRef(appRoot, cfg, result), Config: cfg,
...
func buildVNextInspectServicesResponse(appRoot string, cfg appcfg.Config, result *vnext.Result) inspectdata.ServicesResponse {
	return inspectdata.ServicesResponse{SchemaVersion: "scenery.inspect.services.v1", App: vnextInspectAppRef(appRoot, cfg, result), Services: vnextInspectServices(result)}
}

func buildVNextInspectRoutesResponse(appRoot string, cfg appcfg.Config, result *vnext.Result) (inspectdata.RoutesResponse, error) {
	endpoints, err := vnextInspectEndpoints(result)
	if err != nil {
		return inspectdata.RoutesResponse{}, err
	}
	serviceRoots := vnextServiceRoots(result)
	...
	return inspectdata.RoutesResponse{SchemaVersion: "scenery.inspect.routes.v1", App: vnextInspectAppRef(appRoot, cfg, result), Routes: routes}, nil
}

func buildVNextInspectEndpointsResponse(appRoot string, cfg appcfg.Config, result *vnext.Result) (inspectdata.EndpointsResponse, error) {
	endpoints, err := vnextInspectEndpoints(result)
	if err != nil {
		return inspectdata.EndpointsResponse{}, err
	}
	return inspectdata.EndpointsResponse{SchemaVersion: "scenery.inspect.endpoints.v1", App: vnextInspectAppRef(appRoot, cfg, result), Endpoints: endpoints}, nil
}

func buildVNextInspectDurableResponse(appRoot string, cfg appcfg.Config, result *vnext.Result) inspectDurableResponse {
	...
	for _, execution := range result.Manifest.Resources {
		if execution.Kind != "scenery.execution/v1" || inspectString(execution.Spec["mode"]) != "durable" {
			continue
		}
	...
	return inspectDurableResponse{
		SchemaVersion: "scenery.inspect.durable.v2", App: vnextInspectAppRef(appRoot, cfg, result),
...
func inspectOperationOutputType(operation vnext.Resource) string {
...
func vnextInspectServices(result *vnext.Result) []inspectdata.ServiceDetails {
	...
	roots := vnextServiceRoots(result)
...
func vnextInspectEndpoints(result *vnext.Result) ([]inspectdata.EndpointRecord, error) {
	if result == nil || result.Manifest == nil {
...
func vnextInspectAppRef(appRoot string, cfg appcfg.Config, result *vnext.Result) inspectdata.AppRef {
	modulePath := ""
	for _, resource := range result.Manifest.Resources {
...
func inspectBindingAccess(binding vnext.Resource) string {
...
func inspectResolveReference(resource vnext.Resource, reference, kind string) string {
