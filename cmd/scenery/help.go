...
var helpCommands = []helpCommandEntry{
	...
	{Command: "migrate", Group: "Edition 2027", Summary: "Plan, apply, and inspect bounded mixed legacy/native migration state.", Usage: []string{"scenery migrate status [--app-root <path>] [-o human|json]", "scenery migrate verify <service> [--app-root <path>] [-o human|json]", "scenery migrate activate <service> --native --dry-run --out <plan> [--evidence <class>=<reference>] [-o human|json]", "scenery migrate apply <plan> [--expect-workspace-revision <rev>] [--expect-contract-revision <rev>] [--approval-token <file>] [-o human|json]"}, Subcommands: []string{"init", "status", "verify", "compare", "service", "activate", "rollback", "apply", "finish"}, Flags: []string{"--app-root <path>", "--dry-run", "--out <plan>", "--evidence <class>=<reference>", "--approval-token <file>", "-o human|json"}, JSON: true, Stability: "draft"},
	{
		Command:   "up",
		Group:     "Local dev",
	...
	},
	{
		...
		Subcommands: []string{"list", "shell", "apply", "seed", "setup", "reset", "drop", "diff", "snapshot", "server"},
		Flags:       []string{"--app-root <path>", "--json", "--dry-run", "--generated", "--yes"},
		JSON:        true,
		Stability:   "stable",
	},
	{
