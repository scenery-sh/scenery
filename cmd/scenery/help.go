...
var helpCommands = []helpCommandEntry{
	...
	},
	{
		...
		Usage:         []string{"scenery telemetry [--app <id-or-name>]... [--command <coarse-command>]... [--measurement completion|startup]... [--since <duration>] [--limit <n>] [-o human|json]"},
		Flags:         []string{"--app <id-or-name>", "--command <coarse-command>", "--measurement completion|startup", "--since <duration>", "--limit <n>", "-o human|json"},
		Notes:         []string{"Repeat --app, --command, or --measurement to match any requested value.", "scenery up records startup immediately when its new owner reaches readiness; it does not record supervisor lifetime or already-running acquisition as startup.", "p50/p95 use the latest bounded 10,000 matching records and report their sample count.", "New records carry configured app ID and name only; filesystem paths and raw arguments are never recorded.", "Historical records without app identity remain visible as unattributed unless an app filter is selected."},
