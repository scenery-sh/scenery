...
var helpCommands = []helpCommandEntry{
	...
	{Command: "explain", Group: "App contract", Summary: "Explain a canonical resource and provenance.", Usage: []string{"scenery explain <address> [-o human|json]"}, Flags: []string{"-o human|json"}, JSON: true, Stability: "stable"},
	{
		...
		Usage:     []string{"scenery up [--port <n>] [--listen <addr>] [--app-root <path>] [--claim-aliases] [--verbose] [-o jsonl] [--detach] [--wait=ready|registered]"},
		Flags:     []string{"--port <n>", "--listen <addr>", "--app-root <path>", "--claim-aliases", "--verbose", "-o", "json", "--detach", "--wait=ready|registered"},
		JSON:      true,
		Stability: "stable",
	},
	{
	...
	},
	{
		...
		Usage:       []string{"scenery deploy <ssh-target> [--app-root <path>]", "scenery deploy enable [--app-root <path>] [-o json]", "scenery deploy disable [--app-root <path>] [-o json]", "scenery deploy status [-o json]", "scenery deploy setup [--acme-email <email>] [--acme-ca production|staging] [-o json]", "scenery deploy resume [-o json]", "scenery deploy teardown [-o json]"},
		...
		Notes:       []string{"The positional SSH target performs beta source sync with brief downtime.", "`enable`, `disable`, `status`, `setup`, `resume`, and `teardown` manage the local public edge."},
		JSON:        true,
		Stability:   "beta",
	},
	{
