...
func TestManagedElectricProcessEnvExpandsDatabaseURL(t *testing.T) {
	...
	env := managedElectricProcessEnv(plan, []string{"PATH=/bin", "DATABASE_URL=postgres://old/db"}, "postgres://session/db", 3456, "scenery_session")
	for _, want := range []string{
		"DATABASE_URL=postgres://session/db",
		"DatabaseURL=postgres://session/db",
		"ELECTRIC_PORT=3456",
		"PORT=3456",
		"ELECTRIC_REPLICATION_STREAM_ID=scenery_session",
		"ELECTRIC_USAGE_REPORTING=false",
	} {
		if !containsString(env, want) {
...
func TestManagedElectricProcessEnvIncludesSessionIdentity(t *testing.T) {
	...
	plan := &managedElectricPlan{}
	env := managedElectricProcessEnv(plan, []string{"PATH=/bin"}, "postgres://session/db", 3456, "scenery_session",
		"SCENERY_APP_ROOT=/repo/app",
		"SCENERY_SESSION_ID=main-dbe32e",
		"SCENERY_BASE_APP_ID=demo",
		"SCENERY_RUNTIME_APP_ID=demo--main-dbe32e",
...
func TestManagedElectricReplicationStreamIDUsesSessionIdentifier(t *testing.T) {
	...
	if got != "scenery_feature_my_worktree_123456" {
...
func TestManagedElectricSlotDiagnostics(t *testing.T) {
	...
	slot := managedElectricSlotName("scenery_main_dbe32e")
	if slot != "electric_slot_scenery_main_dbe32e" {
	...
	got := describeElectricPostgresLocks([]electricPostgresLock{{
		...
		WaitEventType: "Lock",
		WaitEvent:     "advisory",
		Query:         "SELECT pg_advisory_lock(hashtext('electric_slot_scenery_main_dbe32e'))",
		Application:   "scenery-electric:root:session:runtime:stream",
		ClientAddr:    "127.0.0.1",
		SlotName:      slot,
	}})
	for _, want := range []string{"pid=123", "kind=advisory-lock", "wait=Lock/advisory", "electric_slot_scenery_main_dbe32e", "application=", "client=127.0.0.1", "slot="} {
		if !strings.Contains(got, want) {
...
func TestManagedElectricPostgresApplicationNameIsScopedAndURLSafe(t *testing.T) {
	...
	first := managedElectricPostgresApplicationName(root, session, "scenery_main_dbe32e")
	second := managedElectricPostgresApplicationName(root, session, "scenery_main_dbe32e")
	otherRoot := managedElectricPostgresApplicationName("/repo/other", session, "scenery_main_dbe32e")
	otherRuntime := managedElectricPostgresApplicationName(root, &localagent.Session{SessionID: "main-dbe32e", BaseAppID: "demo", RuntimeAppID: "demo--other"}, "scenery_main_dbe32e")
	...
	if !strings.Contains(got, "application_name=scenery-electric%3A") || !strings.Contains(got, "sslmode=disable") {
...
func TestCleanupManagedElectricStreamProcessesStopsSameSessionProcess(t *testing.T) {
	...
	listManagedElectricStreamProcesses = func(streamID string) ([]managedElectricStreamProcess, error) {
		if streamID != "scenery_main_dbe32e" {
		...
		return []managedElectricStreamProcess{{
			...
			State:   "S",
			Stream:  streamID,
			Command: "docker run -e ELECTRIC_REPLICATION_STREAM_ID=scenery_main_dbe32e -e SCENERY_APP_ROOT=" + root + " -e SCENERY_SESSION_ID=main-dbe32e -e SCENERY_RUNTIME_APP_ID=demo--main-dbe32e -e SCENERY_DEV_SUPERVISOR=1 -e SCENERY_ROLE=electric electricsql/electric:canary",
		}}, nil
	}
	defer func() { listManagedElectricStreamProcesses = restore }()

	err := cleanupManagedElectricStreamProcesses(context.Background(), root, &localagent.Session{SessionID: "main-dbe32e", BaseAppID: "demo", RuntimeAppID: "demo--main-dbe32e"}, "scenery_main_dbe32e")
	if err != nil {
...
func TestCleanupManagedElectricStreamProcessesFailsForLiveUnownedStream(t *testing.T) {
	...
	err := cleanupManagedElectricStreamProcesses(context.Background(), t.TempDir(), &localagent.Session{SessionID: "main-dbe32e"}, "scenery_main_dbe32e")
	if err == nil || !strings.Contains(err.Error(), "pid=424242") || !strings.Contains(err.Error(), "scenery_main_dbe32e") {
...
func TestCleanupManagedElectricStreamProcessesFailsForDifferentRuntimeSameStream(t *testing.T) {
	...
	listManagedElectricStreamProcesses = func(streamID string) ([]managedElectricStreamProcess, error) {
		return []managedElectricStreamProcess{{
			...
			State:   "S",
			Stream:  streamID,
			Command: "docker run -e ELECTRIC_REPLICATION_STREAM_ID=" + streamID + " -e SCENERY_APP_ROOT=" + root + " -e SCENERY_SESSION_ID=main-dbe32e -e SCENERY_RUNTIME_APP_ID=demo--other -e SCENERY_DEV_SUPERVISOR=1 -e SCENERY_ROLE=electric electricsql/electric:canary",
		}}, nil
	}
	defer func() { listManagedElectricStreamProcesses = restore }()

	err := cleanupManagedElectricStreamProcesses(context.Background(), root, &localagent.Session{SessionID: "main-dbe32e", BaseAppID: "demo", RuntimeAppID: "demo--main-dbe32e"}, "scenery_main_dbe32e")
	if err == nil || !strings.Contains(err.Error(), "demo--other") || !strings.Contains(err.Error(), "state=S") || !strings.Contains(err.Error(), `stream="scenery_main_dbe32e"`) {
		t.Fatalf("cleanupManagedElectricStreamProcesses error = %v", err)
	}
}
...
func TestCommandContainsEnvValueRequiresExactValue(t *testing.T) {
	...
	command := "docker run -e ELECTRIC_REPLICATION_STREAM_ID=scenery_main_dbe32e_other electricsql/electric:canary"
	if commandContainsEnvValue(command, "ELECTRIC_REPLICATION_STREAM_ID", "scenery_main_dbe32e") {
...
func TestClassifyElectricPostgresLocksForCleanupUsesExactApplication(t *testing.T) {
	...
	ownedApp := "scenery-electric:root:session:runtime:stream"
	locks := []electricPostgresLock{
		{PID: 101, Application: ownedApp, SlotName: "electric_slot_scenery_main_dbe32e"},
		{PID: 202, Application: ownedApp + "-other", SlotName: "electric_slot_scenery_main_dbe32e"},
		{PID: 303, Application: "", SlotName: "electric_slot_scenery_main_dbe32e"},
	}
...
func TestTextMentionsExactIdentifier(t *testing.T) {
	...
	if !textMentionsExactIdentifier("SELECT pg_advisory_lock(hashtext('electric_slot_scenery_main_dbe32e'))", "electric_slot_scenery_main_dbe32e") {
		t.Fatal("expected exact slot mention")
	}
	if textMentionsExactIdentifier("SELECT 'electric_slot_scenery_main_dbe32e_other'", "electric_slot_scenery_main_dbe32e") {
...
func TestManagedElectricDatabaseURLPrefersManagedPostgres(t *testing.T) {
	...
	}, baseEnv, got, 3456, "scenery_session")
...
func TestManagedElectricDatabaseURLUsesReadyPostgresBranch(t *testing.T) {
	t.Setenv("SCENERY_AGENT_HOME", t.TempDir())
	t.Setenv(devPostgresAdminURLEnv, "postgres://scenery:secret@127.0.0.1:55436/postgres?sslmode=disable")
	root := t.TempDir()
	cfg := app.Config{
		Name: "demo",
		ID:   "demo",
	...
	if got != "postgres://scenery:secret@127.0.0.1:55436/demo_demo_review_a?sslmode=disable" {
		t.Fatalf("database URL = %q", got)
	}
}
...
func TestManagedElectricContainerEnvUsesInternalPort(t *testing.T) {
	...
	env := managedElectricContainerEnv(plan, []string{"PATH=/bin"}, "postgres://scenery:postgres@127.0.0.1:55432/session?sslmode=disable", "scenery_session")
	for _, want := range []string{
		"DATABASE_URL=postgres://scenery:postgres@host.docker.internal:55432/session?sslmode=disable",
		"ELECTRIC_PORT=3000",
		"PORT=3000",
		"ELECTRIC_REPLICATION_STREAM_ID=scenery_session",
