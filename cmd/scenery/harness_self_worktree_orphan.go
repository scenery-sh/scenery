...
func (p *worktreeRuntimeProbe) orphanCleanup(source, sibling string) error {
	return p.scenario("A12", "ordinary Git removal leaves discoverable data and retryable exact cleanup", func(e map[string]any) error {
		...
		resolver, err := newWorktreePostgresResolver(p.ctx, root, before.AppID)
		if err != nil {
			return err
		}
		volume, container, err := resolver.docker.Inspect(p.ctx, before)
		if err != nil || volume != nil || container != nil {
			return fmt.Errorf("selected orphan resources remain after successful cleanup: %v", err)
		}
