...
func TestSupervisorClosePropagatesCtrlCToAppProcessGroup(t *testing.T) {
	if os.Getenv("SCENERY_TEST_SUPERVISOR_CLOSE_HELPER") == "1" {
	...
	cmd.Env = append(os.Environ(),
		"SCENERY_TEST_SUPERVISOR_CLOSE_HELPER=1",
		"SCENERY_TEST_READY="+readyPath,
		"SCENERY_TEST_INTERRUPTED="+interruptPath,
		"SCENERY_TEST_GRANDCHILD="+grandchildPath,
	)
	cmd.Stdin = nil
...
child=$!
echo "$child" > "$SCENERY_TEST_GRANDCHILD"
echo ready > "$SCENERY_TEST_READY"
wait "$child"
`, map[string]string{
		"SCENERY_TEST_READY":      readyPath,
		"SCENERY_TEST_GRANDCHILD": grandchildPath,
...
func TestSupervisorCloseStopsTypeScriptWorkerAndClearsState(t *testing.T) {
	...
	cmd, done := startShellProcessTree(t, `
set -eu
trap 'echo interrupted > "$SCENERY_TEST_INTERRUPTED"; exit 0' INT TERM
echo ready > "$SCENERY_TEST_READY"
...
func TestDetachedDevReapsStaleTypeScriptWorker(t *testing.T) {
	...
	root := t.TempDir()
	outputDir := filepath.Join(root, ".scenery", "generated", "temporal", "typescript")
...
func TestSecondCtrlCUsesDefaultSignalBehavior(t *testing.T) {
	if os.Getenv("SCENERY_TEST_SECOND_CTRL_C_HELPER") == "1" {
	...
	cmd.Env = append(os.Environ(),
		"SCENERY_TEST_SECOND_CTRL_C_HELPER=1",
		"SCENERY_TEST_READY="+readyPath,
		"SCENERY_TEST_FIRST="+firstPath,
	)
	cmd.Stdin = nil
	configureChildProcess(cmd)
	if err := cmd.Start(); err != nil {
...
func runSecondCtrlCHelper() {
	...
	<-ctx.Done()
	_ = os.WriteFile(os.Getenv("SCENERY_TEST_FIRST"), []byte("first"), 0o644)
	select {}
}
...
func runSupervisorCloseHelper() {
	...
	_ = os.WriteFile(os.Getenv("SCENERY_TEST_GRANDCHILD"), []byte(strconv.Itoa(child.Process.Pid)), 0o644)
	_ = os.WriteFile(os.Getenv("SCENERY_TEST_READY"), []byte("ready"), 0o644)

	<-signals
	_ = os.WriteFile(os.Getenv("SCENERY_TEST_INTERRUPTED"), []byte("interrupted"), 0o644)
