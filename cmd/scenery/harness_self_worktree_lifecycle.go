...
func (p *worktreeRuntimeProbe) verifiedContainerCommand(record localagent.WorktreeRecord, action string, args ...string) error {
	resolver, err := newWorktreePostgresResolver(p.ctx, record.AppRoot, record.AppID)
	if err != nil {
		return err
	}
	_, container, err := resolver.inspect(p.ctx, record)
	if err != nil {
		return err
	}
	if container == nil || container.ID != record.Postgres.ContainerID {
		return fmt.Errorf("probe container identity changed before fault injection")
	}
	argv := append([]string{"--host", record.Postgres.DaemonEndpoint, action}, args...)
	argv = append(argv, container.ID)
