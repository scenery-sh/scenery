...
const grafanaRequestDurationMetricName = "scenery_request_duration_seconds"
...
func renderGrafanaDatasources(cfg grafanaConfig) ([]byte, error) {
	...
	if cfg.MetricsURL != "" {
		fmt.Fprintf(&buf, "  - name: scenery VictoriaMetrics\n")
	...
	if cfg.LogsURL != "" {
		fmt.Fprintf(&buf, "  - name: scenery VictoriaLogs\n")
		fmt.Fprintf(&buf, "    orgId: 1\n")
		fmt.Fprintf(&buf, "    uid: %s\n", cfg.LogsDatasource)
		fmt.Fprintf(&buf, "    version: 1\n")
	...
	if cfg.TracesURL != "" {
		fmt.Fprintf(&buf, "  - name: scenery VictoriaTraces\n")
		fmt.Fprintf(&buf, "    orgId: 1\n")
...
func renderGrafanaDashboardProvider(cfg grafanaConfig) ([]byte, error) {
	...
	fmt.Fprintf(&buf, "  - name: scenery\n")
	fmt.Fprintf(&buf, "    orgId: 1\n")
	fmt.Fprintf(&buf, "    folder: scenery\n")
...
func writeGrafanaProvisioning(ctx context.Context, cfg grafanaConfig) error {
	...
	if data, err := renderGrafanaDatasources(cfg); err != nil {
		return err
	} else if err := atomicWriteFile(filepath.Join(cfg.ProvisioningDir, "datasources", "scenery.yaml"), data, 0o644); err != nil {
		return err
	}
	if data, err := renderGrafanaDashboardProvider(cfg); err != nil {
		return err
	} else if err := atomicWriteFile(filepath.Join(cfg.ProvisioningDir, "dashboards", "scenery.yaml"), data, 0o644); err != nil {
		return err
	}
...
func grafanaOverviewDashboard(cfg grafanaConfig) map[string]any {
	requestDuration := grafanaMetricSelector(grafanaRequestDurationMetricName, `scenery_session_id=~"$session"`)
	errorDuration := grafanaMetricSelector(grafanaRequestDurationMetricName, `scenery_session_id=~"$session"`, `scenery_is_error="true"`)
	dashboard := baseGrafanaDashboard(grafanaOverviewUID, "scenery up overview", []any{
		...
		traceSearchPanel(6, "Request traces", "$app", "scenery.trace.type=REQUEST", 0, 12, 24, 8),
	})
...
func grafanaLogsDashboard(cfg grafanaConfig) map[string]any {
	dashboard := baseGrafanaDashboard(grafanaLogsDashboardUID, "scenery up logs", []any{
...
func grafanaEndpointDashboard(cfg grafanaConfig) map[string]any {
	...
	endpointDuration := grafanaMetricSelector(requestDuration, `scenery_session_id=~"$session"`, `scenery_service="$service"`, `scenery_endpoint="$endpoint"`)
	endpointErrors := grafanaMetricSelector(requestDuration, `scenery_session_id=~"$session"`, `scenery_service="$service"`, `scenery_endpoint="$endpoint"`, `scenery_is_error="true"`)
	dashboard := baseGrafanaDashboard(grafanaEndpointUID, "scenery up endpoint", []any{
...
func sessionVariable(metric string) map[string]any {
	variable := queryVariable("session", fmt.Sprintf(`label_values(%s, scenery_session_id)`, metric))
...
func appVariable(metric string) map[string]any {
	variable := queryVariable("app", fmt.Sprintf(`label_values(%s, scenery_app)`, metric))
	variable["label"] = "App"
	return variable
}
