...
func startHeadlessApp(root string, cfg app.Config, binary, addr string, opts serveOptions) error {
	...
	cmd.Dir = root
	env, err := appProcessEnv(root, cfg, opts.LogFormat, opts.Env, "SCENERY_LISTEN_ADDR="+addr, "SCENERY_ROLE="+headlessRuntimeRole(cfg))
	if err != nil {
		return err
	}
	cmd.Env = env
	...
	if err != nil {
		return fmt.Errorf("scenery app exited: %w", err)
	}
	return nil
}
