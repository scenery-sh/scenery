...
const (
	...
	scriptRunUsage       = "usage: scenery task run [--app-root <path>] [--env <name>] [--lang go|typescript] <domain>:<name> [-- task args...]"
)
...
func runSceneryScript(ctx context.Context, opts scriptOptions) error {
...
func runScriptProcess(ctx context.Context, root string, cfg app.Config, program string, args []string, opts scriptOptions) error {
	...
	if err != nil {
		return fmt.Errorf("scenery task run exited: %w", err)
	}
	return nil
}
