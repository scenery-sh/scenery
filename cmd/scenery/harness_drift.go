...
func runHarnessToolchainPreflightStep(ctx context.Context, repoRoot string) (harnessStep, *harnessToolchainReport) {
	...
	step := harnessStep{
		...
		Command:    []string{"scenery", "harness", "self", "internal:toolchain-preflight", repoRoot},
...
func runHarnessDriftStep(ctx context.Context, repoRoot string) (harnessStep, *harnessDriftReport) {
	...
	step := harnessStep{
		...
		Command:    []string{"scenery", "harness", "self", "internal:drift-check", repoRoot},
		OK:         !hasErrorDiagnostics(report.Diagnostics),
		DurationMS: time.Since(started).Milliseconds(),
		Summary: map[string]any{
	...
	for _, spec := range []struct {
		...
		if !item.Usage {
			...
			diagnostics = append(diagnostics, checkDiagnostic{
				...
				Severity:        "error",
				File:            filepath.ToSlash(filepath.Join(repoRoot, "cmd", "scenery", "main.go")),
				Message:         item.Error,
...
func runHarnessFixtureMatrixStep(ctx context.Context, repoRoot string) (harnessStep, *harnessFixtureMatrixReport) {
	...
	step := harnessStep{
		Name:       "fixture matrix",
		Command:    []string{"scenery", "harness", "self", "internal:fixture-matrix", repoRoot},
		OK:         !hasErrorDiagnostics(report.Diagnostics),
		DurationMS: time.Since(started).Milliseconds(),
		Summary: map[string]any{
