...
- CRUD resources can declare explicit search/filter/sort capabilities with fingerprint-bound cursor pagination. `table_page` can use that paginated CRUD list or an existing call-delivery HTTP binding whose named result field is one complete typed list. It can compose server metrics, declarative filters, reusable `status_map` badges, loaded-result CSV, visible/export-only columns, inline or resizable-panel row detail, and generated mutation `form_dialog` actions; binding-backed complete-list tables may additionally declare client-side grouping with collapsible counted sections, while paginated CRUD tables cannot group. The generated workbench keeps every filter in one popover, duplicates only `pinned = true` selectors inline, summarizes active filters as chips, and keeps grouping and ordering controls separate. `content_page`, `table_page`, and generic `split_page` still expand to ordinary page/renderer resources. React-enabled TypeScript clients materialize the binary-owned catalog, generated pages, typed route descriptors, TanStack route tree, navigation, and app shell only after staged verification by Scenery's exact managed native TypeScript checker. The catalog exposes composed components plus blessed Astryx primitives from `@scenery/ui` and semantic StyleX variables from the sole `@scenery/ui/tokens.stylex` subpath. Domain-specific UI remains in app-owned `react_component` slots or the generated app's fixed extension/visual slots, never in Scenery's catalog or compiler.
