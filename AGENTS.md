...
- `scenery deploy <ssh-target>` is beta single-server source sync: the target must be allowed by `deploy.ssh`, OpenSSH owns connection configuration, rsync honors `.gitignore` and preserves remote `.env` and `.scenery` state, and the remote current Scenery binary stops then restarts the app with readiness waiting. Brief downtime is intentional. When the app declares `deploy.domain` plus a `serve: "production"` frontend, the deploy additionally runs remote `scenery deploy publish`: the frontend builds on the remote host, lands as an immutable release under the agent home's `deploy-artifacts` store with an atomic `current` switch, and the managed Caddy edge serves it directly from disk (blocked Scenery paths and `/api/*` stay on the trusted agent router); validation, reload, or probe failures roll back to the previous publication, and `scenery deploy status` reports per-frontend serving mode.
- Public deploy hosts have two service managers: launchd on macOS (privileged loopback helper) and systemd on Linux (`scenery deploy setup` as root installs `scenery-agent.service`, `scenery-edge.service` binding public 80/443 directly, and a boot-time deploy resume oneshot). While the edge unit exists, edge restart/reload paths converge through systemd.
...
- CRUD resources can declare an explicit list filter/sort contract with fingerprint-bound cursor pagination. `table_page` expands to ordinary page/renderer resources, and React-enabled TypeScript clients materialize the binary-owned catalog and generated pages only after staged verification by Scenery's exact managed native TypeScript checker.
