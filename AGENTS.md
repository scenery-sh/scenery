...
- `scenery deploy <ssh-target>` is beta single-server source sync: the target must be allowed by `deploy.ssh`, OpenSSH owns connection configuration, rsync preserves remote `.env` and `.scenery` state, and the remote current Scenery binary stops then restarts the app with readiness waiting. Brief downtime is intentional.
