...
For a generated two-pane screen, use the generic `split_page` kind with a unit-input HTTP operation and app-owned `pane` and `detail` `react_component` slots; `pane_actions` and `detail_header` are optional. Scenery owns only transport, state, selection, and reusable layout. Keep every domain-specific component in the client app.
