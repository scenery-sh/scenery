...
For a terminal HTTP tail, explicitly require `scenery.http-path-tail/v1` and `scenery.runtime-http-path-tail/v1`, use only final `{name...}` syntax, and add one matching `path_tail "name"` mapping to `string`, `relative_path`, or `optional(relative_path)`. The empty tail omits the slash; non-empty TypeScript values are semantic text split and encoded one segment at a time. Do not use router globs, pre-encoded fragments, raw request access, or filesystem path cleaning.
