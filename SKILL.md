...
Prefer JSON output for agent decisions. Prefer `scenery up` for local development. Use `scenery build` when you need a deployable app binary. Use `scenery task` for configured and code tasks. Use `scenery validate` for app-owned quality gates. Use `scenery worker` for worker-only durable and cron execution.
...
scenery check --json
scenery build -o ./app
SCENERY_LISTEN_ADDR=127.0.0.1:4000 ./app
...
Standard auth can be enabled from app config without app-local wrapper endpoints. Its tenant tables are framework-owned managed SQLite tables with the `scenery_auth_` prefix; app-local `tenants` services or tables are product-domain concerns only.
