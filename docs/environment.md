...
| `SCENERY_PROCESS_LINK` | injected | Private process-link file for development service processes and their host: session token and the host's private dispatch listener. Invalid wiring fails runtime startup. |
...
| `SCENERY_DEV_PROCESS_MODEL` | user input | `service` runs `scenery up` as a process host plus one process per native service; unset or `application` keeps one application executable. Temporary Plan 0200 rollout gate; applications with event consumers or emissions are not supported yet. |
