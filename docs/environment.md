...
| `SCENERY_DEV_PROCESS_MODEL` | user input | `service` runs `scenery up` as a process host plus one process per native service; unset or `application` keeps one application executable. Temporary Plan 0200 rollout gate; applications with assistants are not supported yet. |
