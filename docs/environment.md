...
| `ONLAVA_DEV_NEON_SELFHOST_DRIVER` | user input | Explicit external `neon-selfhost` branch driver executable path for overriding the built-in self-hosted Neon dev-cell backend. Onlava invokes it with the same branch JSON contract as the fallback driver and prefers it over `ONLAVA_DEV_LOCAL_POSTGRES_BRANCH_DRIVER` when both are set. |
| `ONLAVA_NEON_SELFHOST_ROOT` | internal | Used by the built-in `neon-selfhost-driver` so it reads and writes the same local substrate root as the `onlava db neon ...` commands. Users should not set it directly. |
