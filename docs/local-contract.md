...
- Generated binaries accept `SCENERY_ROLE=all|api|worker`. `scenery up` uses the default combined role. Operator-launched app binaries may use `api` for API-only execution. `scenery worker` uses `worker`.
...
- `scenery logs`, plain `scenery logs --follow`, and `scenery console` read structured dev events for the selected app root's live runtime from VictoriaLogs.
...
- `scenery console` opens the source-aware terminal console when stdin/stdout are real TTYs. In CI, dumb terminals, or redirected output it falls back to normal log following.
...
- `scenery build` produces a generated app binary that can be launched directly by an operator or deployment system.
- Directly launched generated app binaries do not start the dashboard, managed frontends, agent routing, or file watcher.
...
- Cron declarations run through the in-process scheduler. Generated app binaries with API/all roles reconcile schedules, while `scenery worker` executes cron jobs without starting the public HTTP server.
...
- Default Caddy, Victoria sidecar, ZeroFS, and managed image versions are pinned in `scenery.toolchain.json`; environment variables override explicit startup controls for local testing where documented. Caddy edge and managed ZeroFS storage are managed-toolchain only.
...
- Operator-launched generated binaries can use process environment without a `.env` file. In production mode, declared secrets are required by the runtime.
