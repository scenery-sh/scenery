...
- `dev.services` is a beta local-development config surface for onlava-owned substrates. It accepts `postgres`, `neon`, and `electric` service declarations with `kind`, `version`, `isolation`, `image`, `database`, `route`, string `env`, and Neon branch fields where documented. The agent owns managed Postgres, the Neon dev cell, and Electric for this surface, while unsupported service kinds, Neon modes, branch policies, or isolation modes are rejected instead of silently falling back to target-app port orchestration.
- `dev.services.postgres` defaults to `kind: "postgres"`, version `18`, and `isolation: "database"`. With `kind: "neon"`, only `mode: "self-hosted"` and `isolation: "branch"` are accepted. Neon branch fields are `project`, `parent_branch`, `branch_policy: "manual" | "worktree" | "session"`, `branch_name_template` with `{app}`, `{git_branch}`, `{worktree}`, and `{session}`, `ttl`, `database`, `role`, and `database_url_env`. Onlava keeps the command spelling as `onlava up`; `onlava dev` is not an active contract.
- `.onlava/worktree-db.json` is the worktree-local Neon branch pin. It uses schema `onlava.db.branch.v1` with `provider: "neon-self-hosted"`, project, parent branch, branch name/id, database, role, session/worktree identity, owner marker, TTL, and timestamps. Raw connection URLs stay process-injected and are not written to this file.
- `onlava up` prepares declared local DB setup before the app process starts. When `.onlava.json` declares `database.apply` or service-local seed files are discovered, the supervisor runs the same split lifecycle as `onlava db setup`: apply first, then seed. It passes the same managed Postgres `DatabaseURL` env value that the app child receives, so setup targets the session database or Neon branch. Successful setup is fingerprinted from `database.apply` config and seed file hashes; ordinary rebuilds skip setup until those inputs change.
...
- `dev.services.postgres` with `kind: "postgres"` creates or reuses a physical Postgres server substrate, verifies the recorded owner/reachability/version before reuse, and separately allocates a deterministic per-session database. The global Postgres substrate record contains physical-server metadata only: admin URL, version, isolation, data/socket directories, port, source, process owners, and exit metadata. It must not contain `session.<id>` database URLs or names. The session database lease is exposed through session/app env as `DatabaseURL`, `ONLAVA_MANAGED_DATABASE_URL`, and `ONLAVA_MANAGED_DATABASE_NAME`, even when local env contains legacy `DATABASE_URL`.
- `dev.services.postgres` with `kind: "neon"` starts or reuses the Onlava-managed local Neon dev cell under `~/.onlava/agent/substrates/neon/`, writes generated `cell.json`, `branches.json`, `restore-points.json`, and `compose.generated.yml`, and allocates a branch lease before DB apply/seed/setup. The first self-hosted cell is a Postgres-compatible local branch substrate; the upstream Neon Docker Compose files are source material only and are not exposed as user-maintained configuration. `onlava db neon status --json` reports backend, version, redacted admin URL, and per-component status/health/source/version/endpoint/port/log-path metadata. Parent branch reset/delete is protected. `onlava down` leaves the shared cell running; `onlava down --db` removes only the current branch lease/database; `onlava down --state` also removes the worktree branch pin.
- `dev.services.electric` supports explicit upstream routing with `ONLAVA_DEV_ELECTRIC_UPSTREAM`; when set, onlava registers the upstream as a hidden session backend and injects `ELECTRIC_URL`/`ONLAVA_ELECTRIC_URL` using the agent route. Without an explicit upstream, onlava starts a hidden per-session Electric process from `ONLAVA_DEV_ELECTRIC_BIN` or, when `dev.services.electric.image` is set and Docker is available, from that image. Electric uses the common managed process readiness and early-exit lifecycle, but remains session-scoped: it is registered as an agent session backend/process, not as a global Electric substrate row. Electric receives the managed Postgres session database URL when `dev.services.postgres` is declared. For Neon branches, the replication stream id includes the branch id so parallel worktrees do not collide.
...
onlava db neon install [--app-root <path>] [--json]
onlava db neon status [--json]
onlava db neon logs
onlava db neon restart [--app-root <path>] [--json]
onlava db neon uninstall --destroy-data [--json]
onlava db branch status [--app-root <path>] [--json]
onlava db branch list [--app-root <path>] [--json]
onlava db branch checkout <name> [--app-root <path>] [--json]
onlava db branch reset [--app-root <path>] --yes [--json]
onlava db branch restore [--app-root <path>] --at <restore-point|rfc3339> --yes [--json]
onlava db branch diff <branch> [--app-root <path>] [--json]
onlava db branch delete <name> [--app-root <path>] --force [--yes] [--json]
onlava db branch expire [--app-root <path>] --after <duration> [--json]
...
`onlava db psql` connects to the managed session database when `dev.services.postgres` is configured and an agent session is active; for `kind: "neon"` it uses the current worktree/session branch lease. Otherwise it uses explicit local database configuration. `onlava db reset`, `onlava db drop`, and `onlava db snapshot create|restore` are only available for managed session databases. `onlava db apply` runs only an explicit `database.apply` provider and does not run seed files or SQLC generation. Neon branch commands expose `onlava.db.branch.status.v1`, `onlava.db.branch.list.v1`, and `onlava.db.neon.status.v1` JSON without raw connection URLs; `restore --at` uses Onlava-managed restore-point refs or RFC3339 timestamps recorded for the current branch.
...
- `onlava down` stops and unregisters the selected session but is non-destructive by default. `--db` drops that session's managed Postgres database or current Neon branch lease/database, `--state` removes that session's `.onlava/sessions/<id>` state root and local Neon branch pin, and `--all` enables both. It does not stop the shared Neon dev cell.
...
- [onlava.db.branch.v1.schema.json](schemas/onlava.db.branch.v1.schema.json)
- [onlava.db.branch.status.v1.schema.json](schemas/onlava.db.branch.status.v1.schema.json)
- [onlava.db.branch.list.v1.schema.json](schemas/onlava.db.branch.list.v1.schema.json)
- [onlava.db.neon.status.v1.schema.json](schemas/onlava.db.neon.status.v1.schema.json)
