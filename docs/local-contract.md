...
- `scenery.sh/storage`, `.scenery.json` storage declarations, `scenery inspect storage --json`, and `scenery storage ... --json` while the storage runtime boundary and generated browser routes mature
...
- When storage is configured with managed ZeroFS, `scenery doctor --json` warns if `SCENERY_DEV_ZEROFS_BIN` is unset or points to a non-executable file, because `scenery up` will not search `PATH` for ZeroFS.
