...
- `dev.routing.domain` optionally gives the path-mode session a branded browser origin with the same path structure: `https://<sanitized-branch>-<domain>` per Git worktree (dash join, so every host stays on the domain's first subdomain level within single-level DNS wildcards and Cloudflare Universal SSL) and the bare `https://<domain>` on branch `main` (an empty branch label claims no host). The host is carried on the manifest as `route_manifest.domain_host`/`route_manifest.domain_url`, registered as an exact route host on the agent router (so on-demand TLS issuance follows the same registry lookup), and served by rebasing the session's path-mode manifest onto that origin. Domain hosts are single-owner: a second live session requesting a held host keeps its localhost URLs and reports `domain_host_conflict`; provably stale owners are reclaimed after fingerprint verification, and `scenery up --claim-aliases` force-transfers a live host. Browser reachability requires the managed edge (`scenery system edge install` + `trust` for loopback-only use; `scenery deploy setup` when the domain is reachable through a router port-forward) plus operator-owned DNS for the domain — loopback records, or Cloudflare-proxied records to a static IP with SSL mode "Full" so Cloudflare accepts the edge's internal origin certificate; managed dnsmasq is not required and not consulted. When the edge is not serving, `scenery up` still starts, keeps localhost URLs primary, and prints a one-line warning naming the fix commands. When the end-to-end HTTPS probe succeeds, run output advertises the domain URL as `App:`/`app_url`, and the local path router redirects non-Scenery-owned paths to it, taking precedence over the `deploy.domain` redirect. `dev.routing.domain` with `dev.routing.mode: "host"` is rejected.
- `dev.routing.expose` optionally narrows what the dev domain origin serves to the listed route names: `root`, `api`, `console`, `runtime`, and configured frontend names (carried on the manifest as `route_manifest.public_routes`, with `console` normalized to the dashboard route). Absent means the full path-mode surface — nothing is blocked by default. When present, unlisted route records disappear from domain-origin dispatch and the route index, and `/runtime*` plus `/__scenery*` answer 404 unless `runtime` is listed. The localhost listener always serves the full surface regardless of `expose`. Unknown entries fail `scenery up`, and `dev.routing.expose` without `dev.routing.domain` is rejected.
...
- Agent dev-runtime manifests include `route_manifest`, with `mode`, `base_url`, `worktree`, typed route records, an optional path-mode port lease, and optional path-mode `domain_host`/`domain_url` from `dev.routing.domain`.
- `frontends` is a map keyed by frontend name. `root` defaults to `apps/<name>`; `upstream` is optional but ignored by agent dev unless that frontend also sets `allow_shared_upstream: true`. `serve` selects the dev runtime serving mode: `development` (default) runs the package dev server with HMR; `production` runs the package `build` script (local vite/astro binaries first, then the detected package manager, with the vite `--base` flag preserving the path-mode prefix) and serves the package's `dist/` output from a Scenery-internal static server on a hidden loopback port — same backend and routing model, GET/HEAD only, SPA index fallback for extensionless paths, 503 until the first build completes, and `scenery up` fails when the build script is missing or the build produces no `dist/`. Production frontends take precedence over `allow_shared_upstream`, their source trees join the dev watcher (build output `dist/` excluded), and source changes rebuild the bundle in place without restarting the app; failed rebuilds keep serving the previous bundle and report the build error. With an active agent, `scenery up` prefers to start supported Vite/Astro frontends on hidden loopback ports, inject routed API/base-path URLs into their process environment, register those hidden ports as runtime backends, and expose `/<frontend>/` under the runtime base URL in path mode. Managed Vite/Astro frontends receive allowed-host controls for the current route host. If a managed frontend process exits while the dev supervisor is still running, Scenery restarts that frontend on a fresh hidden loopback port and re-registers the session backend/process metadata. Managed frontend routes serve the frontend shell for HTML SPA deep links, while `/runtime/*`, `/__scenery/*`, `/api/*`, and concrete asset paths are not history-fallback routes. `SCENERY_FRONTEND_<NAME>_ADDR` still overrides scenery-owned frontend startup for manual debugging. `serve: "production"` has two compatible implementations: the Scenery-internal static server above for local runtime verification, and direct managed-Caddy static serving for an enabled public deploy domain after `scenery deploy publish`.
...
- `react_component` declares only a symlink-safe workspace-relative `module` and named `export`. `table_page` requires `path`, a CRUD `source` with `list` and HTTP projections, `title`, and at least one labeled `column`; it also accepts `description`, `page_size` (default 50, bounded by the CRUD maximum), `row_link`, labeled `filter`/`sort` children, and singleton `toolbar`/`empty` override slots. Columns may use `appearance = "auto"|"text"|"number"|"datetime"|"badge"`. Filters and sorts must be in the CRUD allowlist. Expansion produces the ordinary page plus a built-in web renderer with provenance back to the declaration. `scenery schema scenery.table-page -o json` is the authored grammar; `scenery compile --view expanded -o json` exposes the derived resources.

- The table catalog styling surface is exactly these CSS variables, each with an embedded fallback: `--scenery-ui-background`, `--scenery-ui-foreground`, `--scenery-ui-muted`, `--scenery-ui-border`, `--scenery-ui-header-background`, `--scenery-ui-control-background`, `--scenery-ui-badge-background`, `--scenery-ui-error`, `--scenery-ui-focus`, `--scenery-ui-font`, `--scenery-ui-title-size`, `--scenery-ui-page-padding`, `--scenery-ui-cell-padding`, and `--scenery-ui-radius`. There is no layout override or supported catalog fork.
...
- `scenery deploy publish [-o json]` runs on the host that serves the deploy domain. For every frontend with `serve: "production"` it runs the package `build` script with the path-mode `--base /<name>/` prefix, copies the validated `dist/` output into an immutable release under `<agent home>/agent/deploy-artifacts/<app-id>/<frontend>/<release>/` (regular files only — symlinks and special files are rejected; a regular `index.html` entry document is required), atomically switches that frontend's `current` symlink, records the publication on the deploy target (`frontends` with name, `current` path, root ownership, release id), validates the regenerated Caddyfile with Caddy itself, reloads the managed edge, and probes the public entry document (must be 200 without `@vite/client`) and `/api` through the local edge listener with the real domain SNI. Validation, reload, or probe failure rolls the `current` pointers and registry back to the previous publication. Three past releases are retained; repeated publication is safe. The payload is `scenery.deploy.publish`.
- Published production frontends make the managed edge serve those routes directly from disk: Caddy rejects `/runtime`, `/dashboard`, `/console`, and `/__scenery`, proxies `/api/*` to the agent with the trusted public-edge headers, serves each published frontend under `/<name>/` (GET/HEAD only, concrete files first, SPA `index.html` fallback only for extensionless paths, gzip/zstd, immutable caching for `/assets/*`, revalidation elsewhere, ETag/range semantics from Caddy's file server), serves the `deploy.root` frontend at `/`, and keeps the agent proxy as catch-all otherwise. A registry target without publication metadata, or one whose `current` artifact does not resolve to a complete release, keeps the full agent-proxy behavior for those routes.
