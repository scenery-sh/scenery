...
- package and test hotspots are not reported from contended full-suite timing alone. Packages over their budget are rerun once in isolation; tests observed at or above 500ms are rerun three times and reported only when the isolated median remains over budget. The default package budget is two seconds, with an explicit ten-second baseline for `scenery.sh/cmd/scenery`.
