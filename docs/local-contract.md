...
- `budgets.integration_exceptions` is the complete sorted exception inventory. Each entry names one exact package plus top-level test, `class: "integration"`, a 100ms single-observation target, a 3s visibility budget, and a nonempty `classification_reason` for a real process, toolchain, service, or OS boundary. Package, prefix, regexp, and subtest exemptions do not exist; merely slow in-process tests remain fast. Excepted roots observed at or above 100ms are retained once in `observed_integration_tests`; a single observation at or above 3s emits an advisory warning, but integration roots never enter 20-run p95 confirmation or the fast release error. The exception changes no package, total-suite, test-binary-count, or cold-prepare budget.
