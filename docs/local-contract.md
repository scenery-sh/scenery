...
`scenery telemetry report [--since <duration>] [--agent-transcripts] [-o human|json]` reconstructs past runs on this host from what they left behind, over the last 30 days unless `--since` selects another window. It reads the CLI telemetry file; the event logs of detached `scenery up` supervisors under the agent home (`agent/dev/*.log` and `worktrees/*/control/dev/*.log`; foreground sessions keep none), from which it derives initial and rebuild timings, per-step p50/p95 of successful rebuilds, failure causes (the diagnostic code and first line with digests, addresses, paths and long numbers replaced and credentials redacted) and streaks of at least 5 consecutive failed builds per session; and, only with `--agent-transcripts`, the Claude Code transcripts under `~/.claude/projects` and Codex rollouts under `~/.codex/sessions` modified inside the window. Only agent sessions that ran a command `scenery help` advertises are counted; for them it reports tool calls and error kinds, and per Scenery command the calls, failures and waiting time, the failure classes (invalid invocation, failed precondition, capability unavailable, internal failure, application diagnostic, executable not found, timeout, other) and the inputs Scenery rejected as unknown flags or commands. It keeps no command text, output or file content. CLI records are summarized per command and app with failure bursts: consecutive hours in which one command failed with one exit code at least 60 times. Findings order the conclusions critical, warning, then info. The command reads files only and never records, sends or changes anything. JSON data uses kind `scenery.telemetry.report` and the exact checked schema in `docs/schemas/scenery.telemetry.report.schema.json`.
