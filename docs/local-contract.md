...
- cached and fresh self-harness Go test commands use `-p 8`, the package parallelism selected by repeated measurement on the maintainer machine.
- `.scenery/harness/test-timing-latest.json` identifies the timing lane and keeps the seven-second optimization target separate from operational budgets: cached default runs are advisory at 12 seconds, fresh runs are advisory at 18 seconds, and release runs enforce 30 seconds.
- package and test hotspots are not reported from contended full-suite timing alone. Packages over their budget are rerun once through one serial `-p 1` confirmation process; tests observed at or above 500ms are rerun three times and reported only when the isolated median remains over budget. Same-package test candidates share one serial `-parallel=1` confirmation process. The default package budget is two seconds, with an explicit ten-second baseline for `scenery.sh/cmd/scenery`.
