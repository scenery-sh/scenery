...
Each complete build-input capture first binds every input identity to one resolved
path, rejecting ambiguous path or module identities, then reads each unique
input once. This reuse is confined to that capture; a later capture reads current
bytes again, including same-size and same-mtime changes.
...
compute the generated-content inventory used by ordinary watcher repair/retry.
Explicitly embedded generated bytes remain consumed authored-snapshot inputs.
