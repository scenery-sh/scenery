...
- explicit fresh runs record `test_binaries`: `prepare_seconds` for all pre-execution work, `list_seconds` for package discovery, `build_seconds` for the summed per-binary link time, `built_count`, and a `builds` array of `{package, build_id, seconds}` ranked slowest first. Pre-execution work never appears in Go's per-package elapsed times, so this is the only attribution for a cold-run penalty.
