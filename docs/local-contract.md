...
- `dev.services` is a beta local-development config surface for scenery-owned substrates. It accepts `sqlite` and `zerofs` service declarations with `kind`, `database`, `database_url_env`, `image`, `route`, and string `env` values. The agent currently owns managed SQLite and storage planning for this surface, while unsupported service kinds or isolation modes are rejected instead of silently falling back to target-app port orchestration.
- `scenery up` prepares declared local DB setup before the app process starts. When app config declares `database.apply` or service-local seed files are discovered, the supervisor runs the same split lifecycle as `scenery db setup`: apply first, then seed. It passes the same managed SQLite database URL env value that the app child receives, so setup targets the dev-runtime database. Successful setup is fingerprinted from `database.apply` config and seed file hashes; ordinary rebuilds skip setup until those inputs change. Apps can set `database.seed.enabled: false` to opt out of seed discovery when local seed files target a database dialect or lifecycle outside Scenery-managed SQLite.
- `dev.setup` is an optional beta list of shell commands that `scenery up` runs from the app root after managed dev services and the DB setup lifecycle are prepared, but before the app process starts. Setup commands receive the same managed SQLite database URL env value as the app child, so target apps can keep existing app-local setup during migration.
...
- `scenery db branch delete <name> [--force]` drops the matching Scenery-owned branch database, removes its lease, and removes the current worktree pin only when the deleted branch is current and `--force` is present. `scenery db branch reset --yes` recopies the branch database from the parent template. `scenery db branch restore --at <timestamp-or-lsn> --yes` currently maps to template reset and emits `scenery.db.branch.restore.v1` with restore-point metadata. `scenery db branch diff <branch> [--json]` emits a deterministic schema-catalog diff when both branch databases exist and reports existence otherwise. `scenery db branch expire [<name>] --after <duration>` updates the local lease expiration, and `scenery db branch prune [--older-than <duration>]` removes expired non-current branch files.
- `scenery up`, `scenery db shell`, and DB setup rerun the configured branch-provider ensure boundary and wait briefly for a pending non-parent lease to become ready before synthesizing process-local database URL/path env values. They fail explicitly when no branch driver is configured, the pending lease does not become ready, or the lease is missing, expired, protected, or endpoint-less.
...
- Doctor reports local state size through informational `storage.scenery_home` and `storage.sqlite_database` checks. `storage.scenery_home` walks the resolved Scenery agent home (`~/.scenery` by default or `SCENERY_AGENT_HOME` when set). `storage.sqlite_database` walks the managed SQLite state root at `<scenery-home>/agent/sqlite`; if that path has not been installed yet, the check is `skipped` with the expected path.
...
- Managed Caddy resolves from the managed store or manifest-driven download. Managed Grafana and Victoria binaries resolve from explicit env overrides, the managed store, or manifest-driven download. They do not use implicit system `PATH` binaries.
...
- Structured dev logs carry source identity. Current source ids include `api`, `worker`, `build`, `supervisor`, `grafana`, `victoria`, and `frontend:<name>`.
...
- `scenery serve` builds once and starts the app runtime headlessly. It does not start the dashboard, managed frontends, agent routing, or file watcher.
- `scenery serve` starts the generated binary with `SCENERY_ROLE=api`, so it serves HTTP APIs without registering durable worker or cron handlers.
...
- Cron declarations run through the in-process scheduler. `scenery serve` reconciles schedules from the API role, while `scenery worker` executes cron jobs without starting the public HTTP server.
...
- Default Caddy, Grafana, Grafana plugin, Victoria sidecar, ZeroFS, and managed image versions are pinned in `scenery.toolchain.json`; environment variables override explicit startup controls for local testing where documented. Caddy edge and managed ZeroFS storage are managed-toolchain only.
...
- it runs docs knowledge validation, `scenery inspect docs --json`, architecture checks, UI static architecture checks, Go package tests, parallel dev-session safety, live SQLite branch lifecycle safety, dashboard UI typecheck/build, UI freshness checks, worktree-local `go build -o .scenery/harness/bin/scenery ./cmd/scenery`, and local binary freshness checks
...
- the default, race, and release self-harness modes create two SQLite branch databases under a temporary agent home, check branch data isolation with SQLite tooling, exercise reset, restore, diff, delete, and prune, and tear the temporary state down. `--quick` intentionally skips it.
...
Rules:
- Use `scenery inspect ... --json` for app, route, service, endpoint, wire, build, path, docs, generator, durable, and storage metadata. Use `scenery traces list --json` and `scenery metrics list --json` for local observability metadata.
