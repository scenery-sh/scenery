...
- Every Scenery build links with stock `go build`. `scenery up` links the process entrypoints whose linked identity changed and reuses the others; `build --development`, `build`, workers and binding CLI invocations link one application executable. No retained compiler, recorded tool recipe or direct compiler/linker execution exists. Retained compiler state an earlier Scenery kept for a workspace under the development cache is removed on that workspace's next process-model build; the Go build cache is untouched.
