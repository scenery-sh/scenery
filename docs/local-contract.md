...
`scenery down --db` drops the managed app database recorded in the active dev session metadata. It refuses external DSNs. `scenery down --state` removes only the app root's local runtime state.
