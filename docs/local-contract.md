...
- `scenery up` runs the process model by default: a process host plus one process per native service, each linked and replaced independently. `SCENERY_DEV_PROCESS_MODEL=service` selects it explicitly; `SCENERY_DEV_PROCESS_MODEL=application` selects the deprecated single application executable and reports a deprecation warning (a `warning` event under `-o jsonl`); any other value fails. An application the process model cannot run yet (one with event consumers or emissions, or without a native service) fails its build with a message that names the missing capability and the deprecated `application` selection; nothing falls back automatically.
