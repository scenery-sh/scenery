...
`scenery schema` and agent `schema.get` expose the same recursive authored definitions used for source validation and semantic creation: attribute versus block shape, labels, cardinality, ordering, value/reference type, expression phase, revision domain, defaults/constraints, sensitivity, and patchability. Agent capabilities return `resource_create_kinds`; only kinds whose full recursive metadata is complete appear there. `resource.create` renders attributes and nested blocks from those definitions, preserves ordered children, canonically sorts unordered children, resolves nested local module instances to their declared package source unit, and returns `capability_unavailable` for any unadvertised kind rather than emitting guessed source.

A Go service `config` block accepts dynamic lower-snake attributes, but every key must exactly match and reference the same named typed package input. The compiler derives the generated config field's type, contract/implementation/deployment phase, and sensitivity from that input, then validates the resolved module value and secret-reference flow before generation.
