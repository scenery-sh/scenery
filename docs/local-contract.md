...
- legacy async runtime workflow/activity and cron runtime/admin affordances until their lifecycle, retry, scheduling, and clear/delete semantics are frozen
- cron UI
- `scenery.sh/legacy-async-runtime` workflow/activity declarations and worker registration
- `scenery.sh/durable` typed task declarations, startup DB reconciliation, queued job starts, interval schedules, retrying local Go handler execution, durable step/signal helpers, authenticated durable worker lease/heartbeat/complete/fail HTTP endpoints, durable job admin, and `scenery inspect durable --json` while the SQLite durable execution runtime is implemented under ExecPlan 0089
...
```json
{
  ...
  "legacy-async-runtime": {
    ...
    "mode": "local",
    "namespace": "default",
    "address_env": "LEGACY_ASYNC_RUNTIME_ADDRESS",
    "task_queue_prefix": "scenery.myapp",
    "payload_codec": "scenery-json-v1",
    "api_key_env": "LEGACY_ASYNC_RUNTIME_API_KEY",
    "tls": {
      "enabled": false,
      "server_name_env": "LEGACY_ASYNC_RUNTIME_TLS_SERVER_NAME",
      "ca_cert_file_env": "LEGACY_ASYNC_RUNTIME_TLS_CA_CERT_FILE",
      "client_cert_file_env": "LEGACY_ASYNC_RUNTIME_TLS_CERT_FILE",
      "client_key_file_env": "LEGACY_ASYNC_RUNTIME_TLS_KEY_FILE"
    },
    "local": {
      "auto_start": false,
      "db_filename": ".scenery/legacy-async-runtime/dev.db"
    },
    "typescript": {
      "enabled": false,
      "runtime": "bun",
...
- `legacy-async-runtime` is optional and disabled by default. Scenery only starts or connects to legacy async runtime when `legacy-async-runtime.enabled` is explicitly `true`; workflow/activity declarations, TypeScript worker settings, and local `auto_start` settings do not enable legacy async runtime by themselves.
...
- Agent dev-runtime manifests include `route_namespace`, the app-derived local browser namespace used by routed URLs. `route_namespace.workspace` comes from `proxy.workspace` when present and otherwise falls back to app identity only when no explicit route hosts are configured. `route_namespace.base_domain` defaults to `local.dev` and may be overridden with `proxy.route_base_domain`. `route_namespace.hosts` preserves explicit configured route hosts such as `api`, `console`, `legacy-async-runtime`, `grafana`, and configured frontend names; those hosts become route aliases for that route rather than changing the generated base for every route.
- `proxy.frontends` is a map keyed by frontend name. Each frontend requires `host`; `root` defaults to `apps/<name>`; `upstream` is optional but ignored by agent dev unless that frontend also sets `allow_shared_upstream: true`. With an active agent, `scenery up` prefers to start supported Vite/Astro frontends on hidden loopback ports, inject routed API URLs and the internal route host into their process environment, register those hidden ports as runtime backends, and expose `https://<frontend>.<route-id>.<route_namespace.base_domain>:<agent-router-port>/` by default. Managed Vite/Astro frontends receive the route host through Vite/Astro allowed-host controls so app configs do not need to hard-code route hosts. If a managed frontend process exits while the dev supervisor is still running, Scenery restarts that frontend on a fresh hidden loopback port and re-registers the session backend/process metadata. Managed frontend routes serve the frontend shell for HTML SPA deep links, while `/__scenery/*`, `/api/*`, `/sync/*`, and concrete asset paths are not history-fallback routes. `SCENERY_FRONTEND_<NAME>_ADDR` still overrides scenery-owned frontend startup for manual debugging.
- `storage` is a Scenery-owned app capability config. App declarations still accept `kind: "zerofs"` stores only because ZeroFS is the managed local-dev substrate, but app code depends on `scenery.sh/storage`, not ZeroFS. `cell_id` is optional and defaults to a stable app identity; it must not include a worktree path, branch, session ID, or process ID. `share` defaults to `worktree`, meaning Scenery resolves shared storage-cell paths under the agent storage root for the same app/storage cell. `default` names the default store, and `stores.<name>` accepts `kind`, `access`, `tenant_scoped`, and `max_object_bytes`. Store `access` defaults to `auth`; `private` stores are available to app/runtime helpers but are not externally reachable through the reserved storage HTTP routes. Store names and cell IDs use identifier-safe strings. Unknown storage fields and unsupported storage kinds fail config validation.
- App processes, workers, setup commands, and app-local code tasks receive `SCENERY_STORAGE_CONFIG` and `SCENERY_STORAGE_CELL_ID` when storage is configured. `SCENERY_STORAGE_CONFIG` is consumed by `scenery.sh/storage` and contains store capability metadata, not raw object-store credentials. The production-supported headless contract is an explicit operator-provided runtime config whose stores use `kind: "proxy"` and `proxy_socket`; headless `scenery serve` and standalone `scenery worker` runtimes fail closed when storage is declared and the config is missing, empty, local-root-backed, or otherwise not proxy-backed. Agent-backed dev sessions use a session-local Scenery storage proxy socket in that config so app code does not receive direct object-root paths or raw ZeroFS socket paths; the proxy speaks the managed ZeroFS 9P Unix socket. Non-session CLI/task validation paths may still use Scenery-owned local storage-cell roots because they are not production app runtimes. The app runtime also uses this env to mount reserved storage object routes when storage is configured. When a managed ZeroFS Web UI route is attached to the current dev session, app processes also receive `SCENERY_ZEROFS_WEBUI_URL` as protected operator/debug metadata. `SCENERY_STORAGE_CELL_ROOT`, `SCENERY_STORAGE_ZEROFS_CONFIG`, and `SCENERY_ZEROFS_WEBUI_ADDR` are substrate metadata for managed ZeroFS process env/config expansion and are not app integration surfaces.
...
- Storage `ContentType` and user metadata are durable object metadata. File-backed local and ZeroFS-backed stores persist that metadata in Scenery-owned sidecars under `__scenery/metadata/`, hide sidecars from `List`, and remove sidecars on `Delete` and `DeletePrefix`. Reserved HTTP/proxy routes carry metadata through `X-Scenery-Storage-Meta-*` headers.
...
- `dev.services` is a beta local-development config surface for scenery-owned substrates. It accepts `postgres` and `zerofs` service declarations with `kind`, `mode`, `version`, `isolation`, `project`, `parent_branch`, `parent_database`, `branch_policy`, `branch_name_template`, `ttl`, `database`, `role`, `database_url_env`, `image`, `route`, and string `env` values. The agent currently owns managed Postgres and storage planning for this surface, while unsupported service kinds or isolation modes are rejected instead of silently falling back to target-app port orchestration.
- `scenery up` prepares declared local DB setup before the app process starts. When app config declares `database.apply` or service-local seed files are discovered, the supervisor runs the same split lifecycle as `scenery db setup`: apply first, then seed. It passes the same managed Postgres `DatabaseURL` env value that the app child receives, so setup targets the dev-runtime database. Successful setup is fingerprinted from `database.apply` config and seed file hashes; ordinary rebuilds skip setup until those inputs change.
...
- `dev.services.postgres` currently defaults to version `18` and `isolation: "database"`. Other isolation modes are rejected until implemented. With an active agent-backed dev runtime, scenery creates or reuses a physical Postgres server substrate, verifies the recorded owner/reachability/version before reuse, and separately allocates either a deterministic per-runtime database or, when `branch_policy`, `branch_name_template`, `parent_branch`, or `parent_database` is set, a deterministic branch database. The global Postgres substrate record contains physical-server metadata only: admin URL, version, isolation, data/socket directories, port, source, process owners, and exit metadata. It must not contain `session.<id>` database URLs or names. The runtime or branch database lease is exposed through app env as `DatabaseURL`, `SCENERY_MANAGED_DATABASE_URL`, and `SCENERY_MANAGED_DATABASE_NAME`, even when local env files already contain stale database URLs. Managed app, setup, DB setup, and worker environments do not receive Scenery-injected `DATABASE_URL`; `SCENERY_MANAGED_DATABASE_URL` remains available as tooling/debug metadata. The admin cluster comes from `SCENERY_DEV_POSTGRES_ADMIN_URL`, a reusable agent Postgres substrate, or Docker pulling the requested pinned `postgres` image from `scenery.toolchain.json`. Set `SCENERY_DEV_POSTGRES_EXTERNAL=1` to keep an explicit external `DatabaseURL` instead of using the managed runtime database; external mode requires `DatabaseURL` and ignores `DATABASE_URL` as an app database authority. Old substrate records with legacy `session.<id>` keys remain readable during adoption, but new writes omit those keys.
- `dev.services.postgres.kind: "postgres"` with explicit branch fields uses the managed Postgres branch provider. It supports `mode: "local"` and `isolation: "database"` through template database cloning. `scenery db postgres install|start|status|logs|stop|restart|uninstall --json` manages or inspects the shared local Postgres dev cell. Branch pins live at `<app-root>/.scenery/worktree-db.json` with `schema_version: "scenery.db.branch.v1"` and provider `postgres`. The global branch registry lives at `~/.scenery/agent/postgres/branches.json` or `SCENERY_AGENT_HOME/agent/postgres/branches.json` with `schema_version: "scenery.db.branch.registry.v2"`, provider `postgres`, and endpoint metadata only. Raw connection URLs are not stored in the pin, registry, restore-point state, or branch status/list JSON.
...
- `scenery up`, `scenery db psql`, and DB setup rerun the configured branch-provider ensure boundary and wait briefly for a pending non-parent lease to become ready before synthesizing a process-local `DatabaseURL`. They still synthesize that URL only from a ready lease endpoint, and fail explicitly when no branch driver is configured, the pending lease does not become ready, or the lease is missing, expired, protected, or endpoint-less.
- `dev.services.<name>.kind: "zerofs"` starts a managed ZeroFS process from the pinned `zerofs` binary artifact in `scenery.toolchain.json`, syncing it into the Scenery agent toolchain store when missing and downloads are enabled. Declaring this service makes managed ZeroFS required for `scenery up`: startup preflights the pinned managed toolchain artifact before preparing dev-session routes, and fails with a `scenery system toolchain sync --tool zerofs` repair action when the artifact is unavailable, corrupt, or blocked by `SCENERY_TOOLCHAIN_DOWNLOAD=0`. ZeroFS preflight failures write `scenery.dev.failure.v1` artifacts under `<app-root>/.scenery/evidence/` with `phase`, `session.status: "not_created"`, and substrate/toolchain context. Scenery writes a shared storage-cell TOML under a `0700` run directory in the agent storage root, writes the secret-bearing TOML as `0600`, serializes startup per storage cell, starts `zerofs run -c <config>`, then waits for a bounded two-minute readiness window for the short temp 9P/RPC Unix socket paths plus the private loopback Web UI to become ready. Session-backed toolchain, process-start, and readiness failures report a `scenery.dev.failure.v1` artifact path under the session state root's `artifacts/` directory; readiness failures also report the last probe error, process output tail, storage cell ID, source, PID, config path, log path, socket paths, and Web UI address. Readiness does not retry indefinitely until green. These failure artifacts must answer which phase failed, which session was active or not yet created, and which substrate component/socket/log/config was involved without including raw secrets. Once ready, Scenery registers the Web UI as the configured dev-session route and records a normalized `zerofs-<cell-id>` agent substrate with `cell-id`, `ninep-socket`, `rpc-socket`, and `webui-addr` endpoint metadata. Later dev supervisors for the same storage cell first probe and reuse that healthy substrate, register their own protected session route to it, and discard stale substrate records when probes fail. On shutdown, a ZeroFS-owning supervisor scans live agent sessions and detaches instead of interrupting the shared process when another live session still points the same storage route at the same Web UI backend. The generated TOML includes `[cache] dir`, `disk_size_gb`, and `memory_size_gb`, `[storage] url` plus a stable Scenery-owned local-dev `encryption_password`, `[servers.ninep]` with a private loopback `addresses` listener plus `unix_socket`, `[servers.rpc] unix_socket`, and `[servers.webui] addresses` bound only to private loopback. It must not include NFS, NBD, wildcard binds, or public 9P/RPC listeners by default. Agent-backed app storage operations go through the Scenery storage capability runtime boundary and session-local storage proxy, which speaks the managed ZeroFS 9P Unix socket without exposing that socket to app code. The pinned ZeroFS artifact is AGPL-licensed, so managed ZeroFS remains local-dev/proof-only unless `docs/zerofs-legal.md` records an owner-approved legal/compliance decision.
...
- `legacy-async-runtime.address_env` defaults to `LEGACY_ASYNC_RUNTIME_ADDRESS`; when that env var is unset, runtime defaults to `127.0.0.1:7233`.
- `legacy-async-runtime.namespace` defaults to `LEGACY_ASYNC_RUNTIME_NAMESPACE` when that env var is set, otherwise `default`.
- `legacy-async-runtime.task_queue_prefix` defaults to `scenery.<app-name>` with unsafe task-queue characters normalized to dots. `SCENERY_LEGACY_ASYNC_RUNTIME_TASK_QUEUE_PREFIX` overrides the effective runtime prefix; `scenery up` sets it to a runtime-scoped value when the local agent is active. Test-marked runtimes append `SCENERY_LEGACY_ASYNC_RUNTIME_TASK_QUEUE_TEST_SUFFIX` to the effective prefix so `scenery test` workers cannot poll or schedule onto a live dev session's legacy async runtime queues for the same checkout.
- `legacy-async-runtime.payload_codec` defaults to `scenery-json-v1` and is validated at runtime. This is the only supported payload profile for scenery-managed Go and external workers in this milestone.
- `legacy-async-runtime.api_key_env` defaults to `LEGACY_ASYNC_RUNTIME_API_KEY`. When set, the runtime uses legacy async runtime API-key credentials.
- `legacy-async-runtime.tls.enabled` enables TLS without requiring an API key. `legacy-async-runtime.tls.server_name_env`, `ca_cert_file_env`, `client_cert_file_env`, and `client_key_file_env` default to `LEGACY_ASYNC_RUNTIME_TLS_SERVER_NAME`, `LEGACY_ASYNC_RUNTIME_TLS_CA_CERT_FILE`, `LEGACY_ASYNC_RUNTIME_TLS_CERT_FILE`, and `LEGACY_ASYNC_RUNTIME_TLS_KEY_FILE`. Client certificate and key env vars must be set as a pair for mTLS.
- legacy async runtime worker deployment metadata is runtime-owned: `deployment_name` defaults to the task-queue prefix normalized for legacy async runtime Worker Deployment naming and can be overridden with `SCENERY_LEGACY_ASYNC_RUNTIME_DEPLOYMENT_NAME`; `worker_build_id` defaults to `dev` and can be set with `SCENERY_BUILD_ID`.
- legacy async runtime workers opt into Worker Deployment Versioning. `SCENERY_LEGACY_ASYNC_RUNTIME_VERSIONING_BEHAVIOR` accepts `pinned` or `auto_upgrade` and defaults to `pinned`.
- legacy async runtime workers enable Go SDK host resource reporting by default using legacy async runtime's `contrib/sysinfo` provider, so Worker heartbeats can include CPU and memory usage for legacy async runtime Cloud worker health views. Set `SCENERY_LEGACY_ASYNC_RUNTIME_HOST_RESOURCE_REPORTING=0` to disable this provider.
- Local scenery-managed worker processes set their `worker_build_id` as the current legacy async runtime Worker Deployment version on startup so schedules and new workflow executions have a versioned routing target. Non-local workers do not self-promote; operators must promote deployment versions explicitly.
- `legacy-async-runtime.local.auto_start` and `legacy-async-runtime.local.db_filename` are local development settings for supervised legacy async runtime dev server work and are only active when `legacy-async-runtime.enabled` is true. With an active agent, the legacy async runtime dev server is registered as a shared agent substrate and its local database state is stored under the agent directory; each dev runtime also registers a `legacy-async-runtime` route for the shared UI, while app workers receive runtime-scoped task queue prefixes. Explicit workflow/activity task queues are prefixed in active dev runtimes too, so parallel worktrees do not poll or schedule onto each other's queues. Reuse of an agent-recorded legacy async runtime substrate requires a verified owner fingerprint and a reachable legacy async runtime listener before app workers start; stale ready records are discarded and replaced. On dev startup, Scenery scans the current session task queues for open workflows that predate the session and emits one aggregated warning event with count, oldest start time, sample workflow ID, and task queues. `scenery worker legacy-async-runtime prune --stale` reports stale workflow candidates for the current app root's active session; it terminates them only with explicit `--yes` and emits `scenery.legacy-async-runtime.prune.v1` with `--json`. legacy async runtime stdout and stderr are always written to stable substrate log files and the agent registry records exit metadata when the managed process exits.
- Supervised API and TypeScript legacy async runtime worker log pass-through collapses repeated identical legacy async runtime activity failure lines after the third repeat, keyed by workflow type and error text, so stale workflow retries do not flood the dev console or dev-event stream.
- `SCENERY_LEGACY_ASYNC_RUNTIME_TASK_QUEUE` overrides the generated legacy async runtime task queue for worker processes. `scenery worker --task-queue <name>` and `scenery worker typescript --task-queue <name>` set it.
- TypeScript legacy async runtime activity support is activity-only. scenery discovers `*.worker.ts` files, plus ordinary `.ts` files with `//scenery:worker`, and generates `.scenery/generated/legacy-async-runtime/typescript/{scenery.ts,registry.ts,worker.ts,manifest.json,tsconfig.json,package.json}`. Source files import `activity` from `scenery/worker` or `@scenery/legacy-async-runtime`; the generated `tsconfig.json` maps both names to the generated local API. Before launching the generated worker, `scenery up` and `scenery worker typescript` install the app-root `package.json` dependencies and the generated worker package dependencies with `bun install`, falling back to `npm install` when Bun is unavailable.
- Go workflows declare TypeScript activities with `legacy-async-runtime.NewExternalActivity` using matching input/output type parameters and call them through `legacy-async-runtime.ExecuteActivity`. `scenery check --json` validates matching TypeScript activity names, task queues, and type names before build/runtime.
- `legacy-async-runtime.typescript.enabled`, `runtime`, and `auto_start` configure the TypeScript worker path. `scenery worker typescript` generates and runs the hidden worker directly. When `legacy-async-runtime.enabled`, `legacy-async-runtime.typescript.enabled`, and `auto_start` are all true, `scenery up` validates Go-to-TypeScript contracts, regenerates the hidden worker runtime, and supervises the TypeScript worker alongside the Go app. The worker receives the supervised legacy async runtime address/namespace, runtime-scoped task queue prefix, deployment name, build ID, and internal agent identity environment. `runtime` accepts `bun` or `node`; when empty, scenery prefers `bun` and falls back to `node --import tsx`.
...
- Packages that declare `scenery.sh/legacy-async-runtime` workflows or activities with `legacy-async-runtime.NewWorkflow`, `legacy-async-runtime.NewActivity`, or `legacy-async-runtime.NewExternalActivity` are imported into the generated main so their declarations register at startup.
- Packages that declare `scenery.sh/durable` tasks with `durable.NewTask` are imported into the generated main so their declarations register at startup; `durable.TaskConfig.Service` is required for static literal configs. Runtime startup reconciles declarations into `<app-root>/.scenery/state/db/<service>.durable.sqlite`; `durable.Start` writes queued JSON-input jobs into the owning service durable DB. Generated `all` and `worker` roles run a local durable worker loop for registered Go handlers; failed attempts requeue until `MaxAttempts` using `Retry.InitialInterval`, `Retry.MaxInterval`, and `Retry.BackoffFactor`; `api` does not execute durable jobs. `durable.Schedule` records an interval schedule that API/all roles materialize into queued jobs. `durable.Step` persists local handler step results by job/key and reuses succeeded results, while `durable.Signal` appends a JSON signal row and event for a run. The API role exposes `/__scenery/durable/v1/<service>/lease`, `/__scenery/durable/v1/<service>/jobs/<job>/heartbeat`, `/complete`, and `/fail` as JSON POST endpoints for remote workers. Those endpoints require bearer tokens stored only as hashes in the service durable DB and fence heartbeat/complete/fail with `worker_id` plus `lease_id`. `scenery inspect durable --json` emits `scenery.inspect.durable.v1` with durable task declarations, service names, durable DB paths, and current DB existence.
- `legacy-async-runtime.ActivityConfig.MaxConcurrency` maps to the legacy async runtime worker's per-task-queue maximum concurrent activity executions. Use a dedicated task queue when different activities need different limits. `legacy-async-runtime.WithHeartbeatTimeout(duration)` sets the workflow activity heartbeat timeout without changing the stable `ActivityConfig` struct shape.
- Cron jobs can set `cron.JobConfig.OverlapPolicy`, `CatchupWindow`, `PauseOnFailure`, `ActivityStartToClose`, and `ActivityRetryPolicy`. When legacy async runtime is enabled these map to legacy async runtime Schedule overlap/catchup/pause policy and to the generated cron activity options. Defaults are overlap `skip`, catchup window `1m`, pause-on-failure `false`, and activity start-to-close `1h`.
- Optional multi-language worker manifests live under `.scenery/workers/*.json` and use `scenery.worker.manifest.v1` or `scenery.worker.manifest.v2`. They require `build_id` and `payload_codec: "scenery-json-v1"`. v2 manifests use queue-level registrations with `registration_hash` values so `scenery inspect legacy-async-runtime --json` can reject incompatible workers sharing a legacy async runtime task queue.
...
scenery inspect app|routes|services|endpoints|models|views|wire|build|paths|generators|legacy-async-runtime|durable|storage|observability|validation --json [--app-root <path>]
...
- When storage is configured with managed ZeroFS, `scenery doctor --json` reports the pinned `zerofs` managed toolchain artifact and warns when the artifact is missing while `SCENERY_TOOLCHAIN_DOWNLOAD=0` prevents on-demand sync.
...
- Managed Caddy resolves from the managed store or manifest-driven download. Managed Grafana, Victoria, and legacy async runtime CLI binaries resolve from explicit env overrides, the managed store, or manifest-driven download. They do not use implicit system `PATH` binaries.
...
- Structured dev logs carry source identity. Current source ids include `api`, `worker:typescript`, `build`, `supervisor`, `legacy-async-runtime`, `grafana`, `victoria`, and `frontend:<name>`.
...
- `scenery down` stops and unregisters the selected app root's live dev runtime but is non-destructive by default. It releases only the selected session's managed ZeroFS storage-cell leases and preserves shared storage-cell data. `--db` drops that runtime's managed Postgres database, `--state` removes that runtime's internal `.scenery/sessions/<id>` state root, and `--all` enables both; `--state` still does not delete shared storage-cell data. `--json` reports `scenery.down.v1`, still includes `session_id` for state compatibility, and includes `storage_leases_removed` plus `storage_cells_preserved` when a storage lease was released.
...
- Cron declarations use legacy async runtime Schedules when legacy async runtime is enabled. `scenery serve` reconciles schedules from the API role, while `scenery worker` runs the cron workflow/activity worker on `scenery.<app>.cron.go`. legacy async runtime cron executions derive their scenery request start/idempotency metadata from the workflow scheduled start time.
- `scenery worker` builds once and starts the app runtime in worker-only mode with no public HTTP server. In this beta implementation it runs cron, native legacy async runtime workers, and local durable workers; generated binaries use `SCENERY_ROLE=worker`.
...
- `scenery worker durable jobs list|inspect|cancel|retry ... --json` reads or mutates jobs in one service durable DB and emits `scenery.durable.jobs.v1`; `inspect` includes job events.
- `scenery worker durable token create --service <name> --json` creates or rotates a remote durable worker bearer token for one service durable DB, stores only the token hash in `<service>.durable.sqlite`, and prints the raw secret once in `scenery.durable.worker_token.create.v1`.
...
- `scenery worker deployment set-current`, `ramp`, and `drain` are the explicit operator commands for legacy async runtime Worker Deployment routing changes in non-local environments. They use the app's legacy async runtime connection settings, including TLS/API-key env vars.
...
- Grafana binds to loopback and stores generated config, provisioning, and plugin state under `.scenery/grafana/` when running without the agent; downloaded Grafana binaries live under `.scenery/toolchain/` or `SCENERY_TOOLCHAIN_DIR`. With an active agent, Grafana follows the same shared-substrate model as Victoria: state lives under the agent directory, is scoped to that agent state root rather than all users or all agent homes, and later dev runtimes reuse the verified shared Grafana before exposing a runtime route such as `grafana.<route-id>.<route_namespace.base_domain>` that points at the shared upstream.
...
- Default Caddy, Grafana, Grafana plugin, Victoria sidecar, legacy async runtime CLI, ZeroFS, and managed image versions are pinned in `scenery.toolchain.json`; environment variables override explicit startup controls for local testing where documented. Caddy edge and managed ZeroFS storage are managed-toolchain only.
...
- the default self-harness storage probe exercises configured storage through an app task, storage CLI import/export, and a live managed ZeroFS app route when the pinned toolchain artifact is available. The live ZeroFS proof writes an object, interrupts the managed ZeroFS process, restarts the dev runtime, and reads the same object back through the app route.
...
Rules:
- Use `scenery inspect ... --json` for app, route, service, endpoint, wire, build, path, docs, generator, and legacy async runtime metadata. Use `scenery traces list --json` and `scenery metrics list --json` for local observability metadata.
...
- [scenery.inspect.durable.v1.schema.json](schemas/scenery.inspect.durable.v1.schema.json)
