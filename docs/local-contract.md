...
- In a process-model development session each service entrypoint may be linked by Scenery's retained compiler instead of stock `go build`. A session records an entrypoint's recipe only after it has linked that entrypoint by stock Go more than once, so the first build of an application with many services records nothing and one edit never rebuilds every closure. Recording runs in the background at a lowered scheduling priority, one recording on the whole machine at a time, with Go tool parallelism bounded to a quarter of the cores. It starts only while no foreground link of any worktree is queued or running, and a foreground link that arrives while it runs makes it yield: its whole process tree is stopped and it starts again once the machine is idle, so foreground links never wait for a recording but a yielded recording repeats its work. It reports a `build.recipe_capture` build step whose `reason` is `captured_<entrypoint>` or `failed_<entrypoint>: <reason>`, and never blocks or fails the build that scheduled it. The recipes of one workspace share a single content-addressed store of retained archives, support inputs and source snapshots, because entrypoints of one application compile nearly the same packages. An entrypoint whose recipe is absent or no longer describes its inputs is linked by stock Go, which records `build.backend` with `retained_process_<reason>`, and a rejected recipe is replaced by the next recording rather than kept because its pointer exists. A recording names the input revision it compiles before any tool runs and is discarded when that revision changes before the tools finish, when a recorded tool read other content than the revision names, or when a tool read a file from a selected package directory that the revision does not name, such as a source file added and removed during the recording. Recording belongs to the session: it ends when the session closes, which waits until the Go command and every tool it started have stopped. The shared store is collected on the first build of a workspace and again after every 16 recipe publications, never while a recording may still be adopting objects into it.
- In a process-model development session (`SCENERY_DEV_PROCESS_MODEL=service`), `devdash.AppStatus` exposes `serviceProcesses`: one entry per service process with its name, PID, published generation, implementation revision and `running` or `degraded` state, and the agent session record names each live service process as `service:<name>` beside `api` and the helper processes. A service is `degraded` when its process stopped and exhausted its restart budget, or while its process serves but has not confirmed the activation of its background work; the second case reports a `reason` beginning with `background work activation unconfirmed`, and the supervisor repeats the activation until the process confirms it or stops being current. Control requests to one service process are serialized per process and never hold the process model while waiting, so an unresponsive service delays only its own control; once its background work is drained it is never activated again. Status and session registration read the last committed process-model state and never wait for an activation in progress. Both are absent for a single application process.
