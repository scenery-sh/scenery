...
- Ordinary development compilation uses Scenery's retained Go input domain and direct compiler/linker executor. Stock `go build` is neither the development default nor a product fallback. The first build uses `cmd/go` to capture its complete selected action graph. Compatible later body edits reuse the retained package graph for `BuildInput`, rebuild changed packages and transitive consumers with captured tools, and atomically advance current source/archive state without repeated package loading. Directory-membership, import, directive, module, configuration or tool changes use `cmd/go` to reconcile the action graph; incomplete observation requires a new complete capture. Direct execution rebinds all captured regular-file inputs, validates refresh stamps before commit, and rejects frontiers requiring unmodeled cgo, assembly or other native actions before tool execution. The supervisor still owns cancellation, candidate preflight, activation and rollback. Ephemeral, production-asset, and deployable artifact builds retain their existing stock path. This internal policy has no app configuration or environment selector. Stock development execution exists only in repository benchmark binaries built with the private `scenery_benchmark_stock` tag.
...
- Development `build.step` events may report `files_hashed`, `bytes_hashed`, `files_reused`, and `bytes_reused` for retained state work. `candidate.prepare.written_paths` contains the one retained executable selected for preflight when candidate preparation succeeds. These are bounded evidence fields; an omitted zero-valued counter is zero, not an unknown operation.
...
- Repository-only `--benchmark native-build-compiler --workload-root <path>` compares the permanently named `GO-BUILD ONLY`, `FULL-PREP + GO-BUILD`, `FULL-PREP + GO-TOOLS`, `RETAINED-PREP + GO-BUILD`, and `RETAINED-PREP + GO-TOOLS` matrix over identical full-ONLV edits. Stock lanes are isolated controls compiled only into the benchmark producer; they are not product fallbacks. Comparisons use `RETAINED-PREP + GO-BUILD` as the equal-input executor baseline and report preparation-versus-bare deltas separately. Every sample carries absolute phase starts, backend, owner, generation and runtime identity, capture subphases, state-commit accounting, candidate verification, concurrent `implementation.check`, the request's `scheduling_policy` and enclosing transaction duration. The report counts `scheduling_policies` across samples and records host `load_average` before (`environment`) and after (`load_average_after`) the run. The final executable and result are retained per measured lane. Optional `--benchmark-short` is valid only for the native build driver/compiler benchmarks and reduces the series to one cohort, one warmup and three rounds; it is an observation and never a GO/NO-GO gate.
