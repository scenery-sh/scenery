...
- `dev.services` is a beta local-development config surface for scenery-owned substrates. It accepts `sqlite` and `postgres` service declarations. SQLite services use per-worktree files under `.scenery/sqlite/` and may use the existing branch provider. Postgres services are opt-in and use fixed precedence: if the service `database_url_env` is present in the app/setup environment, Scenery treats that `postgres://` or `postgresql://` URL as external and manages no server; otherwise `scenery up` ensures one machine-wide Docker-backed shared Postgres server and creates one database per app root/worktree/service. Managed Postgres database names are derived from app ID, service name, and a short hash of the absolute app root. Postgres services accept only `kind`, `database`, `database_url_env`, and `env`; old provider/isolation fields are rejected. Storage no longer needs a `dev.services` entry: declaring `storage.stores` is sufficient and `scenery up` serves those stores from the local backend.
- App processes, setup commands, DB setup, and workers receive the configured service URL env for every managed database service. `SCENERY_SQLITE_DATABASES_JSON` describes SQLite services, and `SCENERY_POSTGRES_DATABASES_JSON` describes Postgres services including service, database, URL, configured env name, and source (`managed` or `external`). The `DatabaseURL` alias is injected only when exactly one database service of any engine is configured and that service did not explicitly set `database_url_env`.
...
`scenery db list --json` reports configured SQLite and Postgres services as `scenery.db.list.v2`; each entry includes `engine`, `service`, `database_url_env`, and engine-specific target fields. `scenery db path` prints the selected SQLite database file path and errors for Postgres services because they have no file path. `scenery db shell` opens SQLite services with `sqlite3` and Postgres services with `psql <dsn>` when the host tools are available. SQLite branch-aware commands still read `.scenery/worktree-db.json` directly and connect only when the Scenery-owned local lease is ready; pending, missing, expired, protected, or endpoint-less leases fail explicitly. Postgres branch commands fail because worktree isolation is automatic through per-worktree database names. `scenery db snapshot create|restore` stores SQLite backups and Postgres custom dumps under `.scenery/db/snapshots/<name>/`; Postgres snapshots use `pg_dump -Fc` and `pg_restore --clean --if-exists`, and restore requires `--yes` when any Postgres service is present. `scenery db reset` and `scenery db drop` reset or remove managed SQLite files and managed Postgres databases; they refuse external-DSN Postgres services and require `--yes` when a destructive Postgres operation targets multiple services. `scenery db server status|start|stop|logs` manages only the shared local Postgres server, reports `scenery.db.server.status.v1` for status, and refuses to stop while leases exist unless `--yes` is supplied. `scenery db apply` runs only `database.apply.command` and does not run seed files or SQLC generation.
...
- `scenery db seed` applies service-local initial data such as `SERVICE/db/seed.sql` and generated model seed data at `.scenery/gen/db/<service>/seed.sql` to that service's configured database. If the seed service does not match a configured database service, Scenery falls back only when there is one database service or a conventional `db` service; mixed-service apps must declare matching service databases. It runs after schema exists and does not participate in Atlas or SQLC generation. SQLite ledgers use `scenery_internal_seed_runs`; Postgres ledgers use `scenery_internal.seed_runs`, both keyed by app ID and seed path. Unchanged seeds are skipped; changed previously-applied seeds fail closed with status `changed`. Seed validation also fails closed before opening the database when SQL contains destructive setup patterns such as `DROP`, `TRUNCATE`, `DELETE FROM ...` without `WHERE`, `WHERE true`, or `WHERE 1 = 1`; diagnostics include the seed path, line, message, and statement context.
...
- `scenery down` stops and unregisters the selected app root's live dev runtime but is non-destructive by default. It preserves shared storage-cell data. `--db` drops that runtime's managed SQLite database, `--state` removes that runtime's internal `.scenery/sessions/<id>` state root, and `--all` enables both; `--state` still does not delete shared storage-cell data. `--json` reports `scenery.down.v1` and still includes `session_id` for state compatibility. To delete storage-cell data, use `scenery storage cleanup --yes`.
...
- `scenery serve` and `scenery worker` never start the managed shared Postgres server. If an app declares a Postgres service, each service's configured database URL env must be set to a valid Postgres URL before startup; otherwise startup fails closed and points back to `scenery up` as the dev-substrate path.
...
- [scenery.db.list.v2.schema.json](schemas/scenery.db.list.v2.schema.json)
