...
- `split_page` is domain-neutral two-pane composition. It requires `path`, `title`, a call-delivery HTTP `source` whose operation has unit input and one result, and `pane` plus `detail` `react_component` slots. Optional `pane_actions` and `detail_header` slots receive the same typed `{data, state, selection}` props; `query_parameter` controls URL-backed selection. The source operation must also have an inherited internal binding for page loading. Expansion produces the ordinary page plus the built-in split renderer. Scenery owns only layout, loading, error, and selection wiring; all domain rendering remains in the app's declared slot modules. `scenery schema scenery.split-page -o json` exposes the authored grammar.
