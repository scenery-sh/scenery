...
- Starting `onlava dev` for an existing app-root/session claims that exact session and supersedes older owners for the same app root and session id. The new owner cleans recorded app, worker, and managed frontend child processes from the previous owner, plus Onlava-owned session processes whose injected app root/session environment matches. It must not clean other sessions, other app roots, or unrelated user processes.
- `onlava status --json` treats a `starting` or `running` session with a missing or mismatched owner fingerprint as stale instead of reporting it as healthy.
