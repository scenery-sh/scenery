...
- Ordinary development compilation uses Scenery's retained Go input domain by default. Its first build records a complete stock `go build -a` compiler/linker recipe; compatible later body edits rebuild changed packages and transitive consumers with the same stock tools. Package/file membership, imports, build or embed directives, module inputs, build flags, relevant environment, tool identity, native inputs, or retained-artifact drift forces a fresh captured stock bootstrap before publication. The supervisor still owns cancellation, candidate preflight, activation and rollback. Ephemeral, production-assets, and deployable artifact builds use ordinary stock `go build`. This internal policy has no app configuration or environment selector.
