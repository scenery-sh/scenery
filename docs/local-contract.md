...
- Each development build request records `watch.scan` for the scan that captured its source snapshot: `actions` counts the watched files, `cache_hits` the directory listings reused from an earlier scan, `cache_misses` the directories read, and `files_hashed`/`bytes_hashed` the files whose content was read because their metadata changed. A directory listing is reused only while the directory's identity, modification time and size are unchanged and the directory had not been modified for two seconds before it was read; every watched file is still checked by its own metadata, so a body edit reads no directory and hashes only the edited file. Build requests also record `process.plan` (runtime integration plan), `workspace.verify` (`before_compile` and `after_compile` membership and byte checks of the private workspace) and `supervisor.publish` (status, dashboard and session publication after activation).
