...
- `dev.services` is a beta local-development config surface for scenery-owned substrates. It accepts `sqlite` service declarations with `kind`, `database`, `database_url_env`, `image`, `route`, and string `env` values. The agent owns managed SQLite planning for this surface, while unsupported service kinds or isolation modes are rejected instead of silently falling back to target-app port orchestration. Storage no longer needs a `dev.services` entry: declaring `storage.stores` is sufficient and `scenery up` serves those stores from the local backend.
