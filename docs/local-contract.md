...
- Typed and raw HTTP responses that start a request trace include `X-Trace-Id` and list it on `Access-Control-Expose-Headers` so browser clients can read it. The dashboard API explorer copies that header into `trace_id`.
...
- only the explicit `--fresh-tests` lane performs isolated timing confirmation. Packages over their budget are rerun once through one serial `-p 1` confirmation process; tests observed at or above 500ms are rerun three times and reported only when the isolated median remains over budget. Same-package test candidates share one serial `-parallel=1` confirmation process. The default package budget is ten seconds, with an explicit fifteen-second baseline for `scenery.sh/cmd/scenery`.
- `budgets.confirmation_scope` selects which candidates are confirmed. `--fresh-tests` alone uses `regressions`: a candidate is re-run only when the previous `.scenery/harness/test-timing-latest.json` did not record it over budget, or when it is now at least 25% and 0.5s worse than that recorded level. `--release --fresh-tests` uses `all` and confirms every candidate; it is the periodic audit lane. Every skipped candidate is recorded in `deferred_confirmations` with its baseline, and a single informational diagnostic reports the skip count. A missing or unreadable baseline confirms everything.
...
- explicit fresh runs record `test_binaries`: `prepare_seconds` for all pre-execution work, `list_seconds` for package discovery, `build_seconds` for the summed per-binary link time, `built_count`, `test_package_count` for packages that produce a test binary, and a `builds` array of `{package, build_id, seconds}` ranked slowest first. Pre-execution work never appears in Go's per-package elapsed times, so this is the only attribution for a cold-run penalty.
- `budgets.test_binary_count` is the maximum number of packages that produce a test binary. Fresh runs compare it to `test_binaries.test_package_count` on every run, including warm ones, because adding a package is a cold-link cost even when this run reused cached binaries. `budgets.cold_build_seconds` is the maximum aggregate per-binary link CPU and applies only when `built_count` equals `test_package_count` (a full cold prepare). Both are advisory in default/fresh and errors in `--release`. The current line is 60 binaries and 180 seconds of link CPU.
