...
- `dev.services` is a beta local-development config surface for scenery-owned substrates. Phase 5 accepts `postgres`, `electric`, and `zerofs` service declarations with `kind`, `mode`, `version`, `isolation`, `project`, `parent_branch`, `parent_database`, `branch_policy`, `branch_name_template`, `ttl`, `database`, `role`, `database_url_env`, `image`, `route`, and string `env` values. The agent currently owns managed Postgres, Electric, and storage planning for this surface, while unsupported service kinds or isolation modes are rejected instead of silently falling back to target-app port orchestration.
