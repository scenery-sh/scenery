...
- Ordinary development compilation uses Scenery's retained Go input domain by default. Its first build records a complete stock `go build -a` compiler/linker recipe; compatible later body edits rebuild changed packages and transitive consumers with the same stock tools and atomically advance the current retained source/archive state. Package/file/import/directive/module changes first use an ordinary shared-cache stock build to refresh the graph and preserve compatible actions. A refresh that does not observe every required action, or a build-configuration, tool-identity or retained-state incompatibility, performs a complete bootstrap. Direct execution rebinds all captured regular-file inputs and rejects frontiers requiring unmodeled cgo, assembly or other native actions before tool execution. The supervisor still owns cancellation, candidate preflight, activation and rollback. Ephemeral, production-assets, and deployable artifact builds use ordinary stock `go build`. This internal policy has no app configuration or environment selector.
...
- Repository-only `--benchmark native-build-compiler --workload-root <path>` compares complete-capture stock execution, retained-validation stock execution, and retained-validation direct compiler/linker execution over identical full-ONLV edits. Every sample carries backend, owner, generation and runtime identity plus an enclosing transaction duration. Optional `--benchmark-short` is valid only for the native build driver/compiler benchmarks and reduces the series to one cohort, one warmup and three rounds; it is an observation and never a GO/NO-GO gate.
