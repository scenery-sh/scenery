...
This document defines deterministic semantic compatibility comparison for Scenery resource graphs. It is used by `scenery diff --semantic`, package publication, generated-client version recommendations, migration shadow comparison, change plans, deployment gates, and agents.

Compatibility is not one Boolean. A change can be compatible for callers while breaking response consumers, stronger for security while requiring caller migration, and unrelated to storage. This profile reports those dimensions separately.
...
- `unknown`: the profile cannot prove one of the other results.
...
The canonical result kind is `scenery.semantic-diff/v1` and contains ordered `changes`. Each change includes at least:
...
~~~json
{
  ...
  "expected_kind": "scenery.binding/v1",
...
Rules have stable codes. Results also include base/target revisions, comparison profile and catalog digests, graph view, scope, summary, required migrations, generated consequences, and risk records.
...
Pattern changes use the edition-pinned regex dialect. A tool may classify subset/superset relations only when the catalog contains a normative proof algorithm; otherwise the result is unknown.
...
- Adding a result or declared error is response- and generated-client-breaking for closed exhaustive consumers; it is compatible only under a profile that normatively preserves unknown outcomes.
...
- Changing gateway, method, effective path, codec profile, content type, mapping, status, header, or cookie contract is breaking unless an exact rule below proves equivalence.
...
- Additive nullable columns or independently initialized fields may be compatible when the provider profile proves it.
...
Without a claimed data/provider compatibility profile, storage changes are unknown.
...
Package semantic-version recommendation is:

- `major` if any public source, request, response, generated-client, or security dimension is breaking or unknown;
- `minor` for compatible public additions;
- `patch` when public projections are unchanged;
- `migration` metadata in addition to major/minor when any runtime/storage/deployment dimension is migration_required.

The recommendation never overrides package policy or prove runtime rollout safety.
...
- compatibility profile/catalog digest;
- comparison digest;
...
Core rule catalogs are immutable and content-addressed. Provider and future profile rules may extend only their declared resource schemas and dimensions. Extensions cannot redefine a core rule or convert unknown to compatible without a claimed normative profile and evidence schema.
...
Version 1 does not claim arbitrary program equivalence, regex-language inclusion beyond catalogued cases, SQL query equivalence, opaque provider compatibility, behavioral equivalence of Go handlers, or safe stateful rollback without operational evidence.
