...
Run `scenery inspect docs --for-path <changed-spec-path> -o json` after changing this directory, then run the conformance tests for every affected boundary and `go run ./scripts/verify --summary --write` for substantial changes.
