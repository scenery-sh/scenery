...
This document defines how the current Scenery contract is implemented, generated, type-checked, built, registered, and verified in Go.
...
ordinary generated operation input and do not change the handler
ABI or expose transport objects.

A conforming tool MUST implement the exact topology, mappings, signatures, staged verification, descriptor, and registration rules below.
...
Two resolved Scenery packages MUST NOT own the same implementation import path or derived `/scenerycontract` import path. A Scenery package needing independent Go boundary packages must be split into multiple Scenery packages.
...
`package_contract_abi_revision` fingerprints generated Go ABI shape independently of application contract and exact artifact bytes.
...
Package identity is artifact provenance. Exact published bytes are the artifact digest. Neither belongs in the ABI-shape hash.
...
~~~text
scenery.package-contract-abi-revision\0
~~~
...
An input that would change the projection is illegal under this contract.
...
Implementation-check and build verification perform the following stages.
Graph-valid Go package generation is a bootstrap operation and does not require
application implementation success; it validates the package ABI mapping and
publishes the current package projection before ordinary Go tooling is usable.
...
10. For `build`, publish the applicable verified private artifact set.

A stale on-disk generated package MUST NOT satisfy a verification stage. Public
package bootstrap requires contract validation; private executable composition
requires the applicable implementation validation.
...
For path-tail bindings, generated registration
additionally includes each route's canonical terminal template, tail mapping,
target type, empty-capture behavior, schema revision, and deterministic
precedence data. Registration MUST reject route conflicts or metadata
...
`scenery.package-generated` contains:

- artifact kind and content digest;
- package identity as provenance;
...
- application `contract_revision`;
- producer version as artifact provenance;
...
The Go ABI is semantically versioned independently of the Scenery specification.
...
A package Go-surface change creates a new package ABI revision when the mapping rules change. A package artifact change with identical reachable Go ABI shape retains the same package ABI revision.
...
Unknown files beneath generated roots are not adopted implicitly and retain
ordinary authored revision membership. Replacement/retirement requires exact
descriptor/digest ownership; hand edits or missing/corrupt evidence MUST fail
closed. Publication serializes through the workspace transaction boundary,
recovers interruption before Scenery reads, and preserves unchanged bytes and
mtimes. It does not promise a globally atomic snapshot to concurrent raw Go tools.

## 20. Clean clone and ordinary Go workflow

After a source-only checkout, `scenery generate --target contracts` MUST publish
the app-imported Go packages before ordinary `go doc`, `go mod tidy`, or
`go test ./...` is expected to resolve them. Go generation MUST NOT create
nested modules, root workfiles, generated import replacements, or Git ignores.
User workspaces remain byte-identical. Compilation and graph inspection do not
...
context and `GOWORK=off`; ordinary user Go commands require no such override.

Default `generate` publishes public Go plus selected TypeScript outputs in one
artifact operation. Targeted TypeScript remains targeted; `--target contracts`
does not invoke TypeScript, live infrastructure or implementation verification.
The retired materialize/prune/editor-merge switches are invalid requests, not
alternate generation modes.

Invalid source/rendering leaves the previous complete output untouched. There
is no editor generation-retention or editor-specific doctor readiness contract.
Existing old managed workfiles require an explicit ownership-verified one-time
...
- package ABI revision stability when only non-ABI artifact provenance changes;
