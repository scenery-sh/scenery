...
This document defines the exact HTTP boundary produced and consumed by Scenery runtimes, generated clients, conformance tools, and agents. Content revisions identify exact wire behavior without selecting an older implementation.
...
`scenery.http-codec` depends on:

- the exact current specification catalog, type system, and canonical graph;
- the operation, binding, execution, authentication, authorization, and pipeline resource schemas.

The HTTP runtime implements direct delivery using this codec contract. Durable enqueue and wait delivery additionally require durable execution support.
...
An explicit `HEAD` route conflicts with another explicit `HEAD` route. A specification-provided automatic `HEAD` projection from `GET` is derived from the owning `GET` route and does not create an independent owner. `OPTIONS` generated for CORS follows the same ownership rule.
...
parameter. The base HTTP contract supports only single-segment parameters.
`{name...}` plus a matching `path_tail` block is defined only by the separately
implemented [HTTP path-tail behavior](http-path-tail.md). All other
...
Context mappings read only fields authorized by the binding/runtime contract from `principal` or `context`. Untrusted request values cannot populate runtime-owned context fields.

## 9. Body codecs

### 9.1 Core codecs

The HTTP contract defines:
...
JSON and text codecs use UTF-8. A declared non-UTF-8 text codec requires future explicit codec support. Media type parameters are normalized in canonical IR.
...
Every reachable operation result, declared error, and delivery-specific dispatch outcome has exactly one response mapping or a documented specification-provided mapping. `system.internal` is always reachable and uses the standard problem response unless explicitly narrowed by a permitted policy.
...
All current codecs are framework-enforced. A future coupled contract may use implementation-declared body semantics, but admission, limits, and pipeline facets remain independently classified.
...
`openapi_revision[gateway]` additionally includes the exact OpenAPI generator identity and projection options. A TypeScript client uses `typescript_client_revision[target]` from [typescript-client.md](typescript-client.md), which may cover one or more gateways.
...
The current HTTP contract does not define:
...
- non-UTF-8 text;
- wildcard path segments other than terminal `{name...}` under the path-tail contract;
- implementation-selected status codes outside declared outcomes.

These require future explicit contracts.
