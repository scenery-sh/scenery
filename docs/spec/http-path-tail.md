# Scenery HTTP Path Tails

Normative companion specification

Umbrella specification: [SPEC.md](SPEC.md)\
Base HTTP contract: [http.md](http.md)

## 1. Purpose

This contract adds typed terminal HTTP path tails without changing
`scenery.http-codec` or `scenery.runtime-http`. It defines:
...
The current path-tail contract builds on `scenery.http-codec`.

`scenery.runtime-http-path-tail` depends on:

- `scenery.runtime-http`;
...
MUST additionally record `scenery.http-path-tail`; a runtime route table
MUST additionally require `scenery.runtime-http-path-tail`.

A tool that does not implement path tails MUST emit
`feature_unavailable`. It MUST NOT lower a path tail as a single-segment
...
continue to scope the route namespace exactly as in `scenery.http-codec`.
...
All non-tail route conflicts remain governed by `scenery.http-codec`.
...
`relative_path` construction applies the specification-defined primitive rules after
...
type, empty-capture behavior, decoding guarantee, and schema revision.

The template, mapping, target type, empty-capture behavior, and schema revision
participate in `contract_revision`, the gateway HTTP-surface revision,
generated-client revision, OpenAPI revision, compatibility comparison, and
source-map provenance.

## 7. Generated Go adapter and runtime router

The generated Go adapter and runtime router obey these rules:
...
The generated descriptor and metadata record both the base HTTP codec revision
and the path-tail schema revision. Adding or removing a path tail changes the
client revision.
...
- reject projection with `feature_unavailable`.
...
from the old client/server direction independently by the current evolution
rules.
...
A conforming path-tail implementation MUST pass fixtures for:
...
- explicit unsupported-capability failure when path tails are unavailable.
...
- honest OpenAPI vendor extension or explicit unsupported-capability failure.

## Appendix A: Deliberate exclusions

The current path-tail contract does not define:
...
Those require future explicit contracts or ordinary operation logic.
