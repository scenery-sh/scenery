...
Onlava should be resilient when an app runs browser-backed TypeScript Temporal workers. The immediate motivating case is an ONLV ChatGPT browser worker, but this plan is deliberately scoped to onlava itself: source/build preparation must ignore browser runtime artifacts, and `onlava dev` must confidently supervise and clean up generated TypeScript worker processes.

The visible end state for onlava is:
...
- Hidden app-local artifact roots such as `.onlava/artifacts/...` remain ignored by build/source walking.
- Dev-started TypeScript Temporal workers are covered by tests for supervisor PID monitoring.
- Detached dev-session cleanup can reap stale generated `worker.ts` processes for the current app root without touching unrelated Bun or foreground `onlava worker typescript` processes.

The ONLV app endpoint, ChatGPT automation, Playwright dependency, smoke script, and DevBootstrap docs are out of scope for this onlava ExecPlan.
...
- [x] 2026-05-30: Tightened generated TypeScript worker tests so `worker.ts` must read `ONLAVA_DEV_SUPERVISOR_PID`, probe the supervisor with `process.kill(pid, 0)`, install an interval, and exit when the supervisor disappears.
...
- [x] 2026-05-30: Focused validation passed: `go test -count=1 ./internal/build ./internal/workers ./cmd/onlava`.
- [x] 2026-05-30: Full validation passed: `go test -count=1 ./...`, `go install ./cmd/onlava`, `git diff --check`, and `onlava harness self --json --write`.

## Surprises & Discoveries

- 2026-05-30: The bug note names ONLV files such as `agents/chatgpt_login.ts` and `agents/chatgpt_browser.worker.ts`, but those are app-level deliverables and are not part of this onlava plan.
- 2026-05-30: The relevant onlava substrate risks are independent of ChatGPT selectors: browser profile files can create non-regular filesystem entries during build prep, and stale generated TypeScript workers can keep polling old Temporal queues after detached dev sessions move on.
...
- Decision: Keep this plan limited to onlava runtime/substrate hardening.
  Rationale: The pasted note mixes onlava and ONLV work. The user explicitly asked for onlava-specific items and to ignore ONLV app items, so this plan excludes ONLV endpoint, workflow, Playwright, smoke-script, and docs changes.
  Date/Author: 2026-05-30 / Codex.
- Decision: Do not change standalone `onlava worker typescript` ownership semantics.
...
- Decision: Use a detached-dev worker registry instead of broad Bun/process scanning.
  Rationale: A registry under `.onlava/generated/temporal/typescript/dev-worker.json` lets onlava match the stale worker to the current app root and generated `worker.ts` path without killing arbitrary foreground workers or unrelated Bun processes.
  Date/Author: 2026-05-30 / Codex.
...
Completed on 2026-05-30. Onlava now ignores browser runtime artifact directories and unsupported non-regular files during build prep, generated TypeScript worker supervisor monitoring is locked by tests, and detached dev sessions can conservatively reap registry-matched stale generated TypeScript workers while recording an observable dashboard event.

Validation passed with the default harness writing `.onlava/harness/self-latest.json`. The harness reported existing warning-level package timing and knowledge review diagnostics, but `ok` was true and no new validation failures remained.

## Context and Orientation

The source/build preparation code lives in `internal/build/build.go`. It discovers app source files, copies them into generated build workspaces, computes fingerprints, and skips generated or irrelevant directories. Existing hidden-directory behavior already keeps `.onlava/...` out of app source walking, but browser runtime directories outside hidden paths can still contain sockets or other non-regular files.

The TypeScript Temporal worker generation and dev supervision code is split across:

- `internal/workers/typescript.go`, which generates TypeScript worker runtime files under `.onlava/generated/temporal/typescript/`.
- `cmd/onlava/dev_typescript.go`, which generates and starts TypeScript workers during `onlava dev`.
- `cmd/onlava/dev_supervisor.go`, which owns dev process lifecycle and shutdown behavior.
- `cmd/onlava/worker.go`, which owns the standalone `onlava worker typescript` foreground command and should not be swept into dev-session cleanup.

The generated TypeScript worker already has the right architectural shape: onlava owns the registry, worker entrypoint, Temporal environment, and worker process startup. This plan adds defensive filesystem hygiene and lifecycle confidence around that runtime.
...
Add regression coverage that generated `worker.ts` reads `ONLAVA_DEV_SUPERVISOR_PID` and exits when the supervisor process disappears. The test should assert generated source behavior, not only runtime logs.

Milestone 3: Dev-supervisor shutdown handles TypeScript workers explicitly.

Add lifecycle tests around dev-supervisor close behavior for TypeScript worker child processes. The test should prove interrupt, wait-or-kill, and state detachment behavior for worker processes started by `onlava dev`.
...
Next, add generated-worker tests in `internal/workers/workers_typescript_test.go`. The generated `worker.ts` should continue to include supervisor monitoring tied to `ONLAVA_DEV_SUPERVISOR_PID`. This prevents future edits from silently removing the cleanup safety net while still allowing foreground `onlava worker typescript` to run without dev-supervisor ownership.

Finally, implement dev-supervisor process cleanup. Keep the matching rules conservative:

- Process must be marked as dev-supervised, for example by environment such as `ONLAVA_DEV_SUPERVISOR=1`.
- Process must belong to the current app root, for example `ONLAVA_APP_ROOT=<current app root>`.
- Command line must include `.onlava/generated/temporal/typescript/worker.ts`.
...
3. Add generated TypeScript worker tests in `internal/workers/workers_typescript_test.go`.
   ...
   Assert that generated `worker.ts` includes `ONLAVA_DEV_SUPERVISOR_PID` handling and supervisor-disappearance monitoring.

4. Add dev-supervisor lifecycle coverage in `cmd/onlava/dev_supervisor_test.go`.
...
5. Add detached dev-session stale-worker cleanup in `cmd/onlava/dev_typescript.go` and/or `cmd/onlava/dev_supervisor.go`.

   Keep the cleanup path detached-dev-only. It must not run for foreground `onlava worker typescript`.
...
```sh
go test ./internal/build ./internal/workers ./cmd/onlava
```
...
- Existing `.onlava/artifacts/...` hidden-directory behavior remains unchanged.
...
- Foreground `onlava worker typescript` behavior remains unchanged.
...
Expected generated/local artifacts remain under `.onlava/` and are not committed.

The plan intentionally does not prescribe ONLV paths such as `agents/chatgpt_browser.worker.ts`, `scripts/smoke-chatgpt-browser.sh`, or ONLV `AGENTS.md`. Those belong to the ONLV app plan and should not be implemented in this onlava repository under this ExecPlan.
...
rg -n "ONLAVA_DEV_SUPERVISOR_PID|worker.ts|typescript worker" internal/workers cmd/onlava
ps -axo pid,ppid,command | rg 'bun .*worker.ts|onlava dev' | rg -v rg
```

## Interfaces and Dependencies

Onlava interfaces in scope:

- Build/source walker behavior in `internal/build`.
- Generated TypeScript Temporal worker runtime under `.onlava/generated/temporal/typescript/`.
...
- Browser runtimes such as Chromium, Playwright, or Bun are treated as external processes/filesystem producers; this plan does not add Playwright to onlava.
