...
- T4. Remove global stubs blocking `t.Parallel()`: Generate/task lifecycle stubs move onto injected structs (~0.7-1.4s).
...
No public CLI or JSON contract changes are planned. Internal seams added: injectable frontend restart delay/starter, per-call generate/task/db lifecycle hooks, `SourceSnapshot` through build prepare/refresh/reuse, concurrent Victoria component start. Depends on plan 0050's testlimit and timing tooling.

## Execution Order

Workstreams overlap in `internal/build` and `watch.go`, so phases keep file sets disjoint:

- Phase 1 (parallel): T1+T2 (frontend supervisor/tests), T4 (generate/task stubs), U1+U2+U3 (build pipeline; one agent, sequential internally).
