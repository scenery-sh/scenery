...
Start from the existing generated collection page path. Add no new runtime service, no new dependency, and no new page framework. The static DSL should only describe field-level query/display metadata. The generator should continue to use the source row type for Electric/TanStack DB input, materialize page records through the generated projection function, and expose mountable generated pages through the existing barrel.
