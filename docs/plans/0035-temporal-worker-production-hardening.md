...
Milestone 2 removes ambiguous activity routing and adds the start/handle API needed by real apps. Empty activity task queues must no longer silently register on one queue but execute on another. `temporal.Start` should accept options such as workflow ID, task queue, memo, search attributes, timeouts, and versioning override without breaking existing callers.

Milestone 3 migrates ONLV away from process-local async coordination. Short request/response waits should return workflow results through `Run.Get`. Long-running or streaming jobs should write durable job status and events and return a stable job/workflow ID.
...
Next extend `temporal.Start` with variadic options so current call sites still compile. Add deterministic workflow ID support first because ONLV already has business identifiers such as `job_id`, `attempt_id`, and request IDs. Then add typed handles for existing workflows so API processes can reconnect to a workflow by ID and run ID without preserving Go memory.
...
7. Add `StartOption` support without breaking existing calls:
...
8. Add typed handle support:
   ...
   Signals, queries, and updates can be added in the same milestone if the SDK mapping stays small; otherwise record a follow-up.
9. Stop setting `VersioningOverride` on every `Start` by default once Worker Deployment promotion is operator-controlled. Keep explicit opt-in through `WithPinnedBuildID` or a similarly named option.
10. Migrate ONLV `temporal.Start` call sites to deterministic IDs such as `jobs.submission-attempt.<attempt_id>`, `house.process.<job_id>`, `maps.earth.generate.<job_id>`, and `codex.exec.<job_id>`. Pick a conflict policy that makes retries idempotent and document it in code comments only where the policy is not obvious.
11. Replace ONLV waiter flows. For bounded flows, make workflows return response structs and have API handlers call `run.Get(ctx)`. For streaming or long-running flows, persist events/status in durable app storage and return IDs to clients.
12. Replace ONLV activity-starts-workflow chains. In house, a parent process-scene workflow should execute roofmapnet then rooftopology as activities or child workflows. In maps, generation and save should be coordinated by workflow code. Do not pass local temp directory paths across stages unless both stages are explicitly constrained to the same host; prefer durable artifact storage.
13. Add production gating around `EnsureTemporalWorkerDeploymentCurrentVersion`. Local mode may call it automatically. Production/cloud modes should not. Add explicit CLI commands under an appropriate command group, for example `onlava temporal deployment set-current`, `onlava temporal deployment ramp`, and `onlava temporal deployment drain`.
14. Extend Temporal cron config and docs for overlap policy, catchup window, pause-on-failure, and per-job activity timeout/retry. Use scheduled start time from Temporal schedule metadata where available instead of `time.Now()` inside `runTemporalCronActivity`.
15. Introduce a stronger worker manifest shape, preferably as `onlava.worker.manifest.v2`, with queue-level registrations:
...
16. Extend Temporal connection config in `internal/app/root.go`, `runtime/temporal.go`, and docs to cover TLS/mTLS/API-key environment variables and a real `onlava-json-v1` payload codec profile. Keep config validation strict and fail with actionable messages when required env vars are missing.
17. Update `docs/local-contract.md`, `docs/app-development-cookbook.md`, inspect schemas, generated binding docs, and ONLV `.onlava.json` examples to make Temporal production behavior explicit.
...
- `temporal.Start` supports deterministic workflow IDs, task queue override, memo/search attributes, timeouts, and opt-in version pinning.
...
Queue filtering and StartOptions are additive or narrowly breaking changes. If a step fails, rerun tests after reverting only the files changed in that step; do not revert unrelated user work in the repository.

When changing public Temporal API behavior, keep existing call sites compiling where possible by using variadic options and targeted validation. If requiring `ActivityConfig.TaskQueue` causes widespread breakage, update fixtures and ONLV call sites in the same branch before treating the milestone as complete.
...
temporal.Start(ctx, workflow, input, opts...)
temporal.GetWorkflow[O](ctx, workflowID, runID)
temporal.WithWorkflowID(id)
temporal.WithWorkflowIDPrefix(prefix)
