...
- [x] (2026-09-24) Review follow-up (findings R1–R3, R5 of the 2026-09-24
  review): `dev-runtime.ts` keeps unfinished calls by request id, so abort,
  `close()`, `dispose()` and drops discard unsent calls instead of sending them
  later; one owned retry timer; `dispose()` refuses and cancels storage
  transfers; body-phase transfer failures are normalized; malformed frames fail
...
- (2026-09-24 review) `close()` rejected pending calls but left their frames in
  the send queue, and the replaced socket's close handler skipped cleanup, so a
  `db/query` DELETE rejected with `closed` was sent on the next connection; a
  reconnect timer also survived `dispose()`, and an aborted queued call was
  still sent. The generator test only matched source strings.
...
- Decision: `status` results carry payload identity and the generated client
  ...
  Date: 2026-09-23.
- Decision: `dispose()` also aborts in-flight storage transfers (code
  `closed`) and refuses new ones before `fetch`; `close()` stays socket-only.
  Rationale: a disposed client must not keep writing, while a transfer does not
  depend on the socket. Date: 2026-09-24. Author: Claude.
- Decision: connection listeners hear only changes of `connected`: a failed
  connection attempt notifies nothing, and `close()` or `dispose()` of an open
  socket notifies `false`. Rationale: listeners mirror the `connected` getter;
  an unreachable runtime already fails calls with `closed`. Date: 2026-09-24.
- Decision: a frame that is not a JSON-RPC response fails the connection's
  calls with `protocol` instead of being ignored, and the client adds no call
  deadline; callers bound calls with an `AbortSignal` such as
  `AbortSignal.timeout()`. Rationale: an uncorrelated frame would otherwise
  leave its caller waiting forever; deadlines are caller policy.
  Date: 2026-09-24.
- Decision: a transfer's network failure is `unavailable` before and after the
...
top of it. Building and preparing the Scenery CLI no longer needs Bun. The
