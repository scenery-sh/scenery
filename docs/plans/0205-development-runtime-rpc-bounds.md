...
- [x] (2026-09-24 14:18 CEST) Milestone 3: catalog diagnostics
  ...
  documentation, and a `DevRuntimeClient` block in
  `internal/generate/testdata/typescript_client_conformance.test.ts`
  (`bun test` of that file: 30 pass).
...
- [ ] Milestone 6, remaining: a complete `--probe worktree` run with the final
  fixture module (A9 included). The fourth run (started 14:43 CEST) stopped
  when the machine's data volume filled up (296 MiB free, OrbStack's Docker
  daemon gone) and could not write its report; it had not reached A9. Its
  retained root `/var/folders/v3/1ff5y8y13dqc_czsbf085w1h0000gn/T/scn-worktrees-1842240800`
  still records eight managed PostgreSQL clusters, to be removed with
  `SCENERY_AGENT_HOME=<root>/state .scenery/harness/bin/scenery down --app-root <app-root> -o json`
  and `... prune --app-root <app-root> --db --older-than 1ns -o json` once
  Docker runs again.
...
- The review named `internal/generate/testdata/dev_runtime_client.test.ts` as
  the generated client's behavior tests. No such file exists in any branch
  (`git log --all -- internal/generate/testdata/dev_runtime_client.test.ts` is
  empty); the client was covered only by `generate_typescript_dev_runtime_test.go`
  and the generated-client typecheck.
...
3. Diagnostics and clients. Catalog rows `SCN8011`–`SCN8013`, regenerated
   committed clients, client documentation, and a dev runtime block in
   `internal/generate/testdata/typescript_client_conformance.test.ts` proving
   the failure objects reach `DevRuntimeError`. Proof: `go test ./internal/spec
...
`postgresRows` in `internal/generate/dev_runtime_client.ts`, and adds a
`DevRuntimeClient` block to the TypeScript conformance test with a scripted
