...
- (2026-07-07, Claude + Petr) **Tokens are encrypted at rest** with AES-256-GCM (stdlib only). Historical implementation note: the first design named the key through `auth.google_oauth.token_cipher_key_env`; the current contract uses fixed canonical `AUTH_TOKEN_CIPHER_KEY` and rejects that removed selector. In local runtime with no key set, a fixed dev key is derived from the local dev JWT secret so `scenery up` works with zero setup.
...
- `auth/standard.go` — current `StandardConfig` / `GoogleOAuthConfig{Enabled, AllowedScopes}`, standard endpoint registration, canonical Google credential loading, and lazy `standardAuthService`.
...
- `auth/standard_google.go` and the native contract/runtime registration — current Google sign-in and connection endpoint ownership. The former `internal/standardauthmeta` and special TypeScript standard-auth generator were removed during post-v0 cleanup.
