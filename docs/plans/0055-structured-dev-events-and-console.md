...
`onlava dev --detach`, `onlava attach`, and `onlava logs --jsonl` already provide the right lifecycle shape for local development sessions. The missing foundation is a structured event plane that identifies where output came from and what it means.

Today the local log store is centered on raw process output: app id, session id, pid, stream, output, and time. That is enough to follow stdout and stderr, but it is not enough for a reliable local console that separates the API app, TypeScript Temporal worker, Temporal server, managed Postgres/Electric, frontends, Grafana, build phases, and supervisor events without fragile pid or text heuristics.

After this work, `onlava dev` emits normalized, versioned dev events. The SQLite store persists raw output plus parsed fields and source identity. Plain terminal following, JSONL output, dashboard views, agent consumers, and a terminal UI consume the same event stream:
...
- [x] 2026-05-31: Implemented structured source identity, `dev_sources`/`dev_events` storage, best-effort parsing, and the `onlava.dev.event.v1` schema.
- [x] 2026-05-31: Routed app, TypeScript worker, managed frontend, build, supervisor, Postgres, Electric, Temporal, Victoria, and Grafana state/output into structured dev events while preserving legacy process output.
- [x] 2026-05-31: Added `onlava logs` and `onlava attach` filters for source, kind, level, grep, and since; JSONL now emits `onlava.dev.event.v1`.
- [x] 2026-05-31: Added `onlava attach --tui`, `onlava console`, non-TTY fallback, source summaries, event expansion, errors-only mode, and grouped recent errors.
- [x] 2026-05-31: Added `onlava status --watch`, docs, schemas, and focused tests for storage, filters, fallback, and console rendering.
- [x] 2026-05-31: Validated with focused tests, full `go test ./...`, `git diff --check`, and `go install ./cmd/onlava`. Ran `onlava harness self --json --write` twice; it wrote `.onlava/harness/self-latest.json` and all non-timing steps passed, but the Go test timing budget step still failed.
- [x] 2026-06-01: Added dual-write of structured dev events into VictoriaLogs and made `onlava logs`/plain `attach` support `--backend auto|victoria|sqlite` for side-by-side parity verification.
- [x] 2026-06-01: Revalidated with focused tests, full `go test ./...`, `git diff --check`, `go install ./cmd/onlava`, and `onlava harness self --json --write`; the harness again only failed the existing Go test timing budget.
...
- 2026-05-31: `cmd/onlava/dev_supervisor.go` already centralizes app and TypeScript worker output through `captureProcessOutput`, which strips ANSI, writes the store row, notifies the dashboard, and emits `process.output` console events.
- 2026-05-31: Managed frontend processes currently write stdout/stderr to per-session log files in `cmd/onlava/dev_frontends.go` instead of the devdash store. The new managed process helper must cover them explicitly.
...
- 2026-05-31: `onlava harness self --json --write` is functionally green for this change except the existing timing gate. The latest written snapshot reports only the `go tests` step as failed: full suite 9.049s over the 7.000s target, with several packages over the 2.000s package budget.
- 2026-06-01: VictoriaLogs' `/select/logsql/query` returns JSON Lines and accepts `limit`, `start`, and `end` form/query args. The `/insert/jsonline` API lets onlava choose stable field names instead of depending on OTLP attribute flattening.
...
- Decision: Introduce a new versioned event schema named `onlava.dev.event.v1`.
  Rationale: The existing `onlava.logs.event.v1` JSONL schema is process-output-shaped. A new schema can model source identity, parsed fields, raw output, and parsing metadata without overloading the old event name.
  Date/Author: 2026-05-31 / Codex

- Decision: Persist raw output forever and treat parsing as best-effort.
  Rationale: onlava controls some logs, but Temporal, Electric, Grafana, Bun/Node workers, Vite/Astro frontends, and future sidecars will not all use one logger. Consumers need structured fields when available and exact raw text when parsing fails.
  Date/Author: 2026-05-31 / Codex
...
- Decision: Dual-write dev events to VictoriaLogs through its JSON-line ingest API while keeping SQLite as the canonical local insert during verification.
  Rationale: SQLite still assigns the event id and remains the comparison baseline. JSON-line ingest gives onlava stable queryable fields for `onlava.dev.event.v1`; `onlava logs --backend victoria` can then be compared against `--backend sqlite` before the SQLite read path is removed.
...
The implementation adds a source-aware structured dev event plane without removing the legacy `process_output` path. `onlava dev` now records normalized `onlava.dev.event.v1` events for app output, TypeScript worker output, managed frontends, build phases, supervisor lifecycle notices, and substrate readiness/status. Each event preserves raw text, parsed level/message/attrs when possible, source identity, stream, pid, restart id, and parse metadata.

The CLI now exposes the structured stream through `onlava logs` and `onlava attach` with `--source`, `--kind`, `--level`, `--grep`, and `--since` filters. JSONL output uses the new schema, while plain output remains pipeable and falls back to legacy rows when structured events are unavailable. `onlava attach --tui` and `onlava console` provide an observing-only terminal cockpit with source tabs, source summaries, errors-only mode, search, event expansion, grouped errors, and automatic non-TTY fallback. `onlava status --watch` was added for a lightweight non-TUI overview.
...
- `onlava dev --detach` starts a background dev session.
- `onlava attach` follows the current session logs by default.
- `onlava logs --jsonl` emits line-delimited JSON.
- `onlava status --json` reports agent-backed session status.
...
```json
{
  ...
  "fields": {
    "activity": "SendEmail",
    "task_queue": "onlava.billing.feature-x-839a"
  },
...
The app process, TypeScript worker, frontends, managed Electric, managed Postgres, Temporal, Victoria, Grafana, setup/build phases, and supervisor notices should all write source-aware events. Shared or external substrates should still write state events even when onlava does not own their stdout.
...
```sh
onlava logs --source api
onlava logs --source worker:typescript --follow
onlava logs --kind substrate
onlava logs --level error
onlava logs --grep "activity failed"
onlava logs --since 15m
onlava attach --jsonl
onlava status --watch
```

`onlava attach` remains the human default. `onlava attach --jsonl` and `onlava logs --jsonl` emit `onlava.dev.event.v1` after the schema and docs are updated.

Milestone 4: Observing-only terminal UI.

Add `onlava attach --tui` and `onlava console`. In a real TTY, open a full-screen source cockpit. In non-interactive contexts, fall back to normal `onlava attach` following.

The first view has three regions:

```text
onlava dev session: billing-dev / feature-x-839a
[all] [api] [worker:go] [worker:typescript] [temporal] [postgres] [electric] [frontend:web] [grafana] [build]
...
electric            starting  pid 12360   waiting on pg           last log 0s ago
---------------- logs: worker:typescript ----------------
12:44:01.223 INFO  registered activity     activity=SendEmail queue=onlava.billing.abc
...
Once the data model is in place, make the supervisor write `DevEvent` records. Preserve raw terminal output for normal attached `onlava dev`, but ensure stored and JSONL events always include source metadata. Replace direct frontend log-file writes with managed process capture or a tee that both writes the historical log file and records structured events during the migration.

After source-aware events exist, update `onlava logs` and `onlava attach` to query `dev_events`. Add filters as plain CLI behavior with focused tests. Only then add the terminal UI, using the same store query/follow API as `logs`.

For terminal rendering, start with a small internal package under `cmd/onlava` or `internal/termui` that handles TTY detection, alternate screen setup, key input, resize, and plain ANSI drawing. Keep rendering pure enough to snapshot in tests. If the MVP becomes complex enough to justify Bubble Tea, add that as a deliberate follow-up decision with an architecture-harness update and dependency rationale.
...
4. Add `docs/schemas/onlava.dev.event.v1.schema.json` and update self-harness schema validation if needed.
...
11. Make `onlava logs --jsonl` and `onlava attach --jsonl` encode `onlava.dev.event.v1`; update `docs/local-contract.md`, CLI usage, and schemas in the same change.
12. Add `onlava attach --tui` parsing and the `onlava console` alias. Ensure non-TTY fallback is automatic and tested.
...
- `onlava logs` and `onlava attach` filters.
- `onlava attach --jsonl` schema output.
- `onlava attach --tui` non-TTY fallback.
...
```sh
onlava dev --detach --app-root testdata/apps/basic
onlava logs --app-root testdata/apps/basic --session current --source api --follow --limit 20
onlava attach --app-root testdata/apps/basic --session current --jsonl --limit 5
onlava attach --app-root testdata/apps/basic --session current --tui
onlava down --app-root testdata/apps/basic
```

Acceptance criteria:

- Every event emitted by `onlava dev` has app id, session id, source identity, time, raw text or structured message, level, parsed fields, and parse metadata.
- `onlava logs --source`, `--kind`, `--level`, `--grep`, and `--since` work in both historical and follow modes.
- JSONL output validates against `onlava.dev.event.v1`.
- `onlava attach` remains pipeable and does not require a TUI.
- `onlava attach --tui` and `onlava console` render a usable service cockpit in a TTY and fall back outside a TTY.
...
If a managed process exits during startup, emit a source status event and preserve the last raw lines. Retrying `onlava dev` should reuse normal supervisor cleanup paths and should not require manual database cleanup.
...
cmd/onlava/dashboard.go
cmd/onlava/main.go
cmd/onlava/*console* or cmd/onlava/*tui*
cmd/onlava/*test.go
...
```sh
onlava logs --errors
onlava console --view errors
onlava console --source worker:typescript
onlava status --watch --json
```

The UI should expose why a source exists when that is known. Examples: "generated TypeScript Temporal activity worker", "managed Electric sync service from dev.services.electric", "shared Temporal dev server", or "frontend route from proxy.frontends.web".
...
```text
onlava attach [--tui] [--jsonl] [--source <id>] [--kind <kind>] [--level <level>] [--grep <text>] [--since <duration>]
onlava console [same selection flags as attach --tui]
onlava logs [--source <id>] [--kind <kind>] [--level <level>] [--grep <text>] [--since <duration>] [--follow] [--jsonl]
onlava status --watch [--json]
```
...
- Do not add Bubble Tea, Bubbles, or Lip Gloss in the MVP without updating `cmd/onlava/harness_arch.go`, `go.mod`, the decision log, and validation evidence that the dependency pays for itself.
