...
- [x] 2026-06-12: M4 frontend foundation candidate implemented: `scenery generate data` now writes beta hidden TypeScript packages under `.scenery/gen/web/<frontend>/` with row/create/patch types, entity source definitions, TanStack DB collection descriptors/materializers, route/default page exports, slot type assertions, and a model-dsl fixture alias/typecheck/render proof.
- [x] 2026-06-12: M4 runtime-adapter follow-up candidate implemented: generated web packages now include `runtime.ts` with row-source collection factories, richer route records with stable IDs/entity/collection metadata, `createGeneratedRoutes(runtime)`, and `registerGeneratedRoutes(...)`; the model-dsl fixture proves the layout-kit contract consumes runtime-backed routes.
- [x] 2026-06-13: App-owned schema follow-up candidate implemented for issue #137: generated model Atlas HCL, seed SQL, CRUD SQL, and entity source metadata consistently use the service-owned schema-qualified table instead of hardcoded `public`.
...
- [x] 2026-06-13: Database URL env follow-up candidate implemented for issue #145: generated CRUD stores honor `dev.services.postgres.database_url_env` instead of hardcoding `DatabaseURL`, keep `SCENERY_MANAGED_DATABASE_URL` as fallback, and report missing-DSN errors with the configured env name.
- [x] 2026-06-13: Route-validation follow-up candidate implemented for issue #146: generated CRUD routes fail parse/check when their service-scoped route base falls under reserved prefixes (`/__scenery`, `/api`) or overlaps declared app endpoint routes such as static-vs-parameter conflicts.
...
- 2026-06-12: The generated M4 frontend package can expose production-facing adapter seams without importing TanStack packages directly. Emitting small factory functions over app-provided row sources keeps Scenery dependency-free while giving production apps a stable place to bind their router, row data source, and TanStack DB collection instance.
...
- 2026-06-12, maintainer worker: Keep the M4 runtime adapters dependency-free and app-owned at the edge. Scenery generates typed route/runtime factories and metadata, but the production app still supplies the real router registration function, row data source, TanStack DB runtime, and layout-kit implementation.
- 2026-06-13, maintainer worker: Generated model database artifacts use the service-owned schema derived from the model package/service root. `model.Table(...)` remains the table name; Atlas HCL, seed SQL, generated CRUD SQL, and entity source metadata all target the same schema-qualified table.
...
- 2026-06-13, maintainer worker: Generated model stores use the same configured app database URL env as the managed Postgres runtime path. `DatabaseURL` remains the default, but apps that set `dev.services.postgres.database_url_env` get generated store lookups and missing-DSN diagnostics for that configured env name.
- 2026-06-13, maintainer worker: Generated CRUD route validation now treats route parameters as overlapping concrete generated routes for collision diagnostics, so a generated `/api/tasks` route fails against a declared `/api/:table_name` endpoint instead of relying on runtime route precedence.
- 2026-06-13, maintainer worker: Generated CRUD create/patch request structs keep the DB-column JSON contract for compatibility but overlay exact exported field-name aliases. This lets response-shaped payloads round-trip fields like `CreatedAt` while preserving existing clients that send `created_at`.
- 2026-06-13, maintainer worker: Generated CRUD stores now use a single package-level pgx pool keyed by DSN instead of per-entity pools. Generated list endpoints expose simple offset pagination (`limit`, `offset`) and always emit bounded SQL; cursor pagination stays out of this bounded production-readiness slice.
