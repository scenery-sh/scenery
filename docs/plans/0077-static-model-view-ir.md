...
- 2026-06-12: The generated M4 frontend package can expose production-facing adapter seams without importing sync or TanStack packages directly. Emitting small factory functions over app-provided `sync.baseURL` and row sources keeps Scenery dependency-free while giving production apps a stable place to bind their router, sync client, and TanStack DB collection instance.
...
- 2026-06-12, maintainer worker: Keep the M4 runtime adapters dependency-free and app-owned at the edge. Scenery generates typed route/runtime factories and metadata, but the production app still supplies the real router registration function, sync base URL/client, TanStack DB runtime, and layout-kit implementation.
- 2026-06-13, maintainer worker: Generated model database artifacts use the service-owned schema derived from the model package/service root. `model.Table(...)` remains the table name; Atlas HCL, seed SQL, generated CRUD SQL, and sync shape metadata all target the same schema-qualified table.
...
- 2026-06-13, maintainer worker: Generated CRUD route validation now treats route parameters as overlapping concrete generated routes for collision diagnostics, so a generated `/sync/tasks` route fails against a declared `/sync/:table_name` endpoint instead of relying on runtime route precedence.
