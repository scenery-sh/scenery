...
* [ ] 2026-07-07: Milestone 10 preflight: ONLV now declares `deploy.domain` `local.clean.tech` with `deploy.root` `next`, and `.scenery/harness/bin/scenery deploy enable --app-root /Users/petrbrazdil/Repos/onlv --json` registered the enabled target. Current live machine state from `.scenery/harness/bin/scenery deploy status --json`: public helper missing, Caddy edge stopped, resume LaunchAgent missing, no running ONLV `scenery up` session, no Caddy cert for `local.clean.tech`, and ports 80/443 are not listening. DNS for `local.clean.tech` resolves to Cloudflare IPs (`104.21.1.153`, `172.67.129.115`, `2606:4700:3030::6815:199`, `2606:4700:3033::ac43:8173`) instead of this Mac's public IP `217.112.163.198`. Installed `scenery` is still `/Users/petrbrazdil/go/bin/scenery`, older than this worktree's deploy implementation; `sudo -n true` requires a password. Network facts measured: LAN IP `192.168.0.185`, public IP `217.112.163.198`.
* [ ] 2026-07-07: Milestone 10 Cloudflare preflight: the operator confirms Cloudflare's origin record points at public IP `217.112.163.198`, while public DNS still returns Cloudflare proxy anycast IPs. The deploy-status DNS diagnostic now classifies all-Cloudflare DNS answers as accepted proxy evidence instead of a direct-A/AAAA mismatch; the remaining blockers are setup/listener/Caddy/session/cert.
* [ ] 2026-07-07: Milestone 10 DNS preflight: existing ONLV hostnames (`onlv.dev`, `api.onlv.dev`, `ui.onlv.dev`, `next.onlv.dev`, `pulse.onlv.dev`, `blog.onlv.dev`, `console.onlv.dev`) resolve to Cloudflare A records `104.21.28.79` / `172.67.144.228`, not this Mac's public IP `217.112.163.198`; they cannot prove the direct-router acceptance path until DNS/proxy routing is changed or a different test domain is provided.
