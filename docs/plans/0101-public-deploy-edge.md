...
* [ ] 2026-07-08: Separate two-domain acceptance remains open. `micro.scenery.sh` is enabled and has a cert, but it has no live deploy session; `scenery deploy resume --json` failed for that target because an existing Micro runtime owns `/Users/petrbrazdil/Repos/Micro/platform` and is in `compile-error` state. Clean, fix, or disable that target before claiming simultaneous two-app acceptance.
...
Milestone 10 is live-accepted for ONLV on `local.clean.tech`: public helper, Caddy, agent, DNS/Cloudflare proxy, production origin cert, app route, API route, and public containment checks all passed on 2026-07-08. The overall plan remains active for the separate two-domain proof (`micro.scenery.sh`) and any operator-driven reboot/login observation.
