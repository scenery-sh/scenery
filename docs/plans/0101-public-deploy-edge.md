...
* [x] 2026-07-07: Milestone 1 implemented: `deploy.domain` / `deploy.root` config model, config validation, JSON schema, `scenery check` info diagnostic for ambiguous public root, and focused tests (`go test ./internal/app ./cmd/scenery`).
* [x] 2026-07-07: Milestone 2 implemented: machine deploy registry at `<agent home>/agent/deploy.json`, `scenery deploy enable|disable|status`, beta help entry, domain-conflict rejection, target live-session reporting, and tests (`go test ./internal/agent ./internal/app ./cmd/scenery`).
* [x] 2026-07-07: Milestone 3 pure helper slice implemented: `--public` / `--helper-version` parsing and plist round-trip, public 80/443 listen-spec selection, additive `http_target_addr` metadata, and port-80 target selection tests. `scenery deploy setup` sudo orchestration and listener preflight remain open.
* [x] 2026-07-07: Milestone 4 Caddyfile generator slice implemented: public-domain ACME site blocks, staging CA support, pinned storage/email/http_port/https_port directives, HTTP redirect blocks, public-edge header injection, domain sorting/de-duplication, and focused Caddy config tests. Reload wiring and registry-to-edge restart integration remain open.
* [x] 2026-07-07: Milestone 3 setup slice implemented: `scenery deploy setup` is macOS/non-root, preflights 80/443 with process info, writes ACME registry settings, runs the existing sudo privileged-helper installer with `--public --helper-version`, restarts the unprivileged edge, and has focused tests (`go test ./cmd/scenery`).
* [x] 2026-07-07: Milestone 4 restart wiring slice implemented: `edgeRestart` loads enabled deploy registry targets into public Caddy sites, pins public Caddy storage under `<agent home>/edge/caddy-data`, publishes `http_target_addr`, and validates the public helper HTTP target when installed. Graceful reload on enable/disable remains open.
* [x] 2026-07-07: Milestone 4 enable/disable refresh slice implemented: successful registry mutations now refresh a running public setup by rendering a registry-backed `Caddyfile.next`, calling `caddy reload` through the admin socket, promoting the config on success, and republishing target metadata. If public setup is absent, enable/disable still only record intent. Focused tests pass (`go test ./cmd/scenery`).
* [x] 2026-07-07: Milestone 3 teardown slice implemented: `scenery deploy teardown` is macOS/non-root, reinstalls the privileged helper in loopback-only mode, removes the resume LaunchAgent if present, restarts the edge, keeps registry/certs, and has focused tests (`go test ./cmd/scenery`).
...
* [x] 2026-07-07: Milestone 7 implemented: `scenery deploy status --json` now includes structured diagnostics for helper state/version, wildcard 80/443 listeners, LAN/public reachability probes, public IP, DNS mismatch fixes, power sleep posture, macOS firewall state, and Caddy cert expiry; `scenery doctor --json` exposes a summarized `deploy` section and mirrors deploy checks. Focused command tests and full suite pass (`go test ./cmd/scenery`, `go test ./...`).
* [x] Milestone 7: reachability/DNS/power/firewall diagnostics in `scenery deploy status` and `scenery doctor`.
* [x] 2026-07-07: Milestone 8 implemented: helper drift now separates informational helper-version mismatch from actionable target-metadata contract mismatch, `scenery deploy status` / doctor report that distinction, and successful `scenery upgrade` emits a `deploy` notice plus human setup reminder only when the privileged listener contract needs sudo refresh. Focused upgrade tests pass (`go test ./cmd/scenery`).
* [x] Milestone 8: upgrade story — helper version drift detection, `scenery upgrade` notice.
* [x] 2026-07-07: Milestone 9 implemented: updated local contract, agent guide, installable skill, README, cookbook, knowledge index, deploy registry/status schemas, and self-harness schema-validation samples. Split edge process helpers out of `cmd/scenery/edge.go` to clear the architecture hard limit. Self-harness passes with warnings (`scenery harness self --summary --write`; warnings: review-due docs, large-file warnings, Go timing budget).
* [x] Milestone 9: docs, schemas, knowledge index, self-harness.
* [x] 2026-07-07: Config cleanup after app-config `proxy` removal: `deploy.root` now resolves against the top-level `frontends` map, not `proxy.frontends`. ONLV declares `deploy.domain` `local.clean.tech`, `deploy.root` `next`, and top-level `frontends.next.root` `apps/next`; the installed `scenery check --app-root /Users/petrbrazdil/Repos/onlv --json` returned `ok: true` with no diagnostics.
* [x] 2026-07-08: Milestone 10 ONLV live acceptance completed with installed `scenery` `v0.3.2-0.20260707204047-f63c506e75ea+dirty`. `scenery deploy status --json` and `scenery doctor --json` report deploy `ready: true`; the public helper is running on `[::]:80` and `[::]:443`, Caddy and the agent are running, the resume LaunchAgent plist is installed, ACME is set to production, and `local.clean.tech` is enabled with live ONLV session `main-dbe32e` and root service `next`.
* [x] 2026-07-08: Milestone 10 route and containment proof for `local.clean.tech`: origin HTTP via `--resolve local.clean.tech:80:127.0.0.1` returns `308` to HTTPS; origin HTTPS via `--resolve local.clean.tech:443:127.0.0.1` returns `200` at `/next/`; direct LAN and direct public-origin probes with domain SNI return `200`; public Cloudflare HTTPS returns `200` at `/next/`; `/api/auth/me` returns `401`; `/runtime/health` returns `404`.
* [x] 2026-07-08: Milestone 10 certificate proof: Caddy storage has a production Let's Encrypt cert for `local.clean.tech` (`CN=local.clean.tech`, issuer `Let's Encrypt YE2`, SAN `DNS:local.clean.tech`, expires `2026-10-05T15:11:21Z`). Caddy log lines show `http-01` key-auth served and `certificate obtained successfully`. Public DNS still resolves to Cloudflare proxy IPs and the external browser-facing cert is Cloudflare's `clean.tech` wildcard cert, which is expected for the proxied hostname.
* [x] 2026-07-08: Milestone 10 reboot/login notes recorded without rebooting from Codex. The root LaunchDaemon is loaded and running with `RunAtLoad`/`KeepAlive`; the user LaunchAgent plist exists with `RunAtLoad` and runs `/Users/petrbrazdil/go/bin/scenery deploy resume`, but `launchctl print gui/501/dev.scenery.deploy-resume` reports the service is not loaded in this current login session. Manual `scenery deploy resume --json` restarted the edge and reported ONLV `already_running`, proving the resume command path.
* [x] 2026-07-22: Separate two-domain acceptance passed. ONLV at `local.clean.tech/next/` and Micro at `micro.scenery.sh/platform/` both returned 200 through one live Caddy edge at origin and through Cloudflare, both origin HTTP routes redirected to HTTPS, both origin certificates were production Let's Encrypt YE2 certificates, and runtime/dashboard containment paths returned 404 on both domains.
* [x] 2026-07-14: Milestone 11 implemented after a field outage (Cloudflare 525 with green status): frozen tolerant helper handoff reader (`localagent.LoadEdgeHelperTarget`, `EdgeHelperContractRevision`), `--helper-contract` plist stamp with stamp-based drift detection, end-to-end TLS/SNI handshake probes in `scenery system edge status` and `scenery deploy status` (`deploy.tls_handshake.<domain>`), rate-limited helper drop logging, and `helper_drift` in `scenery deploy resume`. Focused tests in `cmd/scenery` and `internal/agent`.
* [x] 2026-08-12: Public Caddy sites now retain ACME as the primary issuer and add the internal CA as a redundant fallback. This restores the documented Cloudflare Full origin-certificate path when proxied HTTP-01 and TLS-ALPN-01 challenges cannot reach Caddy, without weakening direct-domain ACME issuance.
...
* During implementation, the public Caddyfile generator keeps `tls internal { on_demand }` scoped to the local HTTPS site and omits global `local_certs` when public ACME domains exist. This avoids the risk that global local certificates override public ACME issuance; Milestone 10 should still verify staging issuance against real Caddy.
* 2026-07-08: Cloudflare-proxied DNS changes the certificate proof shape. The public hostname presents Cloudflare's `clean.tech` wildcard cert to browsers, while the origin still has Scenery/Caddy's Let's Encrypt cert for `local.clean.tech`. Use direct-origin SNI probes (`--resolve`/`--connect-to`) plus Caddy storage/logs to verify the origin certificate.
* 2026-07-08: `scenery deploy status --json` can be globally `ready: true` while an unrelated enabled target carries a per-target diagnostic. For this pass, ONLV `local.clean.tech` is accepted; `micro.scenery.sh` remains a separate target cleanup/fix.
* 2026-07-08: The resume LaunchAgent plist can be installed even when `launchctl print gui/<uid>/dev.scenery.deploy-resume` says the service is not loaded in the current login session. Manual `scenery deploy resume --json` proves the command path, but a true reboot/login observation still requires an operator-driven reboot.
* 2026-07-14: `scenery deploy setup` failed on its first run with `launchctl bootstrap: exit status 5: Bootstrap failed: 5: Input/output error` and succeeded when rerun. `launchctl bootout` returns before launchd finishes tearing down the old service, so the immediate bootstrap of the replacement races the async teardown. Install now retries bootstrap/kickstart for up to 10 seconds (250ms interval) instead of surfacing the transient error.
* 2026-07-14: The Milestone 8 upgrade story failed in the field. The helper read target metadata through the strict durable-artifact loader (`LoadEdgeTargetState` → `machine.DecodeArtifact` with `DisallowUnknownFields` and exact spec-revision matching), so a running root helper on an older build rejected `~/.scenery/run/edge-target.json` after a newer scenery rewrote it, and `handleEdgeHelperConn` silently reset every connection — Cloudflare surfaced this as 525 while Caddy stayed healthy on `127.0.0.1:19443`. Worse, drift detection compared the *file's* schema revision to the *current binary* right after the current binary had rebound the file to its own identity, so the check was a tautology that could never fire, and readiness only tested TCP dialability. The strict loader would also have let a root helper rewrite the user-owned metadata file during spec-revision rebinding. Fixes: the helper now reads through a frozen tolerant read-only handoff reader, drift compares the plist-stamped `--helper-contract` against the binary's `EdgeHelperContractRevision`, and status requires an end-to-end SNI TLS handshake through port 443 (a TCP-accept-then-reset with Caddy answering TLS directly is reported as helper failure with the `scenery deploy setup` repair action).
...
* Decision: Public domain sites configure ACME first and Caddy's internal issuer second. ACME remains the preferred certificate for direct public origins; the internal issuer is the origin-certificate fallback for TLS-terminating proxies such as Cloudflare in Full mode.
  * Rationale: The same generated config supports direct and proxied deployments without provider-specific DNS detection. Caddy tries redundant issuers in order, while Cloudflare Full accepts the internal origin certificate and still presents its public certificate to browsers.
  * Date: 2026-08-12. Author: deployment follow-up.
...
* Decision: `deploy.root` resolves against top-level `frontends`, not any `proxy`-scoped frontend map.
  * Rationale: App-config `proxy` was removed with no compatibility path. Public deploy still needs a small way to name the frontend that owns `/`, and the singular current config surface is `frontends`.
...
* Decision: The helper handoff is a frozen tolerant contract, not a durable artifact. The helper reads target metadata only through `localagent.LoadEdgeHelperTarget` — plain JSON decode of the eight frozen payload fields, unknown fields and identity revisions ignored, file never rewritten — and installs stamp `--helper-contract <EdgeHelperContractRevision>` into the plist. Drift detection compares that stamp (missing stamp = pre-contract helper = actionable) against the current binary; the target metadata file is no longer consulted for drift. Readiness requires an end-to-end SNI TLS handshake through port 443; PID/listener presence alone never counts. Helper drops are logged rate-limited to the helper's launchd log.
  * Rationale: The 2026-07-14 outage proved strict current decoding is the wrong trust model for a root process that outlives the binaries writing its input: exact spec-revision matching turned every scenery upgrade into a potential silent public-edge outage, and the strict loader's rebind path would even let root rewrite user-owned state. The security boundary is unchanged — ownership, permissions, loopback-target, port-range, PID, executable, and process-start checks all remain — only the decoding tolerance changed. The stamp lives in the plist because plist and helper binary are replaced atomically together, so it always describes the running helper after a kickstart, which the metadata file never can.
  * Date: 2026-07-14. Author: outage follow-up implementation.
...
* Decision: Public deploy host lookup uses one lifecycle-published immutable in-memory snapshot. Agent startup restores and validates sessions, control-plane registration/deletion republishes session state, an `fsnotify` parent-directory watcher reloads atomic `deploy.json` replacements, and a bounded owner monitor invalidates exited or fingerprint-mismatched sessions. Public requests perform only the snapshot lookup.
  * Rationale: Deploy and process identity change at lifecycle frequency, not request frequency. This preserves exact owner fingerprint and PID-reuse protection while removing durable-state reads and process inspection from every document, asset, and API request. Backend connection failure remains the final fail-closed `503` boundary.
  * Date: 2026-09-02. Author: implementation follow-up.
...
* Decision: Public deploy restart has a dedicated readiness projection over the existing edge status: running Caddy, verified helper target PID/address, and matching live agent upstream. It deliberately excludes local wildcard DNS, while the ordinary edge command retains the full local-browser readiness contract. The one-shot resume LaunchAgent's completed nonzero exit is durable degraded state until a successful rerun.
  * Rationale: A public app must recover independently of an optional local developer convenience, and launchd accepting a kickstart is not evidence that the recovery command succeeded.
  * Date: 2026-07-22. Author: implementation follow-up.
...
The 2026-09-02 lifecycle snapshot follow-up removed both `deploy.json` loading and owner fingerprint inspection from the public HTTP path. Registration/restoration and the owner/deploy monitors retain the same fail-closed routing decisions, including `404` for unknown hosts and `503` for enabled targets without a validated running session.
