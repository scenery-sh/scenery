...
- [x] 2026-05-29: Made root integration synthetic apps persistent under the shared integration cache with content-hash ready markers. The temporal-secrets, service-init, and headless build tests now use stable app roots without changing the `scenery run` / `scenery build` behavior being asserted.
...
- [x] 2026-05-29: Revalidated the Temporal split with `go test -count=1 ./runtime ./temporal`, `go test -count=1 ./cmd/scenery ./internal/codegen ./runtime ./temporal`, and full `go test -count=1 ./...`. The full suite remains green but still above the strict no-cache target: the latest warmed run was 10.753s wall with root integration at 7.136s and `cmd/scenery` at 3.641s.
...
- [x] 2026-05-29: Moved pure `cmd/scenery` argument and harness knowledge-contract tests onto `t.Parallel()` and made the TypeScript Temporal contract check use explicit `--app-root` instead of process-global `chdir`/cache env. Revalidated with `go test -count=1 ./cmd/scenery`.
...
- [x] 2026-05-29: Continued no-scope-reduction tuning. The Temporal integration helper now starts the real dev server headless and without persistent SQLite state, isolated `cmd/scenery` tests run in parallel where they do not mutate process-global state, `scenery test` primes and reuses generated workspace graph state, and duplicate `parse.App` calls were removed from build-cache tests. Focused packages stayed green.
...
- [x] 2026-05-29: Re-ran the self-harness after updating the plan docs. It remained green with zero failed steps, zero drift errors, no schema-validation failures, and Go timing total 6.219s. The latest artifact shows the same remaining shape: root integration 5.229s, `cmd/scenery` 3.762s, and package scheduling/compile contention across build, parse, codegen, workers, devdash, runtime, and Temporal packages.
...
- 2026-05-29: The remaining target cannot be reached through small unit-test seams. A fresh full-suite timing pass still has root integration at 28.851s; the slow root tests are real `scenery run`, `scenery dev`, `scenery build`, Temporal, and reload lifecycles.
...
- 2026-05-29: Prewarming the shared Temporal dev server from `TestMain` looked attractive but made the dashboard/removed agent transport integration test an outlier by overlapping Temporal startup with dev reload work. The experiment was backed out because it worsened the real full-suite gate.
...
- Decision: Keep Temporal configuration and naming in `runtime`, but move SDK client/worker/cron mechanics to `temporal` behind registration hooks.
  Rationale: `runtime` is imported by many packages that do not need Temporal SDK symbols. The hook split preserves app startup behavior when generated apps blank-import `scenery.sh/temporal`, while cutting unnecessary SDK compile/test-binary cost from non-Temporal runtime dependents.
  Date/Author: 2026-05-29 / Codex.
...
- `scenery_integration_helpers_test.go` and `scenery_integration_test.go` for cache sharing, process fusion, and Temporal test replacement.
