...
The existing Grafana integration in `scenery dev` is directionally correct: it supervises a local Grafana process, writes provisioning under `.scenery/grafana`, pins Grafana and Victoria datasource plugin versions, exposes state through the dashboard and JSON surfaces, and keeps Grafana out of `scenery run`.

This plan hardens the trust edges that remain before Grafana should be treated as a first-class daily development surface. The target behavior is that `scenery dev` only advertises Grafana dashboard links when the Grafana server, expected plugins, expected datasources, and expected dashboards are actually usable. A plain `/api/health` response is not enough.
...
* Starter dashboards must query telemetry that scenery definitely emits, or the runtime must emit the metric contract those dashboards use.
...
* Do not add Grafana to `scenery run`.
...
* Do not add a broad compatibility layer for non-scenery naming or config.
...
* [x] 2026-05-26: Replace speculative dashboard queries by implementing the `scenery_request_duration_seconds` metric contract.
...
* `startGrafanaForDev` checks for an external Grafana on the port before writing provisioning. If a user already has Grafana on the default port, scenery can expose dashboard links even though that external instance probably does not have scenery datasources or dashboards.
...
* The starter dashboards appear to query metric and label names such as `scenery_request_duration`, `scenery_service`, `scenery_endpoint`, `scenery_is_error`, and `scenery_log_service`. The review only found those names in Grafana provisioning, not in telemetry producers.
* Existing trace query code already understands Jaeger/VictoriaTraces data with tags such as `scenery.service`, `scenery.endpoint`, and `scenery.is_error`. Trace-driven dashboards may therefore be more honest than metric-name-driven dashboards until a stable metric contract exists.
...
* First-run Grafana startup can block `scenery dev` for a long time because download and plugin preinstall are synchronous.

Record new findings here with commands, test output, or file references as implementation proceeds.

* 2026-05-26: The implementation already emitted a duration metric from `cmd/scenery/victoria_export.go`, but it used the dotted OTLP name `scenery.request.duration` and dotted trace-style attributes. The dashboards queried underscore-style Prometheus names. The hardening patch makes the emitted metric contract explicit as `scenery_request_duration_seconds` with underscore labels, then updates dashboard JSON generation to query those emitted names.
...
* Decision: Verified Grafana readiness means server health plus expected scenery assets.
...
* Decision: Default `auto` should avoid unverified external Grafana reuse.
  Rationale: A random Grafana already on the port is not equivalent to the managed scenery workbench. Reuse should be explicit or verified through Grafana HTTP APIs.
  Date/Author: 2026-05-26 / Codex
...
* Decision: Implement the metric contract instead of switching the first dashboards to trace-only.
  Rationale: scenery already exports a per-request duration metric to VictoriaMetrics. Renaming it to `scenery_request_duration_seconds` with underscore labels gives Grafana a concrete metric contract and keeps dashboard panels useful without inventing a parallel trace-only dashboard model.
  Date/Author: 2026-05-26 / Codex
...
* External Grafana reuse is verified-only. `/api/health` alone is treated as server health, not readiness; scenery verifies expected datasource and dashboard UIDs before reporting `external` or exposing links.
...
* Managed pinned Grafana is preferred over `PATH` Grafana. `PATH` fallback probes `grafana -v` and rejects mismatched versions unless the user explicitly chooses a binary with `SCENERY_GRAFANA_BIN`.
* Grafana downloads are checksum-verified using Grafana's `.sha256` sidecar file. Custom download URLs must provide `SCENERY_GRAFANA_DOWNLOAD_SHA256`.
* The Grafana child process filters inherited `GF_*` environment variables by default and sets only scenery-owned path/server variables, with `SCENERY_GRAFANA_PRESERVE_GF_ENV=1` for debugging.
* Datasource provisioning includes `prune: true`, `orgId: 1`, and `version: 1`.
* The emitted VictoriaMetrics contract is now `scenery_request_duration_seconds` with underscore labels, and the generated Grafana dashboards query those emitted names.
...
* Verified external Grafana: an already-running Grafana instance whose HTTP APIs confirm the expected scenery datasource UIDs and dashboard UIDs exist.
* Managed Grafana: the pinned Grafana binary installed under `.scenery/grafana` according to `internal/devtools/versions.json`.
...
Make the local proxy's Grafana URL available before Grafana provisioning is written. With `SCENERY_LOCAL_PROXY=1` and workspace `onlv`, generated `grafana.ini` should use `https://grafana.onlv.localhost/` as the public root URL when that is the advertised route.
...
```text
SCENERY_GRAFANA_BIN
.scenery/grafana/home/grafana-<pinned-version>/bin/grafana
...
Add a helper like `grafanaChildEnv(base []string, cfg grafanaConfig) []string`. By default, remove inherited environment variables whose keys start with `GF_`, then set only values scenery owns:
...
Add `SCENERY_GRAFANA_PRESERVE_GF_ENV=1` only for debugging. Tests should prove ambient `GF_SERVER_ROOT_URL`, `GF_PATHS_DATA`, and similar values do not override generated defaults unless this override is set.
...
* Option A: Implement and document an emitted metric contract, preferably with Prometheus duration naming such as `scenery_request_duration_seconds` and labels for app, service, endpoint, method, status, and error state.
* Option B: Make dashboards trace-first using data already available through VictoriaTraces and Jaeger-compatible tags such as `scenery.service`, `scenery.endpoint`, and `scenery.is_error`.
...
* Local proxy root URL test: with `SCENERY_LOCAL_PROXY=1` and workspace `onlv`, assert generated `grafana.ini` uses the proxied public URL.
...
* Optional live smoke behind `SCENERY_TEST_GRAFANA=1`, for example `go test ./cmd/scenery -run TestGrafanaLiveSmoke`, that starts real Grafana, waits for health, verifies expected UIDs through Grafana APIs, and shuts down.
...
Finish with validation and docs. Update `docs/grafana.md`, `docs/local-contract.md`, and any JSON/event schema documentation touched by state changes. Rebuild the CLI with `go install ./cmd/scenery` before finishing any repository change.
...
1. Inspect current Grafana implementation and tests:
   ...
   ```sh
   rg -n "Grafana|grafana|SCENERY_GRAFANA|GF_" cmd/scenery internal docs
...
6. Change default `auto` behavior so unverified external Grafana is not reused unless explicit reuse is requested. Add an environment flag such as `SCENERY_GRAFANA_REUSE_EXTERNAL=1` only if needed for users who intentionally manage Grafana themselves.
...
10. Add `grafanaChildEnv` and set `cmd.Env` for the Grafana process. Filter inherited `GF_*` variables by default. Add `SCENERY_GRAFANA_PRESERVE_GF_ENV=1` tests.
...
Minimum validation from `/Users/petrbrazdil/.codex/worktrees/fe3e/scenery`:
...
```sh
go test ./cmd/scenery -run 'Grafana|LocalProxy'
...
```sh
SCENERY_LOCAL_PROXY=1 SCENERY_DEV_GRAFANA=auto scenery dev
```
...
The implementation should remain safe to rerun. Generated Grafana state lives under `.scenery/grafana` and can be reset by deleting that directory. Tests should use temporary directories and fake HTTP servers rather than modifying a developer's real Grafana state.
...
1. Do not reuse external Grafana unless scenery assets are verified.
2. Fix the local proxy `root_url` mismatch.
3. Make dashboards query telemetry definitely emitted by scenery, or make first dashboards trace-driven.
...
```text
scenery grafana status
scenery grafana open
scenery grafana doctor
...
```make
@dev-scenery:
  SCENERY_LOCAL_PROXY=1 SCENERY_DEV_GRAFANA=auto {{scenery}} dev --app-root={{app_root}}

@grafana-required:
  SCENERY_LOCAL_PROXY=1 SCENERY_DEV_GRAFANA=1 {{scenery}} dev --app-root={{app_root}}

@grafana-reset:
  rm -rf {{app_root}}/.scenery/grafana
```

Before adding those ONLV changes, verify that its `scenery` Just variable points at the checkout containing the Grafana integration.
...
SCENERY_GRAFANA_PORT=<port>
SCENERY_GRAFANA_DIR=<path>
SCENERY_GRAFANA_REUSE_EXTERNAL=1
SCENERY_GRAFANA_PRESERVE_GF_ENV=1
SCENERY_LOCAL_PROXY=1|0
```
...
* Datasource lookup by UID for `scenery-victoriametrics`, `scenery-victorialogs`, and optional traces datasource.
* Dashboard lookup by UID for provisioned scenery dashboards.
