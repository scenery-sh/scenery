...
* [x] 2026-05-27: Make HTTPS the default for newly started agents, with `--router-http` and `ONLAVA_AGENT_ROUTER_TLS=0` as explicit HTTP opt-outs.
...
* Decision: Make agent TLS an explicit router mode first, then flip newly started agents to HTTPS by default after ONLV uses agent-routed URLs everywhere.
  Rationale: Existing local workflows and tests used the high-port HTTP agent router. Staging the change kept the first TLS implementation small; after API, frontend, Electric, Grafana, and Temporal routes became agent-owned, keeping HTTP as the default would preserve the wrong end state. `--router-http` and `ONLAVA_AGENT_ROUTER_TLS=0` remain available for local debugging.
...
* Added agent router TLS mode via `onlava agent --router-tls` and `ONLAVA_AGENT_ROUTER_TLS=1`, then made TLS the default for newly started agents.
* Added `onlava agent --trust` and `ONLAVA_AGENT_TRUST=1` to attempt local CA trust installation while keeping router startup tolerant of trust-install failures.
* Added router scheme tracking in agent state/health and session route generation so default agents emit `https://...onlava.localhost` routes.
* Added SNI-based on-demand TLS certificates for agent-routed `*.onlava.localhost` hosts, including two-label session hosts such as `api.<session>.onlava.localhost`.
* Updated CLI usage, README, local contract docs, and tests.

Validation:

```sh
go test ./internal/localproxy ./internal/agent ./cmd/onlava
go test ./...
...
Milestone 2 adds an explicit TLS mode to `onlava agent` and `localagent.RunOptions`.
...
2. Add `ONLAVA_AGENT_ROUTER_TLS=1`, `ONLAVA_AGENT_TRUST=1`, `onlava agent --router-tls`, and `onlava agent --trust`.
...
4. Use `tls.Config.GetCertificate` in the agent router to issue and cache per-SNI certificates for `*.onlava.localhost` and `*.*.onlava.localhost` style routed hosts.
...
* `onlava agent --router-tls --router-listen 127.0.0.1:0` serves the routed HTTP handler over TLS.
* Sessions registered with a TLS agent receive `https://...onlava.localhost` route URLs.
* The agent can generate a valid leaf certificate for `api.<session>.onlava.localhost`.
* `onlava agent --trust` attempts to install the existing onlava local CA.
...
```text
onlava agent --router-tls [--trust]
ONLAVA_AGENT_ROUTER_TLS=1
ONLAVA_AGENT_TRUST=1
```
