...
branch compute endpoint, run DB lifecycle work against that branch, and keep
the parent branch protected.

Earlier work established the local contract, branch pins, lease registry,
generated dev-cell scaffolding, driver delegation, and cleanup safety. This
plan narrows the remaining work to the built-in `neon-selfhost-driver`, real
...
- [x] 2026-06-09: Updated `README.md`, `SKILL.md`, `docs/agent-guide.md`, and `docs/local-contract.md` to mark real selfhost branch compute creation and default harness coverage as implemented while keeping release-grade driver distribution experimental.
...
- [x] 2026-06-09: Closed the remaining branch isolation proof in the real Neon self-harness. The harness now checks that parallel branch worktrees resolve distinct Neon branch `DatabaseURL`s, replication slot names, and Postgres application names.
...
- 2026-06-09: A recorded reachable branch compute is enough for the driver to prove and return a ready endpoint, which gives `scenery up`, `db psql`, and DB lifecycle the same redacted endpoint contract before the driver can create computes itself.
...
- 2026-06-09: Ready selfhost endpoints require a password-bearing DSN for non-interactive `psql` and application consumption. Scenery keeps public endpoint metadata redacted, but synthesizes `postgres://cloud_admin:***@...` in managed `DatabaseURL` values for the `neon-selfhost-driver` source.
...
- `docs/local-contract.md` for CLI grammar, JSON schemas, generated state paths, driver env vars, and current Neon behavior.
- `README.md`, `docs/agent-guide.md`, `SKILL.md`, and `docs/local-contract.md` for the current done-vs-experimental status of built-in selfhost branch creation, opt-in harness coverage, and driver distribution.
...
- `cmd/scenery/dev_services.go`, `cmd/scenery/db_setup.go`, `cmd/scenery/db_seed.go`, and `cmd/scenery/psql.go` for runtime consumption of ready branch endpoints.
...
4. Branch Ensure and Runtime Consumption: driver `ensure` creates branch timelines and compute endpoints, returns ready endpoint metadata, and lets `scenery up`, DB lifecycle, and `db psql` consume non-parent ready branches.
...
7. Harness Promotion: default, race, and release self-harness modes prove real Docker Neon branch isolation and database isolation; `--quick` keeps fake-driver coverage for the smaller non-Docker loop.
...
managed branch URL resolution and distinct database identity,
...
- `scenery up`, DB apply/seed/setup, and `scenery db psql` consume the branch endpoint and refuse protected parent branches.
- Two worktrees can run against separate Neon branches without database collision.
