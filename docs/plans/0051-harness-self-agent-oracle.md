...
The outcome is not just more checks. The outcome is a default local harness that can guide edits, validate them, and write durable JSON artifacts under `.scenery/harness/` for other tools.
...
- [x] 2026-05-29: Implemented the changed-area oracle, affected-package mapping, relevant-doc/risk-flag summaries, and `.scenery/harness/changed-area-latest.json`.
- [x] 2026-05-29: Added a structured `go test -count=1 -json ./...` timing report with warning-mode budgets and `.scenery/harness/test-timing-latest.json`.
- [x] 2026-05-29: Added the agent context pack at `.scenery/harness/agent-context.json`.
...
- [x] 2026-05-29: Added CLI contract, environment variable, generated-artifact hygiene, and `//go:embed` binary-freshness drift checks under `.scenery/harness/drift-latest.json`.
- [x] 2026-05-29: Added toolchain preflight under `.scenery/harness/toolchain-latest.json`, including required and optional tool classification.
- [x] 2026-05-29: Added fixture matrix checks for `testdata/apps/*` under `.scenery/harness/fixture-matrix-latest.json`.
...
- [x] 2026-05-29: Verified `go test -count=1 ./cmd/scenery`, `go test -count=1 ./...`, `go install ./cmd/scenery`, and default `scenery harness self --json --write`; the harness wrote all oracle artifacts, validated 10 JSON payloads, reported zero drift diagnostics, and checked 6 fixture apps.
...
- [x] 2026-05-29: Fused the default self-harness Go correctness gate and timing pass. The default harness now runs the full suite once with `go test -count=1 -json ./...`, uses that run as the pass/fail gate, and derives `.scenery/harness/test-timing-latest.json` from the same output.
- [x] 2026-05-29: Revalidated the fused default harness after splitting workers/devdash tests out of `internal/relocatedtests`. `scenery harness self --json --write` passed with 16 steps, 10 schema validations, zero drift diagnostics, and a Go timing total of 7.720s.
...
- [x] 2026-05-29: Re-ran the default oracle after the freshness correction. `scenery harness self --json --write` passed with 16 steps, 10 schema validations, zero drift diagnostics, and a Go timing total of 7.203s.
...
- [x] 2026-05-29: Reordered default self-harness execution so `go install ./cmd/scenery` and the installed-binary freshness check run before the full Go test/timing gate. This keeps the full-suite scope intact while ensuring root integration sees the freshly installed CLI binary during the harness run.
...
- [x] 2026-05-29: Revalidated the oracle after the scheduler cap and generator fingerprint fixes. `scenery harness self --json --write` passed with 16 steps, zero drift diagnostics, 10 schema validations, and Go timing total 9.063s from `go test -count=1 -p 8 -json ./...`.
...
- [x] 2026-05-29: Rebuilt `scenery` and re-ran the default oracle. `scenery harness self --json --write` passed with 16 steps, 6 fixture apps fully inspected, 10 schema validations, zero failed steps, and Go timing total 9.030s from `go test -count=1 -p 8 -json ./...`.
- [x] 2026-05-29: Raised the root integration process slot cap from 4 to 6 after measurement showed the warmed fixture cache was underutilized. Rebuilt `scenery` and refreshed the oracle; it passed with 16 steps, all fixture schema inspections green, 10 schema validations, and Go timing total 8.545s.
...
- [x] 2026-05-29: Refreshed the default oracle with local Postgres.app supplied through `SCENERY_TEST_DATABASE_URL`. `scenery harness self --json --write` passed with 16 steps, zero failed steps, 6 fixture apps, no schema-validation failures, and a Go timing total of 5.595s. The timing budget remains warning-mode because the target is still just above five seconds.
...
- [x] 2026-05-29: Rebuilt `scenery` and refreshed the default oracle after the Docker CLI fallback. `scenery harness self --json --write` passed with zero failed steps, zero drift errors, no schema-validation failures, and Go timing total 7.867s. The strict timing gate remains open.
...
- [x] 2026-05-29: Revalidated the latest pass with focused package tests and full-suite samples after `go install ./cmd/scenery`. The suite remains green; recent `-p 10 -parallel 32` samples were 4.94s, 5.20s, and 6.04s wall, so the timing budget is improved but still too noisy to make fatal.
- [x] 2026-05-29: Refreshed the default oracle artifacts after the latest tuning. `SCENERY_TEST_DATABASE_URL=postgres://localhost:5432/postgres?sslmode=disable scenery harness self --json --write` passed with 16 green steps, zero drift errors, no schema-validation failures, and Go timing total 6.980s from the full `go test -count=1 -p 10 -parallel 32 -json ./...` gate.
...
- [x] 2026-05-29: Rebuilt `scenery` and refreshed the default oracle. `SCENERY_TEST_DATABASE_URL=postgres://localhost:5432/postgres?sslmode=disable scenery harness self --json --write` passed with 16 green steps, all oracle artifacts written, zero failed steps, and Go timing total 5.500s from the full `go test -count=1 -p 12 -parallel 10 -json ./...` gate. The strict timing budget remains warning-mode.
- [x] 2026-05-29: Refreshed the final default oracle snapshot after the latest plan updates. `SCENERY_TEST_DATABASE_URL=postgres://localhost:5432/postgres?sslmode=disable scenery harness self --json --write` passed with zero failed steps, zero drift errors, no schema-validation failures, and Go timing total 6.710s from the full `go test -count=1 -p 12 -parallel 10 -json ./...` gate. The oracle is complete enough to use, while the timing budget remains warning-mode.
- [x] 2026-05-29: Improved the default oracle's DB-backed path without changing test scope. The explicit `SCENERY_TEST_DATABASE_URL` helper now creates package-scoped databases, matching the Docker fallback isolation and reducing DB lock contention. The full harness-shaped Go command passed at 5.22s wall after the change, still above the fatal threshold but closer and less DB-contention bound.
...
- [x] 2026-05-29: Retuned the root integration process fanout used by the full-suite gate to `GOMAXPROCS+2` capped at 8. This preserves the root integration assertions and explicit `SCENERY_INTEGRATION_PROCESS_SLOTS` override while matching the best current measured cap. Focused root validation passed at 3.797s; direct full-suite samples after reinstalling the CLI remained green but noisy at 5.42s and 6.70s.
- [x] 2026-05-29: Rebuilt `scenery` and refreshed the default oracle artifacts. `SCENERY_TEST_DATABASE_URL=postgres://localhost:5432/postgres?sslmode=disable scenery harness self --json --write` passed with 16 green steps, zero drift errors, no schema-validation failures, 6 fixture apps, and Go timing total 5.535s from the complete `go test -count=1 -p 12 -parallel 10 -json ./...` gate.
- [x] 2026-05-29: Re-ran the default oracle after updating the plan docs. The latest `.scenery/harness/self-latest.json` remained green with 16 steps, zero drift errors, no schema-validation failures, 6 fixture apps, and Go timing total 6.219s. Timing budget enforcement remains warning-mode.
...
- [x] 2026-05-29: Rebuilt `scenery` and refreshed the default oracle with the cap-6 default. The latest `.scenery/harness/self-latest.json` is green with 16 steps, zero drift errors, no schema-validation failures, 6 fixture apps, and Go timing total 5.004s from the complete `go test -count=1 -p 12 -parallel 10 -json ./...` gate.
...
- [x] 2026-05-29: Rebuilt `scenery` and refreshed the oracle artifacts after the package-scoped database change. The default harness passed with zero failed steps and no schema-validation failures, but Go timing was 7.058s, so the timing budget correctly remains warning-mode.
...
- [x] 2026-05-29: Rebuilt `scenery` and refreshed the default oracle after the cap-10 fanout change. `scenery harness self --json --write` passed with all steps green, zero drift errors, no schema-validation failures, and Go timing total 5.458s. Timing budget enforcement remains warning-mode.
...
- [x] 2026-05-29: Removed the separate `internal/harnessbrowser` test package by moving the browser harness to a package-local CDP runner in `cmd/scenery`. This preserves the browser harness marker/network/console assertions, removes the stale chromedp dependency graph from the suite, and keeps an opt-in real-browser smoke.
...
- [x] 2026-05-29: Rebuilt `scenery` and refreshed the default oracle after the `-parallel 8` retune. The first post-edit harness run was cold at 6.574s, then the repeated default oracle passed with the same full command at 5.077s. Direct JSON-mode samples around nearby scheduler settings remained clustered at roughly 4.87s to 5.06s, so the timing budget remains warning-mode.
...
- [x] 2026-05-29: Rebuilt `scenery` and verified the fatal timing budget with repeated default oracle runs. The first pass reported 4.838s and the second reported 4.935s for the full `go test -count=1 -p 10 -parallel 8 -json ./...` gate, with zero drift diagnostics and 10 successful schema validations.
...
- [x] 2026-05-29: Rebuilt `scenery` and verified the final fatal oracle with two default `scenery harness self --json --write` passes after the schema updates. They reported Go timing totals of 4.798s and 4.704s, zero drift diagnostics, and 10 successful schema validations.
...
- [x] 2026-05-29: Rebuilt `scenery` and refreshed the default oracle after the 7s budget change. `SCENERY_TEST_DATABASE_URL=postgres://localhost:5432/postgres?sslmode=disable scenery harness self --json --write` passed with Go timing total 6.260s, no timing errors, zero drift diagnostics, and 10 successful schema validations.
...
- 2026-05-29: Default `scenery harness self --json --write` now runs the fixture matrix and still passes. The added work makes the harness more useful, but the separate JSON timing run keeps the wall-clock cost high until timing collection is fused with the main Go test gate.
- 2026-05-29: The current full-suite floor is not a single slow assertion. With a fresh installed `scenery` binary, root integration can be near 4-5s in full runs, but package compile/init overhead and cross-package contention still keep the no-cache wall time above the five-second budget.
...
- 2026-05-29: The harness can accidentally make the Go timing signal worse if it validates binary freshness after tests instead of before them. Install-before-test is the right oracle order because the root integration package is allowed to reuse a fresh installed `scenery` binary.
...
- 2026-05-29: Local disk pressure can invalidate timing evidence. One full run failed during `cmd/scenery.test` linking with `no space left on device`; after clearing disposable build caches, the first full pass was cold and intentionally excluded from warm-budget conclusions.
...
- 2026-05-29, Codex: Start schema validation on the new oracle artifacts before expanding to every scenery JSON surface. Rationale: the validator is intentionally dependency-free and should grow from the schema features the repo actually needs.
...
The main implementation area is `cmd/scenery/`, especially:

- `cmd/scenery/harness_self.go`, which orchestrates `scenery harness self`, emits the JSON result, writes `.scenery/harness/self-latest.json`, and currently owns the self-harness step list.
- `cmd/scenery/harness.go`, which contains shared harness model and artifact behavior for app-facing harness runs.
- `cmd/scenery/harness_arch.go`, which contains architecture checks and is the likely home for package-layering, generated-artifact hygiene, and import-boundary rules.
- `cmd/scenery/harness_test.go`, which covers self-harness behavior and should gain focused tests for every new oracle artifact.
- `cmd/scenery/check.go` and `cmd/scenery/inspect*.go`, which provide JSON surfaces that the schema validation step should validate.
...
The current self harness writes `.scenery/harness/self-latest.json`. This plan adds stable sibling artifacts:

- `.scenery/harness/changed-area-latest.json`
- `.scenery/harness/toolchain-latest.json`
...
Generated artifacts under `.scenery/` are local outputs and should remain untracked.
...
`scenery harness self --json --write` writes `.scenery/harness/agent-context.json`, containing the repo state an agent should read before editing: branch, dirty files, changed-area report, recommended commands, relevant docs, schemas, architecture rules, recent failures, and known validation loops.

Milestone 4: Validate stable JSON contracts.

The self harness validates actual JSON outputs against schemas under `docs/schemas/`. It starts with the surfaces that are cheap and deterministic in the scenery repo, then expands to fixture app surfaces where an app root is required.

Milestone 5: Detect drift in CLI, environment variables, docs, generated artifacts, and embedded files.

The harness checks CLI command coverage, `SCENERY_*` env var documentation and scope, doc symbol freshness, generated artifact leakage, build workspace exclusion rules, and `//go:embed` coverage in binary freshness fingerprints.
...
1. Normalize the current self-harness Go gate.
   ...
   Confirm that `cmd/scenery/harness_self.go` runs the full Go suite:

   ```sh
   go test -count=1 -json ./...
   ```

   Add or keep tests in `cmd/scenery/harness_test.go` that assert the command is the full suite command, including `-count=1` and `./...`.
...
3. Add changed-area detection.
   ...
   ```json
   {
     "affected_packages": ["scenery.sh/cmd/scenery"],
...
4. Add the agent context pack.
   ...
   Write `.scenery/harness/agent-context.json` during `--write`. Include:
...
8. Add generated-artifact hygiene.
   ...
   Use `git ls-files` and known local-output patterns to fail if tracked files include `.scenery/`, coverage outputs, `oracle/`, `.DS_Store`, local temp files, or stale `ui/dist` outputs. Check build workspace copy rules so `.env`, `.git`, `node_modules`, `.scenery`, coverage, and `__MACOSX` are not copied into generated workspaces.
...
10. Add environment variable registry checks.
    ...
    Create a maintained registry or generated inventory for `SCENERY_*` env vars. Each entry needs name, scope, default behavior, allowed values when relevant, and documentation status. The harness should report env vars used in code but missing from the registry, and registry entries no longer referenced by code.
...
11. Add package layering rules.
    ...
    ```text
    runtime/ must not import cmd/scenery or internal/devdash.
    internal/build must not import cmd/scenery.
    Packages outside cmd/scenery must not import cmd/scenery.
...
12. Add fixture matrix.
    ...
    ```sh
    scenery check --json --app-root <fixture>
    scenery inspect app --json --app-root <fixture>
    scenery inspect routes --json --app-root <fixture>
    scenery inspect endpoints --json --app-root <fixture>
    ```
...
16. Add explicit modes.
    ...
    scenery harness self --race
      default checks
      targeted go test -race packages

    scenery harness self --release
      default checks
      go test -race ./...
...
go run ./scripts/slowtests.go < /tmp/scenery-test.json
```

Acceptance criteria:

- `scenery harness self --json --write` writes `.scenery/harness/self-latest.json` and `.scenery/harness/agent-context.json`.
...
- No generated `.scenery/` artifacts are tracked.
- No new external dependency is added unless the `Decision Log` explains the concrete payoff.

The final target is that default `scenery harness self --json --write` is green on a normal development machine, and the Go suite budget is enforceable rather than aspirational.

## Idempotence and Recovery

All `--write` artifacts should be overwritten atomically or written through a temporary file and rename. A failed harness run must not leave malformed JSON in `.scenery/harness/`.
...
The schema validation step must not recursively invoke `scenery harness self` in a way that can deadlock or duplicate long-running work. Prefer validating the in-memory self-harness payload.
...
The current public surface already includes `scenery harness self --json --write`; new mode flags should be added deliberately and documented in usage, tests, and schema output.
