...
- [0201 Agent-Readable Failures And CLI Grammar](0201-agent-readable-failures-and-cli-grammar.md) — 2026-09-19: usage errors are invalid requests that name the mistake instead of `SCN9000` (164 of 182 malformed invocations had been internal); every `report_token` the CLI mints, including one from a detached `scenery up`, resolves locally with `scenery inspect report`; `--probe cli-grammar` derives 273 refusals and 42 acceptances from `scenery help -o json`. Later: an application process also logs the cause of a token it mints and reports it to its development session, so those tokens resolve too (`origin` `runtime`).
- [0181 Minimal Native Reload Artifact](0181-minimal-native-reload-artifact.md) — 2026-09-18: closed as a NO-GO experiment; the corrected real ONLV implementation island replaced natively at 934 ms p50 across 238 packages, its conditional stable-host integration was never started, and the edit latency target continues in plan 0200.
- [0180 Incremental Worktree-Native Development Loop](0180-incremental-worktree-native-loop.md) — 2026-09-18: closed with every slice implemented — attributable builds, a root SDK closure reduced from 315 to 212 packages, workspace reads bound to captured bytes, and one immutable generated composition shared across matching worktrees — and the latency target unmet at 1,534 ms p50 against 300 ms; the target continues in plan 0200.
- [0179 Halve the Verified Development Loop](0179-half-latency-development-loop.md) — 2026-09-18: superseded by plan 0200 with both goals unmet (semantic edit 5,461 ms against 3,153 ms; unchanged start 4,032 ms against 4,017 ms); its retained increments shipped, its open items are not carried, and no active plan holds an unchanged-start target.
- [0199 Unloadable Island Reload](0199-unloadable-island-reload.md) — 2026-09-15: screened unloadable reload boundaries on the real ONLV AHJ island; wazero WebAssembly released memory but needed about 4.6 s per edit, Go `c-shared` `dlclose` kept each runtime heap, and a process island released everything with a 262–278 ms direct build but a first-execution floor that misses the 250 ms gate; no Stage 2 benchmark or product code was added.
- [0198 Development Link Latency](0198-development-link-latency.md) — 2026-09-15: made the direct executor's import configuration rewrite linear, cleared inherited Darwin background scheduling before development builds with `process.scheduling` evidence, and linked development executables without DWARF (`-w`, restorable with `-ldflags=-w=false`); isolated full-ONLV link fell from 0.78–0.82 s to 0.50–0.55 s, while the loaded short benchmark still showed a roughly 2 s link phase from its environment.
- [0197 Retained Build Executor Decision](0197-retained-build-executor-decision.md) — 2026-09-15: kept retained direct compiler/linker execution as the normal development path with no stock fallback; reused retained graph and state metadata, closed embed and refresh races, reduced redundant preparation, and recorded the corrected five-path macOS comparison with stable names and phase evidence.
- [0196 Correct Retained Go Compiler](0196-correct-retained-go-compiler.md) — 2026-09-15: corrected state advancement, archive aliases, complete input rebinding, cache-aware graph refresh, resident ownership and benchmark attribution; runtime proof passed, while the short full-ONLV control showed retained validation plus stock build faster than direct compile/link.
