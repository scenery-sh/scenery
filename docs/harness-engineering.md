...
| `process-model` | `testdata/apps/multiservice` through `scenery up` with `SCENERY_DEV_PROCESS_MODEL=service`: three distinct processes; a `greet` request pinned to generation 1 completing against the first `greeter` and `echo` after `greeter` and then `echo` were replaced (generation 2 retires without stopping the first `echo`), with drain before activation on replacement; retirement of the first generation's instances; a failed build and identical restored source keeping the published generation; a shared package edit replacing both services in one generation; a contract-changing generation whose `echo` constructor fails keeping the host and services serving; process/socket/link cleanup; and the tagged `TestRetainedRecordingCrossProcess` journey, which closes the owner of a real recipe recording while a tool started through its recorder is blocked and requires every recording process to have ended before `Close` returns |
