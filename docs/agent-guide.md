...
- Development compilation uses a retained stock-Go recipe by default. The first build records a complete captured stock bootstrap. Compatible body edits validate against the last successful current state, compile only newly changed packages and transitive consumers, relink, and atomically advance retained snapshots and archives without repeating `cmd/go` package loading. Package/file/import/directive/module changes first run an ordinary warm-cache stock build and merge the newly observed graph; if that build cannot capture every required action, Scenery performs a complete bootstrap. Configuration, tool or retained-state incompatibility also bootstraps. Direct execution retains and rebinds captured compiler/linker file inputs such as import, embed and symbol-ABI configuration, and rejects a rebuild frontier requiring unmodeled native actions before invoking tools. The recipe lives in the private development cache, is held resident by the supervisor, and is bound to the exact workspace and Scenery executable. Deleting it is safe and makes the next build bootstrap again. Ephemeral and deployable builds remain ordinary stock `go build`; no app setting or environment variable selects this policy.
- Shared executable reuse remains an independent, conservative stock-path optimization for builds outside the retained development policy. A build consuming framework source, replacements or module dependencies outside its locked private workspace never restores or publishes a shared executable. Go's package cache and the link concurrency budget remain active. Before compilation, Scenery copies each authored non-framework local replacement into an app-owned content generation, rewrites the build workspace to that root, and checks the live origin again before activation. Thus Go cannot consume transient replacement bytes under another captured identity; timestamps are not equivalence. Module-cache and custom tool inputs remain outside this snapshot claim.
