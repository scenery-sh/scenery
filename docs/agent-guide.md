...
- Use `scenery up` for the app root's one live development runtime and all safe local capabilities. It runs the process model: one host process plus one process per native service, so an edit replaces only the services whose Go closure changed. Production builds still produce one application executable.
- Prepare its selected framework with `scenery framework use -o json` and use the reported local executable. Ordinary app development pins `scenery.sh` in `go.mod`; explicit co-development uses `framework use --source <checkout>` to freeze that checkout's current source bytes and rewrite only the local Scenery replacement. `framework inspect -o json` verifies selection bytes, not HTTP health. The session rejects producer/source disagreement before replacement; changing the original source checkout does not update a selected snapshot. A framework update is a deliberate `go.mod` selection: a pinned version bump, or `framework use --source`. A running `scenery up` started by the prepared executable then hands itself off before its next build (the same PID keeps the foreground terminal; a detached runtime relaunches detached), and a startup through a stale executable hands off before acquiring anything. A local replacement pointing at a mutable checkout still needs an explicit `framework use` and restart.
...
- `scenery worker` builds the `development` go_target once and starts a worker-role runtime for declared durable executions and schedules.
...
- `scenery prune --older-than <duration>` removes only eligible stale session records and their matching substrate leases by default; filesystem state and managed databases require explicit `--state`, `--db`, or `--all`, and disposable build state (development-cache workspaces, and with `--app-root` unselected framework snapshots and producers) requires explicit `--build-cache`. `scenery system agent cleanup` is the explicit pre-rebrand sweep: it reports processes using the exact legacy managed paths and signals none; legacy state removal additionally requires `--remove-state` and no reported process.
