...
- Use `scenery up` for the app root's one live development runtime and all safe local capabilities. It runs the process model by default: one host process plus one process per native service, so an edit replaces only the services whose Go closure changed. The single application model (`SCENERY_DEV_PROCESS_MODEL=application`) is deprecated; use it only for an application the process model reports it cannot run yet (event consumers or emissions, or no native service).
...
- Development compilation uses the retained direct compiler/linker executor. Stock `go build` is neither the development default nor a product fallback. The first build uses `cmd/go` to capture its complete selected tool graph. Compatible body edits validate against the last successful state, reuse the retained package graph for `BuildInput`, compile only changed packages and transitive consumers, relink, and atomically advance retained snapshots and archives without repeating package loading. A directory-membership, import, directive, module, configuration or tool change uses `cmd/go` to reconcile the graph; incomplete observation requires a new complete capture. Direct execution retains and rebinds compiler/linker file inputs such as import, embed and symbol-ABI configuration, validates refresh stamps before commit, and rejects unmodeled native actions before invoking tools. The recipe lives in the private development cache, is held resident by the supervisor, and is bound to the exact workspace and Scenery executable. Deleting it is safe and causes a new capture. Ephemeral and deployable artifacts retain their existing stock path. No app setting or environment variable selects development policy; stock development execution is available only in a repository benchmark binary built with `scenery_benchmark_stock`. Development executables omit DWARF (`-ldflags=-w`), so stack traces and profiles work but a debugger cannot inspect variables; add `-ldflags=-w=false` to `build.go_flags` while debugging. On macOS the supervisor removes an inherited Darwin background policy before each build and reports it as the `process.scheduling` build step; a launcher's QoS clamp still slows compilation and linking and is not reported.
- Shared executable reuse remains an independent, conservative optimization for builds outside retained development policy. A build consuming framework source, replacements or module dependencies outside its locked private workspace never restores or publishes a shared executable. Go's package cache and the link concurrency budget remain active. Before compilation, Scenery copies each authored non-framework local replacement into an app-owned content generation, rewrites the build workspace to that root, and checks the live origin again before activation. Thus Go cannot consume transient replacement bytes under another captured identity; timestamps are not equivalence. Module-cache and custom tool inputs remain outside this snapshot claim.
...
Verify validates every payload checksum without discovering or stopping a target app and returns the archive's lowercase hexadecimal `sha256` in JSON. Load holds the same opened validated archive through apply. Verify and `--dry-run` do not allocate state, extract a source cache or recover operations. A complete new storage generation receives independent clone-or-copy payloads, original logical metadata and fresh ETags. Storage-only merge stages the effective result; combined database/storage merge is rejected. Restore preparation and database uncertainty retain one digest/mode/conflict-policy-bound operation record; normal access and SQL-only load stay blocked until the matching resume completes, including after the storage generation has switched. Follow its reported instruction, never remove the record manually or retry with fewer data classes. Use `scripts/snapshot-backup.sh` from the host scheduler for verified retention and optional rclone replication; Scenery does not install or own that schedule or stop applications.
...
For coarse product presentation, declare opaque `application_key` and `access_key` values on generated page macros and, when needed, workspace tabs. Load the application's typed access snapshot in the auth gate, then pass one synchronous `resolveAccess` function to `createSceneryApp`; it controls navigation, direct-route invocation, and workspace tabs from the same generated route catalog. When that snapshot loads or changes outside TanStack Router state, provide `useNavigationRevision` as a hook that subscribes the generated shell to the app-owned snapshot so navigation and its toggle recompute immediately. Use the returned `routes` and `matchSceneryRoute` for app switchers or command menus instead of classifying URL prefixes. A workspace tab inherits only its workspace's application key. Treat this as UI gating only: backend bindings still require application-owned authorization.
