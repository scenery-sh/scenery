...
| Inspect managed SQLite services | `scenery db list --json` |
| Open a managed SQLite shell | `scenery db shell` |
| Inspect SQLite branch pin | `scenery db branch status --json` |
| Pin this worktree to a SQLite branch | `scenery db branch checkout <name> --json` |
| Inspect local SQLite branch leases | `scenery db branch list --json` |
| Create a code worktree with a SQLite branch pin | `scenery worktree create <name> --json` |
...
sync/TanStack/layout-kit wiring.
...
When local dev fails because the host may be missing Go, disk space, memory, Docker engine readiness, or optional tools, run `scenery doctor --json` first. For managed storage apps, doctor reports the pinned `zerofs` toolchain artifact and warns when downloads are disabled before the artifact is installed; `scenery up` also preflights required managed ZeroFS before dev-session route registration and fails with the `scenery system toolchain sync --tool zerofs` repair path when the artifact is unavailable. Managed ZeroFS startup uses a bounded two-minute readiness window and reports the last probe, process output tail, config/log paths, sockets, Web UI address, PID, source, cell ID, and `scenery.dev.failure.v1` evidence artifact path on failure. Preflight artifacts live under `<app-root>/.scenery/evidence/`; session-backed runtime artifacts live under the session state root's `artifacts/` directory. Stay on scenery command surfaces for ordinary app work. Use `scenery help --json` for machine-readable command discovery, `scenery help all` for the grouped human command reference, and `scenery ps --json` when dev runtime status will feed another tool. Use `scenery upgrade --json` to update a prebuilt local Scenery binary from a verified release archive; the upgraded binary then syncs managed toolchain entries already present locally, while `--toolchain all` pulls every frozen tool/image from the upgraded manifest. Inspect managed dnsmasq, Caddy, Grafana, Victoria, Temporal CLI, sync, or ZeroFS details only when intentionally debugging the substrate. Shared substrate failures are visible in `scenery ps --json` under `substrates`, including exit metadata and stdout/stderr log paths; dead registered runtime children such as managed frontend processes appear as session `degraded` status with `status_reason`. Managed SQLite database URL/path values are exposed through the dev runtime environment. SQLite branch leases live under the agent SQLite state root, and `.scenery/worktree-db.json` pins the current app root or worktree to a branch database. sync remains scoped to the dev runtime and is published as an internal backend, not a global substrate. Do not install global binaries as a hidden fix; use `scenery system edge dns install` for wildcard local DNS, `scenery system edge install` for Caddy, `scenery system toolchain sync --json` for managed app-root tools, or document the configured external service.
...
- Use `scenery worker temporal prune --stale --json` to inspect stale open workflows on the current dev session's Temporal task queues after `scenery up` reports a Temporal stale-workflow warning. Add `--yes` only when intentional termination of those stale workflows is desired; Scenery never prunes Temporal workflows by default.
...
- Use `scenery inspect storage --json` and `scenery storage status|webui|ls|stat|put|get|rm|cleanup --json` for Scenery-owned storage capability work. Scenery-launched app processes, workers, setup commands, and app-local code tasks receive the storage capability env consumed by `scenery.sh/storage`, so app code should call `storage.Default(ctx)` or `storage.Named(ctx, name)`. In production/headless runtimes, that env must be an explicit operator-provided proxy config; local-root storage configs are rejected. In agent-backed dev sessions the app-facing proxy store talks through Scenery's session-local storage proxy to the managed ZeroFS 9P Unix socket. Browser code can use generated TypeScript `client.storage` helpers or the reserved `/__scenery/storage/<store>/...` routes for configured `auth` stores; `private` stores stay internal-only on the external HTTP surface and are available only to app/runtime helpers or Scenery's private route table. Tenant-scoped stores require standard-auth tenant context or explicit `storage.WithTenantID(ctx, tenantID)`. `PutOptions.ContentType` and `PutOptions.Metadata` are returned by `Head`, `Get`, and `List`, and browser/proxy routes carry metadata through `X-Scenery-Storage-Meta-*` headers. When an agent-managed ZeroFS substrate is attached, inspect/status includes `storage.runtime` with route, protected Web UI URL, substrate status, attachment metadata, and per-session lease ownership. `scenery down` releases only the selected session's ZeroFS lease and preserves shared storage-cell data; `scenery storage cleanup --json` is dry-run by default and requires `--yes` after live-lease verification before deleting a cell. Treat ZeroFS paths, sockets, and object roots as substrate details unless you are intentionally debugging the storage runtime. Managed ZeroFS and storage CLI/inspect surfaces remain beta local-dev/operator tooling.
...
- Use `scenery db list --json`, `scenery db shell`, `scenery db branch status --json`, and `scenery db branch list --json` for managed SQLite branch work. Branch pins live at `.scenery/worktree-db.json`; Scenery-owned local branch leases live in `branches.json` under the agent SQLite state root. The branch provider copies the parent database file into branch-local state: checkout creates or reuses a branch database file, `reset` recopies from the parent, `delete` removes the branch database and lease, `expire` updates lease metadata, `prune` removes expired non-current branch databases, and `restore` currently maps to reset. `scenery up`, `scenery db shell`, DB setup, and sync consume ready branch endpoints and fail explicitly when the lease is missing, expired, protected, or endpoint-less. The default `scenery harness self --json --write` path includes the live SQLite branch lifecycle proof; use `--quick` for the smaller self-harness mode.
...
- If sync/TanStack `awaitTxId` observes the txid, the UI clears the pending task state normally.
- If sync observation times out or returns a substrate error, the app reports a sync observation failure with txid, app/session, API URL, sync URL or stream context, and observer error details; it must not say the API mutation failed or was rolled back.
