...
For a generated operations workbench, use one of two explicit list contracts. A CRUD source declares paginated `search`, `filters`, and `sorts`. An existing call-delivery HTTP binding can instead return one complete typed list: set `table_page.source` to the binding and `items` to its result-record list field; optional `search`, same-named list filters, and `sort`/`direction` enums on the operation input become the generated controls, and pagination is omitted. Only that complete-list form may declare repeated `group "field"` blocks, because client-side section counts over one cursor page would be misleading; each group may set a label, leading order, and one default, and the runtime selector always includes None. Reuse `status_map` resources for badge/filter/group presentation; set `pinned = true` only on the zero-to-two generated selectors that deserve inline quick access. Every filter remains in the Filters popover, active values remain visible as removable chips, and group/sort/direction stay separate from the filter count. Bind `table_page.stats` to a unit-input metrics operation; declare loaded-result CSV `export`; use `hidden = true` for export-only columns and `export = false` for display-only columns; and fill only domain-specific `row_detail` content with a typed `react_component`. Row detail defaults inline; set `presentation = "panel"` and optional `panel_width = 280..560` for the resizable right panel. `form_dialog` derives string/enum fields from a mutation input record, table actions open it, failures stay inline, and success invalidates list and stats query keys. `row_detail.dialog` is the inline-only edit path when every mutation input can be seeded from a matching row field. Regenerate instead of editing the emitted page, status-map, dialog, or route files.
