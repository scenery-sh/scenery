...
- Use `scenery worker` for worker-role execution of native legacy async runtime workers and cron.
- Use `scenery worker legacy-async-runtime prune --stale --json` to inspect stale open workflows on the current dev session's legacy async runtime task queues after `scenery up` reports a legacy async runtime stale-workflow warning. Add `--yes` only when intentional termination of those stale workflows is desired; Scenery never prunes legacy async runtime workflows by default.
...
- In handwritten services that use sqlc against the default app database, call `scenery.sh/db.Get(ctx)` during service initialization and pass the returned `*pgxpool.Pool` to the generated `Queries` constructor. Use `scenery.sh/pgxpool` directly only when the service intentionally owns an explicit secondary database connection.
...
- If sync observation times out or returns a substrate error such as Postgres lock acquisition timeout, the app reports a sync observation failure with txid, app/session, API URL, sync URL or stream context, and observer error details; it must not say the API mutation failed or was rolled back.
