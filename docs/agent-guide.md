...
- Development compilation uses a retained stock-Go recipe by default. The first build and every incompatible package/file/import/directive/module/configuration/tool change run a complete captured stock bootstrap; compatible body edits validate the retained domain, compile changed packages and transitive consumers, and relink without repeating `cmd/go` package loading. The recipe lives in the private development cache and is bound to the exact workspace and Scenery executable. Deleting it is safe and makes the next build bootstrap again. Ephemeral and deployable builds remain ordinary stock `go build`; no app setting or environment variable selects this policy.
