...
func GoTargetEnvironment(target GoTargetContext) []string {
	return hermeticOverlayGoEnvironment(&target)
}

// AppPackages loads only explicitly bounded workspace package roots. It is the
// mixed-mode legacy frontend boundary and never expands to sibling packages.
func AppPackages(root, name string, packageRoots []string) (*model.App, error) {
	return appPackages(root, name, packageRoots, map[string][]byte{})
}

// AppPackagesWithOverlay loads only bounded workspace packages while replacing
// generated files with their staged bytes. This breaks the bootstrap cycle
// where implementation source adopts a new generated ABI before generation.
func AppPackagesWithOverlay(root, name string, packageRoots []string, overlay map[string][]byte) (*model.App, error) {
	if overlay == nil {
...
func appPackages(root, name string, packageRoots []string, overlay map[string][]byte) (*model.App, error) {
	...
	for _, packageRoot := range packageRoots {
		...
		patterns = append(patterns, "./"+strings.TrimPrefix(clean, "./")+"/...")
	}
	if len(patterns) == 0 {
		return &model.App{Name: name, Root: root}, nil
	}
...
func app(root, name string, overlay map[string][]byte, patterns []string, target *GoTargetContext, allowEmpty bool) (*model.App, error) {
	...
	if overlay != nil {
		cfg.Env = hermeticOverlayGoEnvironment(nil)
	}
	if target != nil {
		cfg.Dir = target.ModuleRoot
		cfg.Env = hermeticOverlayGoEnvironment(target)
	...
	if (overlay != nil || target != nil) && !allowEmpty {
		...
		if len(loadErrors) > 0 {
			...
			return nil, fmt.Errorf("staged Go package loading failed: %s", strings.Join(loadErrors, "; "))
		}
	}
...
func hermeticOverlayGoEnvironment(target *GoTargetContext) []string {
	...
	environment := make([]string, 0, len(processEnvironment)+9)
...
func pruneEmptyServices(services []*model.Service) []*model.Service {
	...
	for _, svc := range services {
		if svc == nil || (svc.Struct == nil && len(svc.Endpoints) == 0 && len(svc.Generated) == 0) {
			continue
		}
...
var generatedEndpointReservedRoutePrefixes = []string{"/runtime", "/__scenery", "/api"}
...
type entityConfig struct {
	Table        string
	SourceKind   model.EntitySourceKind
	SourceSchema string
...
func parseEntity(pkg *model.Package, file *model.File, decl *ast.GenDecl, cfg entityConfig) (*model.Entity, []string) {
	...
	tableName := firstNonEmpty(cfg.Table, defaultTableName(spec.Name.Name))
	if cfg.SourceKind == model.EntitySourceExisting {
		tableName = cfg.Table
	}
	entity := &model.Entity{
		Package:  pkg,
	...
	entity.Source = model.EntitySource{Kind: model.EntitySourceGenerated, Schema: model.EntityDatabaseSchema(entity), Table: entity.Table}
	if cfg.SourceKind == model.EntitySourceExisting {
		entity.Source = model.EntitySource{Kind: model.EntitySourceExisting, Schema: cfg.SourceSchema, Table: entity.Table}
	}
	...
	errs = append(errs, validateExistingEntitySource(pkg, spec, entity)...)
