...
func TestAgentCapabilitiesAdvertiseCurrentSurface(t *testing.T) {
	...
	if !json.Valid(b) || strings.Contains(string(b), `"editions":`) || strings.Contains(string(b), `"profiles":`) || !containsJSONText(b, "resources.list") || !containsJSONText(b, "resource_create_kinds") || !containsJSONText(b, "sha256:") {
		t.Fatalf("result = %s", b)
	}
}
...
func TestAgentSchemaDescribesStructuredResourceAuthoring(t *testing.T) {
	...
	params, _ := json.Marshal(map[string]string{"kind": "scenery.operation"})
	response := HandleAgentRequest(result, AgentRequest{JSONRPC: "2.0", ID: json.RawMessage(`1`), Method: "schema.get", Params: params})
	if response.Error != nil {
		t.Fatal(response.Error)
	}
	...
	serviceSchema, _ := CoreSchema("scenery.service")
	serviceFields, _ := serviceSchema["fields"].(map[string]any)
	serviceConfig, _ := serviceFields["config"].(map[string]any)
	params, _ = json.Marshal(map[string]string{"kind": stringValue(serviceConfig["child_schema"])})
	...
	authorization, _ := CoreSchema("scenery.authorization")
	...
	binding, _ := CoreSchema("scenery.binding")
...
func TestAdvertisedResourceCreateSchemasHaveCompleteMetadata(t *testing.T) {
	...
	for _, required := range []string{"scenery.authentication", "scenery.binding", "scenery.operation", "scenery.record", "scenery.service"} {
...
func TestAgentMutationSchemasRequireOperationInputs(t *testing.T) {
	tests := map[string][]string{
		...
		"module.configure": {"op", "address", "value"},
	}
...
func TestAgentContextContinuationUsesRetainedWorkspaceSnapshot(t *testing.T) {
	base := &Manifest{ContractRevision: "sha256:base", Resources: []Resource{
		...
		{Address: "house/operation/process", Module: "house", Kind: "scenery.operation", Name: "process", Spec: map[string]any{"service": map[string]any{"$ref": "service.house"}}},
	}}
	...
	target.Resources = append(target.Resources, Resource{Address: "house/record/new", Module: "house", Kind: "scenery.record", Name: "new", Spec: map[string]any{}})
...
func TestAgentSessionRetainsExactRevisionSnapshots(t *testing.T) {
	baseManifest := &Manifest{ContractRevision: "sha256:base", Resources: []Resource{{Address: "house/record/item", Kind: "scenery.record", Module: "house", Name: "item", Spec: map[string]any{}}}}
	targetManifest := &Manifest{ContractRevision: "sha256:target", Resources: []Resource{{Address: "house/record/item", Kind: "scenery.record", Module: "house", Name: "item", Spec: map[string]any{"unknown_fields": "preserve"}}}}
	session := NewAgentSession()
	...
	diff, ok := response.Result.(evolution.SemanticDiff)
...
func TestAgentDiffConsumesRevisionBoundRenameReceipts(t *testing.T) {
	before := Resource{Address: "house/record/old", Kind: "scenery.record", Module: "house", Name: "old", Spec: map[string]any{}}
	after := before
	...
	params, _ := json.Marshal(map[string]any{"base": base, "target": target, "rename_receipts": []evolution.RenameReceipt{receipt}})
