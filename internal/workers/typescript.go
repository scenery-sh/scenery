...
func DiscoverTypeScriptActivities(appRoot string) TypeScriptWorkerModel {
	...
	err := filepath.WalkDir(appRoot, func(path string, entry os.DirEntry, err error) error {
		...
		if !strings.HasSuffix(entry.Name(), ".worker.ts") && !bytes.Contains(data, []byte("//scenery:worker")) {
			return nil
		}
...
func GenerateTypeScriptWorker(opts TypeScriptWorkerOptions) (TypeScriptWorkerResult, error) {
	...
	files := []struct {
		...
		{name: "scenery.ts", data: executeTypeScriptTemplate("scenery.ts", tsSceneryTemplate, nil)},
...
func registrationHash(queue string, activities []TypeScriptActivity) string {
	lines := []string{
		"schema=scenery.worker.registration.v1",
		"language=typescript",
...
var tsSceneryTemplate = template.Must(template.New("scenery.ts").Parse(`// Code generated by scenery; DO NOT EDIT.
...
export type ActivityImplementation<I, O> = ((input: I) => Promise<O>) & {
  __sceneryActivity?: { config: ActivityConfig };
};
...
export function activity<I, O>(config: ActivityConfig, handler: ActivityHandler<I, O>): ActivityImplementation<I, O> {
  ...
  Object.defineProperty(wrapped, "__sceneryActivity", {
    value: { config },
    enumerable: false,
  });
...
var tsRegistryTemplate = template.Must(template.New("registry.ts").Parse(`// Code generated by scenery; DO NOT EDIT.
...
var tsWorkerTemplate = template.Must(template.New("worker.ts").Parse(`// Code generated by scenery; DO NOT EDIT.
...
const buildId = process.env.SCENERY_BUILD_ID || "dev";
const deploymentName = sanitizeDeploymentName(process.env.SCENERY_TEMPORAL_DEPLOYMENT_NAME || "onlv-typescript");
const taskQueuePrefix = process.env.SCENERY_TEMPORAL_TASK_QUEUE_PREFIX || "";
const appId = process.env.SCENERY_APP_ID || "";
const sessionId = process.env.SCENERY_SESSION_ID || "";
const appRootHash = process.env.SCENERY_APP_ROOT_HASH || "";
const branch = process.env.SCENERY_BRANCH || "";
const worktree = process.env.SCENERY_WORKTREE || "";
const devReportURL = process.env.SCENERY_DEV_REPORT_URL || "";
const devReportToken = process.env.SCENERY_DEV_REPORT_TOKEN || "";
const appRoot = process.env.SCENERY_APP_ROOT;
const supervisorPID = parsePositiveInteger(process.env.SCENERY_DEV_SUPERVISOR_PID || "");
...
function selectedQueues(): string[] {
  const selected = process.env.SCENERY_TEMPORAL_TASK_QUEUE;
...
function scopedTaskQueue(taskQueue: string): string {
  ...
  const prefix = taskQueuePrefix.replace(/[.]+$/, "") || "scenery";
...
function monitorSupervisorProcess(pid: number): void {
  ...
  if (!processExists(pid)) {
    console.error("scenery TypeScript worker exiting because supervisor pid " + pid + " is not running");
    process.exit(0);
  }
  ...
  for (const signal of ["SIGINT", "SIGTERM"]) {
    once.call(process, signal, () => {
      const timer: any = setTimeout(() => {
        console.error("scenery TypeScript worker force exiting after " + signal);
        process.exit(0);
      }, 5000);
      if (timer && typeof timer.unref === "function") {
        timer.unref();
...
function installTemporalRuntimeTelemetry(): void {
  ...
  const globalTags: Record<string, string> = compactTags({
    scenery_app: appId,
    scenery_session_id: sessionId,
    scenery_language: "typescript",
  });
  Runtime.install({
    telemetryOptions: {
      metrics: {
...
function sceneryTemporalActivityInterceptor(sourceTaskQueue: string, taskQueue: string): any {
  return (activityContext: any) => ({
    inbound: {
      async execute(input: any, next: any): Promise<unknown> {
        const started = new Date();
        const parent = sceneryTemporalParentFromHeaders(input?.headers);
...
function sceneryTemporalParentFromHeaders(headers: any): { trace_id?: string; span_id?: string } | undefined {
  const payload = headers?.[sceneryTemporalTraceHeader] ?? (typeof headers?.get === "function" ? headers.get(sceneryTemporalTraceHeader) : undefined);
...
async function postDevReport(body: unknown): Promise<void> {
  try {
    ...
    clearTimeout(timer);
    if (!response.ok) {
      console.error("scenery TypeScript worker dev report failed: " + response.status + " " + response.statusText);
    }
  } catch (error) {
    console.error("scenery TypeScript worker dev report failed: " + errorMessage(error));
  }
}
...
const workers = await Promise.all(
  selectedQueues().map((sourceTaskQueue) => {
    ...
    return Worker.create({
      ...
      interceptors: {
        activity: [sceneryTemporalActivityInterceptor(sourceTaskQueue, taskQueue)],
      },
      identity: ` + "`scenery:${deploymentName}:typescript:${taskQueue}:pid-${process.pid}:build-${buildId}`" + `,
...
var tsConfigTemplate = template.Must(template.New("tsconfig.json").Parse(`{
  "compilerOptions": {
    ...
    "baseUrl": ".",
    "paths": {
      "scenery/worker": ["./scenery.ts"],
      "@scenery/temporal": ["./scenery.ts"]
    }
  }
}
`))
...
var tsPackageTemplate = template.Must(template.New("package.json").Parse(`{
  "name": "@scenery/generated-temporal-worker",
  "private": true,
  "type": "module",
  "dependencies": {
