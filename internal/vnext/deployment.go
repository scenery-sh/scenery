...
func ResolveDeployment(manifest *Manifest, name string) (DeploymentProjection, []Diagnostic) {
	projection := DeploymentProjection{APIVersion: "scenery.deployment-projection/v1", ContractRevision: "", Resources: map[string]DeploymentResourceProjection{}}
	if manifest == nil {
	...
	if !ok || deployment.Kind != "scenery.deployment/v1" {
	...
	for _, target := range manifest.Resources {
		...
		case "scenery.provider/v1", "scenery.data-source/v1", "scenery.execution-engine/v1", "scenery.event-bus/v1", "scenery.secret-store/v1":
	...
	for _, fixture := range manifest.Resources {
		if fixture.Kind != "scenery.fixture/v1" || !containsFixtureEnvironment(stringValues(fixture.Spec["environments"]), projection.Environment) {
			continue
		}
...
func resolveProviderDeployment(resources map[string]Resource, target Resource, overlay map[string]any, deployment Resource, writers map[string]bool) (DeploymentResourceProjection, []Diagnostic) {
	...
	provider := target
	if target.Kind != "scenery.provider/v1" {
...
func deploymentTargetKindMatches(kind, overlayKind string) bool {
	want := "scenery." + strings.ReplaceAll(overlayKind, "_", "-") + "/v1"
	return kind == want
}
