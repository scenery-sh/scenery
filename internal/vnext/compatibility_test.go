...
func TestSemanticDiffIsDeterministicAcrossResourceOrder(t *testing.T) {
	resources := []Resource{
		{Address: "app/http_gateway/public", Kind: "scenery.http-gateway/v1", Spec: map[string]any{"exposure": "internet"}},
		{Address: "house/operation/get", Kind: "scenery.operation/v1", Spec: map[string]any{"input": map[string]any{"$ref": "string"}}},
	}
...
func TestSemanticDiffClassifiesOpenAndClosedVariants(t *testing.T) {
	baseClosed := Resource{Address: "house/enum/state", Kind: "scenery.enum/v1", Name: "state", Module: "house", Spec: map[string]any{"value": map[string]any{"name": "ready"}}}
...
func TestSemanticDiffReportsNonApplicableDimensionsAndMigrations(t *testing.T) {
	base := Resource{Address: "house/execution/process", Kind: "scenery.execution/v1", Name: "process", Module: "house", Spec: map[string]any{"mode": "durable", "revision": "1"}}
	target := base
	...
	if len(diff.RequiredMigrations) != 2 || !diff.VersionRecommendation.Migration {
		t.Fatalf("migration metadata = %#v recommendation=%#v", diff.RequiredMigrations, diff.VersionRecommendation)
	}
}
...
func TestSemanticDiffMachineMetadataAndVersionRecommendation(t *testing.T) {
	operation := Resource{Address: "house/operation/get", Kind: "scenery.operation/v1", Name: "get", Module: "house", Spec: map[string]any{"input": map[string]any{"$ref": "string"}}}
	...
	if diff.CatalogDigest == "" || diff.Digest == "" || diff.VersionRecommendation.Level != "minor" {
...
func TestSemanticDiffClassifiesOperationOutcomesAndHTTPFacets(t *testing.T) {
	operationBase := Resource{Address: "house/operation/get", Kind: "scenery.operation/v1", Name: "get", Module: "house", Spec: map[string]any{
	...
	bindingBase := Resource{Address: "house/binding/get", Kind: "scenery.binding/v1", Name: "get", Module: "house", Spec: map[string]any{
...
func TestHTTPResponseMappingsDoNotLeakIntoRequestWireCompatibility(t *testing.T) {
	binding := Resource{Address: "house/binding/get", Kind: "scenery.binding/v1", Spec: map[string]any{"protocol": "http"}}
...
func TestSemanticDiffClassifiesBindingFamiliesAndScheduleState(t *testing.T) {
	tests := []struct {
		...
		{"internal", "scenery.binding/v1", map[string]any{"protocol": "internal", "internal": map[string]any{"visibility": "application"}}, map[string]any{"protocol": "internal", "internal": map[string]any{"visibility": "package"}}, "/spec/internal/visibility", "internal_call", CompatibilityBreaking},
		{"cli", "scenery.binding/v1", map[string]any{"protocol": "cli", "cli": map[string]any{"command": []any{"house", "run"}}}, map[string]any{"protocol": "cli", "cli": map[string]any{"command": []any{"house", "process"}}}, "/spec/cli/command", "source", CompatibilityBreaking},
		{"event", "scenery.binding/v1", map[string]any{"protocol": "event", "event": map[string]any{"channel": "old"}}, map[string]any{"protocol": "event", "event": map[string]any{"channel": "new"}}, "/spec/event/channel", "runtime", CompatibilityMigrationRequired},
		{"schedule", "scenery.schedule/v1", map[string]any{"trigger": "0 * * * *", "invoke": map[string]any{"$ref": "binding.run"}, "overlap": "skip"}, map[string]any{"trigger": "15 * * * *", "invoke": map[string]any{"$ref": "binding.run"}, "overlap": "skip"}, "/spec/trigger", "runtime", CompatibilityMigrationRequired},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
...
func TestSemanticDiffStorageProviderFallbackAndTargetInvalidation(t *testing.T) {
	entityBase := Resource{Address: "house/entity/scene", Kind: "scenery.entity/v1", Name: "scene", Module: "house", Spec: map[string]any{"mapping": "scenes"}}
	...
	targetBase := Resource{Address: "app/go_target/prod", Kind: "scenery.go-target/v1", Name: "prod", Module: "app", Spec: map[string]any{"goarch": "amd64"}}
...
func TestSemanticDiffUnknownFallbackAndAggregationSeverity(t *testing.T) {
	base := Resource{Address: "house/renderer/web", Kind: "scenery.renderer/v1", Name: "web", Module: "house", Spec: map[string]any{"runtime": "vite"}}
	target := base
...
func manifestWith(resources ...Resource) *Manifest {
	return &Manifest{APIVersion: ManifestVersion, Resources: resources}
}
...
func manifestWithWireType(resource Resource) *Manifest {
	...
	operation := Resource{Address: resource.Module + "/operation/use_type", Kind: "scenery.operation/v1", Name: "use_type", Module: resource.Module, Spec: map[string]any{
		"input":  typeRef,
...
func recordForDiff(unknownFields string, fields []any) Resource {
	...
	return Resource{Address: "house/record/payload", Kind: "scenery.record/v1", Name: "payload", Module: "house", Spec: spec}
}
