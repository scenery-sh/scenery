...
func TestResolveDeploymentAppliesTypedOverlaysWithProvenance(t *testing.T) {
	...
	if projection.APIVersion != "scenery.deployment-projection/v1" || projection.Environment != "production" {
	...
	if provider := projection.Resources["app/provider/postgres"]; provider.Kind != "scenery.provider/v1" {
...
func TestResolveDeploymentIncludesOnlySelectedEnvironmentFixtures(t *testing.T) {
	...
	manifest.Resources = append(manifest.Resources,
		Resource{Address: "house/record/scene", Module: "house", Name: "scene", Kind: "scenery.record/v1", Spec: map[string]any{"field": map[string]any{"name": "id", "type": map[string]any{"$ref": "string"}}}},
		Resource{Address: "house/entity/scene", Module: "house", Name: "scene", Kind: "scenery.entity/v1", Spec: map[string]any{"type": map[string]any{"$ref": "record.scene"}, "data_source": map[string]any{"$ref": "app/data_source/database"}, "mapping": map[string]any{"relation": "scenes"}, "field": map[string]any{"name": "id", "column": "id", "primary_key": true}}},
		Resource{Address: "house/fixture/demo", Module: "house", Name: "demo", Kind: "scenery.fixture/v1", Spec: map[string]any{"entity": map[string]any{"$ref": "entity.scene"}, "environments": []any{"production"}, "mode": "insert", "values": []any{map[string]any{"id": "one"}}}},
		Resource{Address: "app/deployment/development", Module: "app", Name: "development", Kind: "scenery.deployment/v1", Spec: map[string]any{"environment": "development"}},
	)
	production, diagnostics := ResolveDeployment(manifest, "production")
	if hasErrors(diagnostics) || production.Resources["house/fixture/demo"].Kind != "scenery.fixture/v1" {
...
func TestDeploymentRevisionRequiresImplementationAndProviderPlan(t *testing.T) {
	...
	baseContract, err := contractRevision(base.Resources, []string{"scenery.deployment/v1"}, "app")
	if err != nil {
		t.Fatal(err)
	}
...
func deploymentProfileFixtureManifest() *Manifest {
	return &Manifest{Application: ApplicationIdentity{Name: "app"}, Resources: []Resource{
		{Address: "app/module/house", Module: "app", Name: "house", Kind: "scenery.module/v1", Spec: map[string]any{
		...
		{Address: "app/provider/postgres", Module: "app", Name: "postgres", Kind: "scenery.provider/v1", Spec: map[string]any{"source": "registry/postgres", "version": "1.0.0", "config_schema": map[string]any{"database": map[string]any{"deployment_bindable": true}}}},
		{Address: "app/data_source/database", Module: "app", Name: "database", Kind: "scenery.data-source/v1", Spec: map[string]any{"provider": map[string]any{"$ref": "provider.postgres"}, "lifecycle": "managed", "config": map[string]any{"database": "dev"}}},
		{Address: "app/http_gateway/public", Module: "app", Name: "public", Kind: "scenery.http-gateway/v1", Spec: map[string]any{"exposure": "internet", "base_path": "/", "cors": map[string]any{"$ref": "std.cors.none"}, "trusted_proxies": map[string]any{"$ref": "std.trusted_proxies.none"}, "forwarded": map[string]any{"$ref": "std.forwarded_headers.reject"}}},
		{Address: "house/service/house", Module: "house", Name: "house", Kind: "scenery.service/v1", Spec: map[string]any{"runtime": "go", "implementation": map[string]any{"constructor": "NewService"}}},
		{Address: "app/deployment/production", Module: "app", Name: "production", Kind: "scenery.deployment/v1", Spec: map[string]any{
