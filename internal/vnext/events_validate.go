...
func validateScheduleSemantics(resources map[string]Resource, schedule Resource) []Diagnostic {
	...
	if err := validateScheduleTrigger(trigger); err != nil {
		diagnostics = append(diagnostics, profileDiagnostic("SCN2304", err.Error(), schedule))
	}
	if overlap := stringValue(schedule.Spec["overlap"]); overlap != "skip" && overlap != "queue" && overlap != "replace" && overlap != "allow" {
		diagnostics = append(diagnostics, profileDiagnostic("SCN2304", "schedule overlap must be skip, queue, replace, or allow", schedule))
	}
	...
	if !operationOK || operation.Kind != "scenery.operation/v1" || !executionOK || execution.Kind != "scenery.execution/v1" || resolveResourceRef(execution, refString(execution.Spec["operation"]), "operation") != operationAddress {
		diagnostics = append(diagnostics, profileDiagnostic("SCN2303", "schedule operation and execution must resolve to the same operation", schedule))
		return diagnostics
	}
	if refString(invoke["identity"]) == "" || refString(invoke["authorization"]) == "" || refString(invoke["pipeline"]) == "" {
		diagnostics = append(diagnostics, profileDiagnostic("SCN2303", "schedule requires typed identity, authorization, and pipeline references", schedule))
	} else if err := validateScheduleWorkloadIdentity(resources, schedule, invoke["identity"]); err != nil {
		diagnostics = append(diagnostics, profileDiagnostic("SCN2303", err.Error(), schedule))
	}
...
func validateEventBindingSemantics(resources map[string]Resource, binding Resource) []Diagnostic {
	...
	if stringValue(eventSpec["direction"]) != "consume" {
		diagnostics = append(diagnostics, profileDiagnostic("SCN2704", "event binding direction must be consume", binding))
	}
	if !validEventGuarantee(stringValue(eventSpec["guarantee"])) || strings.TrimSpace(stringValue(eventSpec["channel"])) == "" {
		diagnostics = append(diagnostics, profileDiagnostic("SCN2705", "event binding requires a channel and supported delivery guarantee", binding))
	}
	...
	if !busOK || bus.Kind != "scenery.event-bus/v1" || !eventOK || event.Kind != "scenery.event/v1" {
		diagnostics = append(diagnostics, profileDiagnostic("SCN2705", "event binding bus and contract must resolve to typed resources", binding))
		return diagnostics
	}
	...
	if !operationOK || len(mappings) != 1 || refOrString(mappings[0]["from"]) != "message.payload" || refOrString(mappings[0]["to"]) != "operation."+operation.Name+".input" || !sameTypeExpression(event.Spec["payload"], operation.Spec["input"]) {
		diagnostics = append(diagnostics, profileDiagnostic("SCN2705", "event payload must map exactly once to a type-compatible operation input", binding))
	}
	if retry, ok := eventSpec["broker_retry"].(map[string]any); ok {
		...
		if !attemptsOK || attempts <= 0 || !eventStringIn([]string{"none", "fixed", "exponential"}, stringValue(retry["backoff"])) {
			diagnostics = append(diagnostics, profileDiagnostic("SCN2705", "event broker_retry policy is invalid", binding))
		}
	} else if stringValue(eventSpec["guarantee"]) == "at_least_once" {
		diagnostics = append(diagnostics, profileDiagnostic("SCN2705", "at_least_once event consumption requires broker_retry policy", binding))
	}
	return diagnostics
}
...
func validateEventEmissionSemantics(resources map[string]Resource, emission Resource) []Diagnostic {
	...
	bus, busOK := resources[busAddress]
	event, eventOK := resources[contractAddress]
	if !busOK || bus.Kind != "scenery.event-bus/v1" || !eventOK || event.Kind != "scenery.event/v1" || !validEventGuarantee(stringValue(emission.Spec["guarantee"])) || strings.TrimSpace(stringValue(emission.Spec["channel"])) == "" {
		return []Diagnostic{profileDiagnostic("SCN2706", "event emission requires typed bus, contract, channel, and guarantee", emission)}
	}
	...
	if !operationOK || len(when) != 2 || when[0] != "result" || len(payload) < 2 || payload[0] != "result" || payload[1] != when[1] {
		return []Diagnostic{profileDiagnostic("SCN2706", "event emission must select one operation result and payload", emission)}
	}
	var resultType any
	...
	if resultType == nil {
		return []Diagnostic{profileDiagnostic("SCN2706", "event emission references an unknown result variant", emission)}
	}
	...
	if payloadType == nil || !sameTypeExpression(payloadType, event.Spec["payload"]) {
		diagnostics = append(diagnostics, profileDiagnostic("SCN2706", "event emission payload type does not match the event contract", emission))
	}
	for _, field := range []string{"ordering_key", "deduplication_key"} {
		if _, _, err := eventEmissionKeyExpression(resources, operation, when[1], emission.Spec[field]); err != nil {
			diagnostics = append(diagnostics, profileDiagnostic("SCN2706", "event emission "+field+" "+err.Error(), emission))
		}
	}
	if retry, ok := emission.Spec["broker_retry"].(map[string]any); ok {
		attempts, attemptsOK := integerValue(retry["attempts"])
		if !attemptsOK || attempts <= 0 || !eventStringIn([]string{"none", "fixed", "exponential"}, stringValue(retry["backoff"])) {
			diagnostics = append(diagnostics, profileDiagnostic("SCN2706", "event emission broker_retry policy is invalid", emission))
		}
	}
	return diagnostics
}
