...
func validateUISemantics(root string, resources []Resource) []Diagnostic {
	...
	for _, resource := range resources {
		...
		case "scenery.renderer/v1":
			...
			if page.Kind != "scenery.page/v1" || runtimeName != "web" {
...
func isPageInternalBinding(binding Resource) bool {
	if binding.Kind != "scenery.binding/v1" || stringValue(binding.Spec["protocol"]) != "internal" {
		return false
	}
...
func enrichUIImplementationDigests(root string, resources []Resource) ([]Resource, []Diagnostic) {
	...
	for index := range result {
		...
		if resource.Kind != "scenery.renderer/v1" {
			continue
		}
