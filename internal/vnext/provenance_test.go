...
`)...)
	...
	for _, path := range []string{"/spec/package/version", "/spec/interface_inputs/gateway/type", "/spec/export_metadata/service/value"} {
	...
	if strategyOrigin.Kind != "default" || strategyOrigin.ProvidedBy != "edition:2027" {
	...
	for address, field := range map[string]string{
		...
		if origin.Kind != "default" || origin.ProvidedBy != "edition:2027" {
...
func TestCompilerExplainsPackageDefaultAndModuleInputFieldProvenance(t *testing.T) {
	...
	rootSource = bytes.ReplaceAll(rootSource, []byte("    \"scenery.runtime-http/v1\",\n"), nil)
...
`)...)
	...
	params, _ := json.Marshal(map[string]any{"address": "house/service/house", "view": "effective"})
	explained := HandleAgentRequest(result, AgentRequest{Method: "resources.explain", Params: params})
	explainedJSON, _ := json.Marshal(explained.Result)
	if explained.Error != nil || !strings.Contains(string(explainedJSON), `"/spec/config/model_path":{"kind":"package_default"`) {
		t.Fatalf("resources.explain provenance = %s error=%v", explainedJSON, explained.Error)
	}
	...
	explained = HandleAgentRequest(result, AgentRequest{Method: "resources.explain", Params: params})
	explainedJSON, _ = json.Marshal(explained.Result)
	if explained.Error != nil || !strings.Contains(string(explainedJSON), `"/spec/config/model_path":{"kind":"module_input"`) {
		t.Fatalf("resources.explain caller provenance = %s error=%v", explainedJSON, explained.Error)
	}
}
...
application "module_views" { version = "1.0.0" }
