...
func validateNativeGoServices(appModel *model.App, resources []Resource) []Diagnostic {
	...
	for _, resource := range resources {
		if resource.Kind != "scenery.service/v1" || resource.Origin.Kind != "authored" || stringValue(resource.Spec["runtime"]) != "go" {
			continue
		}
...
func validateNativeGoHandlers(appModel *model.App, resources []Resource) []Diagnostic {
	...
	for _, operation := range resources {
		if operation.Kind != "scenery.operation/v1" || operation.Origin.Kind != "authored" {
			continue
		}
		...
		for _, candidate := range resources {
			if candidate.Kind == "scenery.service/v1" && candidate.Module == operation.Module && candidate.Origin.Kind == "authored" {
