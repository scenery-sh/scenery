...
func typescriptRevision(target Resource, resources []Resource) string {
	...
	revisionInput := map[string]any{"target": targetProjection, "resources": projected, "profile": "scenery.typescript-client/v1", "codec": "scenery.http-codec/v1"}
	if bindingsUseHTTPPathTail(resources) {
		revisionInput["extension_profiles"] = []string{HTTPPathTailProfile}
	}
...
func typeScriptCodecProfileDigests(bindings []Resource) map[string]string {
	profiles := map[string]string{"scenery.http-codec/v1": profileIdentityDigest("scenery.http-codec/v1")}
	if bindingsUseHTTPPathTail(bindings) {
		profiles[HTTPPathTailProfile] = profileIdentityDigest(HTTPPathTailProfile)
	}
	return profiles
}
