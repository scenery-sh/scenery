...
func expandDataResources(resources []Resource) ([]Resource, []Diagnostic) {
	...
	for index := range result {
		if result[index].Kind != "scenery.crud/v1" {
			continue
		}
	...
	for _, crud := range resources {
		if crud.Kind != "scenery.crud/v1" || crud.Origin.Kind == "expanded" {
			continue
		}
...
func expandCRUDResource(resources map[string]Resource, crud Resource) ([]Resource, error) {
	...
	if !ok || entity.Kind != "scenery.entity/v1" {
	...
	if !ok || record.Kind != "scenery.record/v1" {
	...
	lineage := func(output, rule string) Origin {
		return Origin{Kind: "expanded", SourceID: crud.Origin.SourceID, ModuleChain: append([]string(nil), crud.Origin.ModuleChain...), ExpansionLineage: []ExpansionStep{{Generator: crud.Address, GeneratorSchemaRevision: "scenery.crud/v1", Key: rule, SourceRange: crud.Origin.DeclarationRange, ParentAddress: crud.Address, Output: output}}}
	}
	serviceName := crud.Name + "_data"
	serviceAddress := resourceAddress(crud.Module, "service", serviceName)
	generated := []Resource{{
		Address: serviceAddress, Module: crud.Module, Name: serviceName, Kind: "scenery.service/v1", Origin: lineage(serviceAddress, "service"),
	...
	for _, action := range actions {
		...
		generated = append(generated,
			Resource{Address: inputAddress, Module: crud.Module, Name: inputName, Kind: "scenery.record/v1", Origin: lineage(inputAddress, action+".input"), Spec: map[string]any{"field": inputFields}},
			Resource{Address: resultAddress, Module: crud.Module, Name: resultName, Kind: "scenery.record/v1", Origin: lineage(resultAddress, action+".result"), Spec: map[string]any{"field": resultFields}},
		)
		...
		operation := Resource{
			Address: operationAddress, Module: crud.Module, Name: operationName, Kind: "scenery.operation/v1", Origin: lineage(operationAddress, action+".operation"),
			Spec: map[string]any{
		...
		generated = append(generated, operation, Resource{Address: executionAddress, Module: crud.Module, Name: executionName, Kind: "scenery.execution/v1", Origin: lineage(executionAddress, action+".execution"), Spec: executionSpec})
		...
		if internalSpec, ok := crud.Spec["internal"].(map[string]any); ok {
			...
			generated = append(generated, Resource{Address: address, Module: crud.Module, Name: bindingName, Kind: "scenery.binding/v1", Origin: lineage(address, action+".internal"), Spec: map[string]any{
...
func expandCRUDHTTPBinding(crud, entity, record Resource, action, operationName, executionName string, primary, all []map[string]any, httpSpec map[string]any, lineage func(string, string) Origin) Resource {
	...
	return Resource{Address: address, Module: crud.Module, Name: name, Kind: "scenery.binding/v1", Origin: lineage(address, action+".http"), Spec: map[string]any{
