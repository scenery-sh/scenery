...
func validateGoTargets(root string, resources []Resource) []Diagnostic {
	...
	for _, resource := range resources {
		...
		for _, candidate := range resources {
			if candidate.Kind == "scenery.go-target/v1" {
		...
		if byAddress[moduleAddress].Kind != "scenery.go-module/v1" || byAddress[toolchainAddress].Kind != "scenery.go-toolchain/v1" {
