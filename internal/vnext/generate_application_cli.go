...
func cliBindingsForOperations(resources, operations []Resource) []Resource {
	...
	for _, resource := range resources {
		if resource.Kind == "scenery.binding/v1" && stringValue(resource.Spec["protocol"]) == "cli" && owned[resolveResourceRef(resource, refString(resource.Spec["operation"]), "operation")] {
