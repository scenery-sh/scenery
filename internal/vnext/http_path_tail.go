...
func applyHTTPPathTailEffective(binding *Resource, httpSpec map[string]any, resources map[string]*Resource) {
	...
	httpSpec["required_profiles"] = []any{HTTPPathTailProfile}
	setFieldProvenance(&binding.Origin, "/spec/http/required_profiles", httpSpec["required_profiles"], httpPathTailDefaultField())
...
func httpPathTailDefaultField() FieldProvenance {
	return FieldProvenance{Kind: "default", ProvidedBy: HTTPPathTailProfile, Transformations: []string{"http_path_tail_profile_default"}}
}
