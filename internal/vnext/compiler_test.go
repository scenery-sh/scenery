...
func TestCompileRejectsSymlinkedSourceFiles(t *testing.T) {
	...
	if err := os.WriteFile(external, []byte("language { edition = \"2027\" }\n"), 0o644); err != nil {
		t.Fatal(err)
	}
...
func TestLegacySourceMapCoversEveryOriginWithPortableURIs(t *testing.T) {
	...
	resources := []Resource{
		{Address: "house/operation/get", Origin: Origin{Kind: "legacy_v0", SourceID: "src_legacy_file", LegacyIdentity: map[string]any{"file": "house/api.go"}}},
		{Address: "house/service/house", Origin: Origin{Kind: "legacy_v0", SourceID: "src_legacy_virtual"}},
	}
	sourceMap := map[string]SourceRecord{}
	addLegacySourceRecords(sourceMap, resources)
	for _, resource := range resources {
		record, ok := sourceMap[resource.Origin.SourceID]
		if !ok {
			t.Fatalf("missing source map entry for %s", resource.Origin.SourceID)
		}
		if record.URI == "" || filepath.IsAbs(record.URI) {
			t.Fatalf("non-portable legacy source URI %q", record.URI)
		}
	}
	if got := sourceMap["src_legacy_file"].URI; got != "house/api.go" {
		t.Fatalf("legacy file URI = %q", got)
	}
	if got := sourceMap["src_legacy_virtual"].URI; got != "scenery-legacy:///source/src_legacy_virtual" {
		t.Fatalf("legacy virtual URI = %q", got)
	}
}
...
func TestContractRevisionUsesOnlyContractDomains(t *testing.T) {
	resources := []Resource{
		{Address: "house/service/house", Module: "house", Name: "house", Kind: "scenery.service/v1", Spec: map[string]any{"runtime": "go", "implementation": map[string]any{"constructor": "NewOne"}}},
		{Address: "house/operation/get", Module: "house", Name: "get", Kind: "scenery.operation/v1", Spec: map[string]any{"service": map[string]any{"$ref": "service.house"}, "input": map[string]any{"$ref": "string"}, "handler": map[string]any{"method": "GetOne"}, "result": map[string]any{"name": "ok", "type": map[string]any{"$ref": "string"}}}},
		{Address: "house/binding/get", Module: "house", Name: "get", Kind: "scenery.binding/v1", Spec: map[string]any{"protocol": "http", "http": map[string]any{"method": "GET", "path": "/one"}}},
	}
	base, err := contractRevision(resources, []string{"scenery.compiler-core/v1"}, "app")
	if err != nil {
		t.Fatal(err)
	}
	...
	implementationRevision, _ := contractRevision(changedImplementation, []string{"scenery.compiler-core/v1"}, "app")
	if implementationRevision != base {
		t.Fatal("implementation-only symbols changed contract revision")
	}
	withDeployment := append(append([]Resource(nil), resources...), Resource{Address: "app/deployment/prod", Module: "app", Name: "prod", Kind: "scenery.deployment/v1", Spec: map[string]any{"replicas": "4"}})
	deploymentRevision, _ := contractRevision(withDeployment, []string{"scenery.compiler-core/v1"}, "app")
	...
	contractChange, _ := contractRevision(changedContract, []string{"scenery.compiler-core/v1"}, "app")
...
func TestWorkspaceRevisionIncludesOnlyDescriptorOwnedGeneratedFiles(t *testing.T) {
	...
	descriptor := filepath.Join(generated, "scenery.client-generated.v1.json")
...
func TestCompileVerifiesDeclaredGoTargetPlatform(t *testing.T) {
	...
	contract = []byte(strings.Replace(string(contract), "platform  = \"host\"", "platform  = \"fixed\"\n  goos     = \"linux\"\n  goarch   = \"amd64\"", 1))
	if err := os.WriteFile(contractPath, contract, 0o644); err != nil {
		t.Fatal(err)
	}
	broken := "//go:build linux\n\npackage house\n\nvar _ = missingLinuxTargetSymbol\n"
	if err := os.WriteFile(filepath.Join(root, "house", "broken_linux.go"), []byte(broken), 0o644); err != nil {
		t.Fatal(err)
	}
	result, err := Compile(root)
	if err != nil {
	...
	if result.ContractStatus != "valid" || result.ImplementationStatus != "invalid" || !hasDiagnostic(result.Diagnostics, "SCN6202") {
		t.Fatalf("target-specific compile = contract %s implementation %s diagnostics %#v", result.ContractStatus, result.ImplementationStatus, result.Diagnostics)
	}
}
...
func TestFormatPathsScopesWritesAndFormatsMigration(t *testing.T) {
	temp := t.TempDir()
	copyTree(t, filepath.Join("testdata", "house"), temp)
	rootPath := filepath.Join(temp, "scenery.scn")
	packagePath := filepath.Join(temp, "house", "scenery.package.scn")
	migrationPath := filepath.Join(temp, "scenery.migration.scn")
	if err := os.WriteFile(rootPath, []byte("language{edition=\"2027\"}\n"), 0o640); err != nil {
		t.Fatal(err)
	}
	if err := os.Chmod(rootPath, 0o640); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(packagePath, []byte("package{version=\"1\"}\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(migrationPath, []byte("migration{frontend=\"scenery.legacy.v0\"}\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	result, err := FormatPaths(temp, []string{"scenery.scn"}, false)
	if err != nil {
		t.Fatal(err)
	}
	if !reflect.DeepEqual(result.Changed, []string{"scenery.scn"}) {
		t.Fatalf("changed = %#v", result.Changed)
	}
	packageBytes, _ := os.ReadFile(packagePath)
	if string(packageBytes) != "package{version=\"1\"}\n" {
		t.Fatal("path-scoped format changed another file")
	}
	info, _ := os.Stat(rootPath)
	if info.Mode().Perm() != 0o640 {
		t.Fatalf("mode = %o", info.Mode().Perm())
	}
	result, err = Format(temp, false)
	if err != nil {
		t.Fatal(err)
	}
...
func TestProfileDependenciesExpandDeterministically(t *testing.T) {
	temp := t.TempDir()
	copyTree(t, filepath.Join("testdata", "house"), temp)
	path := filepath.Join(temp, "scenery.scn")
	b, err := os.ReadFile(path)
	...
	b = []byte(strings.Replace(string(b), `"scenery.runtime-http/v1",`, `"scenery.runtime-http/v1",
    "scenery.runtime-durable/v1",
    "scenery.events/v1",
    "scenery.data/v1",
    "scenery.deployment/v1",
    "scenery.agent-mutation/v1",
    "scenery.patches/v1",
    "scenery.ui/v1",`, 1))
	if err := os.WriteFile(path, b, 0o644); err != nil {
		t.Fatal(err)
	...
	for _, profile := range []string{
		"scenery.agent-read/v1",
		"scenery.compatibility-core/v1",
		"scenery.compiler-core/v1",
		"scenery.http-codec/v1",
		"scenery.inspection-core/v1",
	} {
		if !containsString(result.Manifest.Profiles, profile) {
			t.Errorf("missing dependency %s in %#v", profile, result.Manifest.Profiles)
		}
	}
}
...
func TestReferenceValidationRejectsUnknownResourcesAndModuleInputs(t *testing.T) {
	resources := []Resource{
		{Address: "app/module/house", Module: "app", Kind: "scenery.module/v1", Name: "house", Spec: map[string]any{"source": "./house", "inputs": map[string]any{"gateway": map[string]any{"$ref": "http_gateway.public"}}}},
		{Address: "app/http_gateway/public", Module: "app", Kind: "scenery.http-gateway/v1", Name: "public", Spec: map[string]any{}},
		{Address: "house/binding/bad", Module: "house", Kind: "scenery.binding/v1", Name: "bad", Spec: map[string]any{
...
func TestResourceSchemaRejectsUnknownFields(t *testing.T) {
	resources := []Resource{{Address: "house/operation/process", Module: "house", Kind: "scenery.operation/v1", Spec: map[string]any{
