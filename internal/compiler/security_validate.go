...
func validateSecurityResources(resources []Resource) []Diagnostic {
	...
	for _, resource := range resources {
		...
		case "scenery.authorization":
			diagnostics = append(diagnostics, validateAuthorizationResource(resource)...)
		case "scenery.pipeline":
			diagnostics = append(diagnostics, validatePipelineResource(resource)...)
		case "scenery.middleware":
			diagnostics = append(diagnostics, securityDiagnostic("SCN4203", "feature_unavailable: custom middleware is not implemented", resource, "/spec"))
		}
	}
	for _, resource := range resources {
...
func validateBindingAuthorizationTypes(resources map[string]Resource, binding Resource) []Diagnostic {
	...
	if !ok || authorization.Kind != "scenery.authorization" {
		return nil
	}
...
func validateBindingAuthentication(resources map[string]Resource, binding Resource) []Diagnostic {
	...
	if !ok || authentication.Kind != "scenery.authentication" {
	...
	if protocol != "http" || provider != "std.provider.standard_auth" || scheme != "session" {
		return []Diagnostic{securityDiagnostic("SCN4106", "capability_unavailable: scenery.runtime-http supports only std.provider.standard_auth session authentication", binding, "/spec/authentication")}
	}
	return nil
}
...
func validatePipelineResource(resource Resource) []Diagnostic {
	...
	for index, step := range orderedChildren(resource.Spec, "step") {
		...
		if !standard[use] {
			diagnostics = append(diagnostics, securityDiagnostic("SCN4201", "scenery.runtime-http pipeline step requires a supported standard middleware", resource, path))
		}
		if seen[use] {
