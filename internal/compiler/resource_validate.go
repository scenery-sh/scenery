	...
	for _, resource := range resources {
		...
		case "scenery.event":
			if resource.Spec["payload"] == nil || !positiveInteger(resource.Spec["version"]) {
				diagnostics = append(diagnostics, resourceDiagnostic("SCN2701", "event requires payload and positive version", resource))
			}
		case "scenery.event-emission":
			if missingAny(resource.Spec, "bus", "channel", "contract", "guarantee", "from") {
				diagnostics = append(diagnostics, resourceDiagnostic("SCN2702", "event emission requires bus, channel, contract, guarantee, and from", resource))
			}
		case "scenery.data-source":
			if missingAny(resource.Spec, "provider", "lifecycle") {
				diagnostics = append(diagnostics, resourceDiagnostic("SCN2505", "data source requires provider and lifecycle", resource))
			}
		case "scenery.entity":
			if resource.Spec["type"] == nil {
				diagnostics = append(diagnostics, resourceDiagnostic("SCN2501", "entity requires a record type", resource))
			}
			if resource.Spec["data_source"] == nil {
				diagnostics = append(diagnostics, resourceDiagnostic("SCN2502", "entity requires a data source", resource))
			}
		case "scenery.view":
			if missingAny(resource.Spec, "data_source", "input", "result", "implementation") {
				diagnostics = append(diagnostics, resourceDiagnostic("SCN2503", "view requires data_source, input, result, and implementation", resource))
			}
		case "scenery.crud":
			if missingAny(resource.Spec, "entity", "implementation", "actions", "execution") {
				diagnostics = append(diagnostics, resourceDiagnostic("SCN2504", "CRUD requires entity, implementation, actions, and execution", resource))
			}
		...
		case "scenery.renderer":
			if missingAny(resource.Spec, "page", "runtime", "module") {
				diagnostics = append(diagnostics, resourceDiagnostic("SCN2602", "renderer requires page, runtime, and module", resource))
			}
		case "scenery.deployment":
			if resource.Spec["environment"] == nil {
				diagnostics = append(diagnostics, resourceDiagnostic("SCN2805", "deployment requires environment", resource))
			}
...
func validateOperation(resource Resource, resources map[string]Resource) []Diagnostic {
	var diagnostics []Diagnostic
	if refString(resource.Spec["service"]) == "" {
		diagnostics = append(diagnostics, resourceDiagnostic("SCN2004", "operation requires a service owner", resource))
	}
...
func InputKeyFieldName(value any) (string, bool) { return inputKeyFieldName(value) }
	...
	for _, gateway := range namedChildren(deployment.Spec, "http_gateway") {
		for _, listener := range namedChildren(gateway, "listener") {
			for name := range listener {
				capability := unsupportedDraftCapability(deploymentListenerSourceSchema.Revision, name)
				if capability == "" {
					continue
				}
				diagnostics = append(diagnostics, resourceDiagnostic("SCN7009", "unsupported_draft: deployment listener field "+name+" requires unresolved capability "+capability, deployment))
			}
		}
	}
	return diagnostics
...
func validateExecution(resource Resource) []Diagnostic {
	...
	if mode == "" || resource.Spec["operation"] == nil {
		return []Diagnostic{resourceDiagnostic("SCN2200", "execution requires operation and mode", resource)}
	}
	if mode == "workflow" {
		return []Diagnostic{resourceDiagnostic("SCN2204", "feature_unavailable: workflow execution is not available in the current Scenery build", resource)}
	}
	if mode != "durable" {
		return nil
	}
	...
	if resource.Spec["engine"] == nil {
		diagnostics = append(diagnostics, resourceDiagnostic("SCN2201", "durable execution requires an engine", resource))
	}
	if !positiveInteger(resource.Spec["revision"]) {
		diagnostics = append(diagnostics, resourceDiagnostic("SCN2202", "durable execution requires a positive revision", resource))
	}
	if missingAny(resource.Spec, "timeout", "lease", "attempts", "retry", "retention") {
		diagnostics = append(diagnostics, resourceDiagnostic("SCN2203", "durable execution requires timeout, lease, attempts, retry, and retention", resource))
	}
	return diagnostics
}
...
func validateExecutionBindings(resources []Resource) []Diagnostic {
	...
	for _, binding := range resources {
		...
		if !ok || execution.Kind != "scenery.execution" {
			continue
		}
...
func validateDurableExecutions(resources []Resource) []Diagnostic {
	...
	for _, execution := range resources {
		...
		if !engineOK || engine.Kind != "scenery.execution-engine" {
			diagnostics = append(diagnostics, resourceDiagnostic("SCN2206", "durable execution engine must reference an execution_engine", execution))
		}
		...
		if timeoutErr != nil || leaseErr != nil || timeout.Sign() <= 0 || lease.Sign() <= 0 || lease.Cmp(timeout) > 0 || !timeout.Nanoseconds().IsInt64() || !lease.Nanoseconds().IsInt64() || !attemptsOK || attempts <= 0 {
			diagnostics = append(diagnostics, resourceDiagnostic("SCN2207", "durable execution requires positive timeout, lease not exceeding timeout, and positive attempts", execution))
		}
		...
		if !validRetry || successErr != nil || failureErr != nil || success.Sign() <= 0 || failure.Sign() <= 0 || !success.Nanoseconds().IsInt64() || !failure.Nanoseconds().IsInt64() {
			diagnostics = append(diagnostics, resourceDiagnostic("SCN2208", "durable execution retry and retention policies are invalid", execution))
		}
		...
		if stringValue(idempotency["mode"]) == "keyed" {
			...
			if retentionErr != nil || retentionValue.Sign() <= 0 || !retentionValue.Nanoseconds().IsInt64() || stringValue(deduplication["conflict"]) != "return_existing" {
				diagnostics = append(diagnostics, resourceDiagnostic("SCN2205", "keyed-idempotent durable execution requires positive return_existing deduplication", execution))
			}
		}
		if concurrency, ok := execution.Spec["concurrency"].(map[string]any); ok {
			...
			if !strings.HasPrefix(key, "input.") || strings.ContainsAny(strings.TrimPrefix(key, "input."), " []()") || !limitOK || limit <= 0 {
				diagnostics = append(diagnostics, resourceDiagnostic("SCN2209", "durable concurrency requires an input field key and positive limit", execution))
			}
		}
	}
	return diagnostics
...
func validateBinding(resource Resource) []Diagnostic {
	...
	if missingAny(resource.Spec, "operation", "execution", "protocol", "delivery", "authentication", "authorization", "pipeline") {
		diagnostics = append(diagnostics, resourceDiagnostic("SCN2401", "binding requires operation, execution, protocol, delivery, authentication, authorization, and pipeline", resource))
	}
	...
	if protocol == "internal" {
		...
		if internal == nil || internal["principal"] != "inherit" {
			diagnostics = append(diagnostics, resourceDiagnostic("SCN2402", "internal binding requires principal = inherit", resource))
		}
		visibility := stringValue(internal["visibility"])
		if visibility != "package" && visibility != "application" {
			diagnostics = append(diagnostics, resourceDiagnostic("SCN2405", "internal binding visibility must be package or application", resource))
		}
		...
		if visibility == "package" && exposure != "local" || visibility == "application" && exposure != "application" {
			diagnostics = append(diagnostics, resourceDiagnostic("SCN2406", "internal binding exposure must match its package/application visibility", resource))
		}
	}
	if protocol == "event" {
		event, _ := resource.Spec["event"].(map[string]any)
		if event == nil || missingAny(event, "direction", "bus", "channel", "contract", "guarantee") {
			diagnostics = append(diagnostics, resourceDiagnostic("SCN2703", "event binding requires direction, bus, channel, contract, and guarantee", resource))
		}
	}
	return diagnostics
}
...
func validateSchedule(resource Resource) []Diagnostic {
	...
	if selectors != 1 {
		diagnostics = append(diagnostics, resourceDiagnostic("SCN2301", "schedule requires exactly one trigger selector", resource))
	}
	...
	if invoke == nil || missingAny(invoke, "operation", "execution", "identity", "authorization", "pipeline", "input") {
		diagnostics = append(diagnostics, resourceDiagnostic("SCN2302", "schedule invoke requires operation, execution, identity, authorization, pipeline, and complete input", resource))
	}
	return diagnostics
}
...
func resourceDiagnostic(code, message string, resource Resource) Diagnostic {
