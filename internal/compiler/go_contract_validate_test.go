...
func TestGoContractOwnershipRequiresServicesAndUniqueOwners(t *testing.T) {
	module := func(name, source, importPath string) Resource {
		...
		return Resource{Address: "app/module/" + name, Module: "app", Kind: "scenery.module", Name: name, Spec: map[string]any{"source": source, "workspace_package_root": source, "package": metadata}}
	}
	service := func(module string) Resource {
		return Resource{Address: module + "/service/api", Module: module, Kind: "scenery.service", Name: "api", Origin: Origin{Kind: "authored"}, Spec: map[string]any{"runtime": "go"}}
	}
