...
var reservedCLICommands = map[string]bool{
	...
	"schema": true, "storage": true, "system": true, "task": true, "test": true, "traces": true,
...
func validateCLIBindings(resources []Resource) []Diagnostic {
	...
	for _, binding := range resources {
		if binding.Kind != "scenery.binding" || stringValue(binding.Spec["protocol"]) != "cli" {
			continue
		}
		...
		if !validCommand {
			diagnostics = append(diagnostics, resourceDiagnostic("SCN2410", "CLI command requires lower-kebab-case non-empty segments", binding))
		} else if reservedCLICommands[command[0]] {
			diagnostics = append(diagnostics, resourceDiagnostic("SCN2410", "CLI command conflicts with reserved scenery command "+command[0], binding))
		...
		for _, group := range []string{"context", "argument", "flag"} {
			for _, mapping := range namedChildren(cli, group) {
				...
				if !valid || mapped[mapKey] {
					diagnostics = append(diagnostics, resourceDiagnostic("SCN2412", "CLI input mapping target is invalid or duplicated: "+target, binding))
				} else {
					mapped[mapKey] = true
				}
				switch group {
				case "argument":
					...
					if !ok || position < 0 || positions[position] {
						diagnostics = append(diagnostics, resourceDiagnostic("SCN2413", "CLI argument positions must be unique non-negative integers", binding))
					} else {
				...
				case "flag":
					...
					if !validCLIName(name) || flags[name] || short != "" && (len(short) != 1 || shorts[short]) {
						diagnostics = append(diagnostics, resourceDiagnostic("SCN2414", "CLI flags require unique portable long names and one-character short names", binding))
					}
		...
		if len(positions) > 0 {
			...
			for index, position := range ordered {
				if index != position {
					diagnostics = append(diagnostics, resourceDiagnostic("SCN2413", "CLI argument positions must be contiguous from zero", binding))
					break
				}
			}
		}
		if shape.Record != nil {
			for name, field := range shape.Fields {
				if !field.Optional && !field.HasDefault && !mapped[name] {
					diagnostics = append(diagnostics, resourceDiagnostic("SCN2415", "required CLI input field has no mapping: "+name, binding))
				}
			}
		} else {
			whole := "operation." + operation.Name + ".input"
			if shape.Unit && len(mapped) != 0 {
				diagnostics = append(diagnostics, resourceDiagnostic("SCN2415", "unit CLI input cannot have argument, flag, or context mappings", binding))
			} else if !shape.Unit && !mapped[whole] {
				diagnostics = append(diagnostics, resourceDiagnostic("SCN2415", "non-record CLI input must be populated exactly once", binding))
			}
		}
		...
		for _, outcome := range namedChildren(cli, "outcome") {
			...
			if when == "" || outcomes[when] || !ok || exit < 0 || exit > 255 {
				diagnostics = append(diagnostics, resourceDiagnostic("SCN2416", "CLI outcomes require a unique condition and exit status from 0 through 255", binding))
			}
			outcomes[when] = true
			for _, stream := range []string{"stdout", "stderr"} {
				...
				if codec != "json" && codec != "problem_json" || from == "" || from != when && !strings.HasPrefix(from, when+".") {
					diagnostics = append(diagnostics, resourceDiagnostic("SCN2417", fmt.Sprintf("CLI %s output must use json or problem_json from its mapped outcome", stream), binding))
				}
			}
		}
		if stringValue(binding.Spec["delivery"]) != "enqueue" {
			for _, kind := range []string{"result", "error"} {
				for _, variant := range namedChildren(operation.Spec, kind) {
					...
					if !outcomes[when] {
						diagnostics = append(diagnostics, resourceDiagnostic("SCN2418", "reachable operation outcome has no CLI mapping: "+when, binding))
					}
				}
			}
		} else if !outcomes["dispatch.enqueued"] {
			diagnostics = append(diagnostics, resourceDiagnostic("SCN2418", "enqueue CLI binding requires dispatch.enqueued outcome mapping", binding))
		}
	}
	return diagnostics
}
