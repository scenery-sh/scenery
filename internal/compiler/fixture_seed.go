...
// BuildFixtureSeedPlans projects validated fixtures into bounded
...
func BuildFixtureSeedPlans(result *Result, environment string) ([]FixtureSeedPlan, error) {
	...
	if result == nil || result.Manifest == nil || !result.Valid() {
		return nil, fmt.Errorf("fixture planning requires a valid current contract")
	}
	if environment == "" {
	...
	for _, fixture := range result.Manifest.Resources {
		if fixture.Kind != "scenery.fixture" || !containsFixtureEnvironment(stringValues(fixture.Spec["environments"]), environment) {
			continue
		}
		...
		if entity.Kind != "scenery.entity" || dataSource.Kind != "scenery.data-source" || record.Kind != "scenery.record" {
		...
		plans = append(plans, FixtureSeedPlan{
			...
			Path: ".scenery/fixtures/" + strings.ReplaceAll(fixture.Address, "/", "_") + "." + environment + ".sql",
...
func fixtureSQLLiteralForType(value any, typeExpression, module string, resources map[string]Resource) (string, error) {
	...
	if enum := resources[namedFixtureTypeAddress(typeExpression, module)]; enum.Kind == "scenery.enum" {
