...
func applyHTTPEffectiveDefaults(resources []Resource) {
	...
	for index := range resources {
		...
		if resources[index].Kind == "scenery.http-gateway" {
			applyHTTPGatewayDefaults(&resources[index])
		}
	}
	for index := range resources {
		binding := &resources[index]
		if binding.Kind != "scenery.binding" || binding.Spec["protocol"] != "http" {
			continue
		}
...
func httpDefaultField() FieldProvenance {
	return FieldProvenance{Kind: "default", ProvidedBy: "scenery.http-codec", Transformations: []string{"http_profile_default"}}
}
