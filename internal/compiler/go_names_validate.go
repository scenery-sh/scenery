...
func validateGoGeneratedNames(resources []Resource) []Diagnostic {
	...
	for module, resource := range goModuleResources {
		...
		for _, fixed := range []string{"PackageIdentity", "PackageContractABIRevision", "PackageImportPath", "GoImplementationABIRange", "RuntimeABIRange", "unmarshalGeneratedContractValue"} {
