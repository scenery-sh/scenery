...
func expandContentPageResources(resources []Resource) ([]Resource, []Diagnostic) {
	...
	for _, content := range resources {
		...
		if content.Spec["source"] != nil {
			...
			source := byAddress[resolveResourceRef(content, sourceRef, "binding")]
		...
		generated := []Resource{
			{Address: pageAddress, Module: content.Module, Name: content.Name, Kind: "scenery.page", Origin: lineage(pageAddress, "page"), Spec: pageSpec},
...
func validateContentPage(resources map[string]Resource, content Resource) []Diagnostic {
	...
	if content.Spec["source"] != nil {
		sourceRef := refString(content.Spec["source"])
		source := resources[resolveResourceRef(content, sourceRef, "binding")]
