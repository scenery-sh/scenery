...
input "provider_token" {
  ...
  config {
	...
	integrity, ok := BuiltinProviderLock("registry.scenery.dev/core/vault")
	if !ok {
...
`, integrity, integrity, deploymentProviderABI)
	...
	if field.Kind != "provider_descriptor" || field.ProvidedBy != "registry.scenery.dev/core/vault#"+strings.TrimPrefix(integrity, "sha256:") {
...
func TestGoServiceConfigRejectsInvalidDynamicAttributes(t *testing.T) {
	...
	service := Resource{Address: "house/service/house", Module: "house", Kind: "scenery.service", Spec: map[string]any{
...
func TestGoServiceConfigSchemaComesFromTypedPackageInputs(t *testing.T) {
	...
	resources = append(resources, Resource{Address: "app/secret/provider_token", Module: "app", Kind: "scenery.secret", Name: "provider_token"})
