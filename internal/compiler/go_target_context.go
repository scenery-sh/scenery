...
// VerificationGoTargets resolves the concrete Go targets selected for default
// implementation verification, including their exact build contexts.
func VerificationGoTargets(result *Result) ([]GoBuildTarget, error) {
	...
	byAddress := resourcesByAddress(result.Manifest)
	resolved := make([]GoBuildTarget, 0, len(targets))
	for _, target := range targets {
		toolchain := byAddress[resolveResourceRef(target.Resource, refString(target.Effective["toolchain"]), "go_toolchain")]
		resolved = append(resolved, GoBuildTarget{
			Name: target.Resource.Name, Address: target.Resource.Address, Role: stringValue(target.Effective["role"]),
			Effective: target.Effective, Resolved: resolvedGoTargetContext(target.Effective, toolchain, &target.Context), Context: target.Context,
		})
	}
	return resolved, nil
}
