...
const (
	openAPIGeneratorProfile = "scenery.openapi"
...
func computeHTTPProjectionRevisions(manifest *Manifest) (map[string]string, map[string]string) {
	...
	for _, gateway := range manifest.Resources {
		if gateway.Kind != "scenery.http-gateway" {
			continue
		}
		projection := map[string]any{"spec_revision": manifest.SpecRevision, "gateway": httpGatewayProjection(manifest.Resources, gateway)}
		httpRevision := revisionHash("scenery.http-surface-revision\x00", projection)
		httpRevisions[gateway.Address] = httpRevision
		openAPIRevisions[gateway.Address] = revisionHash("scenery.openapi-revision\x00", map[string]any{
...
func httpGatewayProjection(resources []Resource, gateway Resource) map[string]any {
	...
	projection := map[string]any{"gateway": gateway.Address, "resources": projected}
	return projection
}
