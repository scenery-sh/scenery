...
func TestChangePlanNormalizesOperationsAcrossTemporarilyInvalidGraph(t *testing.T) {
	...
	for index, operation := range plan.Operations {
		if operation.ExpectedKind != "scenery.service" || operation.ExpectedSchemaRevision != resourceSchemaRevision("scenery.service") || operation.View != "source" {
...
func TestStructuredResourceRendererUsesHTTPWireLabelPolicies(t *testing.T) {
	schema, ok := compiler.AuthoredResourceSourceSchema("binding")
	if !ok {
	...
	for _, child := range []string{"header", "query_parameter"} {
		...
		public := spec.PublicAuthoredBlockSchema(httpSourceSchema.Children[child].Schema)
	...
	if _, diagnostics := scn.Parse(root, path); hasSCNErrors(diagnostics) {
		t.Fatalf("rendered wire labels do not parse: %#v", diagnostics)
	}
	invalid := bytes.Replace(hclwrite.Format(block.BuildTokens(nil).Bytes()), []byte(`header "x-request-id"`), []byte(`header "bad header"`), 1)
	if _, err := scn.CanonicalFormat(invalid, path); err == nil || !strings.Contains(err.Error(), "http_field_name") {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			_, err := renderAuthoredResourceBlock("idempotency", nil, map[string]any{
				"mode": "keyed",
				"key":  test.key,
			}, spec.NamedSourceSchemas()["operation_idempotency"], "house")
...
func TestChangeApplyRequiresBoundApprovalAndRejectsReplay(t *testing.T) {
	...
	token := scenery.NewApprovalToken(plan.PlanID, plan.Caller, []string{"risk_critical"}, time.Now().UTC().Add(time.Minute))
