...
func createResourceBlock(root string, base *Result, operation SemanticOperation) error {
	...
	schema, ok := compiler.AuthoredResourceSourceSchema(blockType)
	if !ok {
		return fmt.Errorf("resource.create does not support resource kind %q", blockType)
	}
	kind := graph.KindForBlock(blockType)
	if !compiler.ResourceCreateKindSupported(kind) {
...
func renderAuthoredResourceBlock(blockType string, labels []string, spec map[string]any, schema *compiler.AuthoredBlockSchema, module string) (*hclwrite.Block, error) {
	if schema == nil {
...
func validateAuthoredMutationValue(attribute compiler.AuthoredAttributeSchema, value any) error {
	...
	for index, value := range values {
		...
		if attribute.Constraints["reference_shape"] == "direct_input_field" {
			if _, ok := compiler.InputKeyFieldName(reference); !ok {
...
func authoredChildValues(name string, value any, child compiler.AuthoredChildSchema) ([]map[string]any, error) {
	...
	if child.Repeatable && !child.Ordered {
		sort.SliceStable(values, func(i, j int) bool {
			...
			left, _ := spec.MarshalCanonical(values[i])
			right, _ := spec.MarshalCanonical(values[j])
...
func authoredChildBlockValue(name string, value map[string]any, schema *compiler.AuthoredBlockSchema) ([]string, map[string]any, error) {
	...
	if !ok || !compiler.ValidAuthoredLabel(schema, label) {
