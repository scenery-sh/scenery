...
func PlanChanges(root string, request ChangeRequest) (ChangePlan, error) {
	...
	for index, requestedOperation := range request.Operations {
		...
		if refreshed, compileErr := compiler.CompileContractGraph(temp); compileErr == nil {
	...
	plan := ChangePlan{
		ArtifactIdentity: machine.NewArtifactIdentity(changePlanKind, changePlanSchemaDescriptor), Application: predicted.Manifest.Application.Name,
...
// ValidateSemanticOperation normalizes and validates one proposed mutation
// against an immutable compiler result without changing the real workspace.
func ValidateSemanticOperation(result *compiler.Result, operation SemanticOperation) (SemanticOperation, error) {
...
func writablePlanningResult(result *Result) (*Result, error) {
	...
	copyResult.Manifest = &Manifest{
		Kind: graph.ManifestKind, SchemaRevision: graph.ManifestSchemaRevision, SpecRevision: string(spec.CurrentRevision()), DiagnosticCatalog: graph.DiagnosticCatalog,
		Application: result.PartialGraph.Application,
...
func semanticOperationsDigest(operations []SemanticOperation) string {
	b, _ := spec.MarshalCanonical(operations)
	return byteDigest(append([]byte("scenery.semantic-operations\x00"), b...))
}
...
func normalizeSemanticOperationValues(operation SemanticOperation, base, predicted *Manifest) SemanticOperation {
	...
	if operation.Precondition != nil {
		...
		if precondition.Equals != nil {
			if resource, ok := resourcesByAddress(base)[operation.Address]; ok {
				if current, exists := compiler.ResourcePointerValue(resource, operation.Path); exists {
	...
	if operation.Op != "value.set" && operation.Op != "module.configure" {
		return operation
	}
	if resource, ok := resourcesByAddress(predicted)[operation.Address]; ok {
		if effective, exists := compiler.ResourcePointerValue(resource, operation.Path); exists {
...
func semanticOperationKind(operation SemanticOperation, manifest *Manifest) string {
	if operation.Op == "resource.create" {
		...
		if len(parts) >= 3 {
			return graph.KindForBlock(parts[len(parts)-2])
		}
		return ""
	}
...
func normalizePresentedSemanticValue(presented, effective any) any {
	if effectiveMap, ok := effective.(map[string]any); ok {
		if effectiveMap["$scalar"] != nil || effectiveMap["$ref"] != nil || effectiveMap["$expression"] != nil {
			return compiler.CloneSemanticValue(effective)
		}
		...
		for name, value := range presentedMap {
			if normalized, exists := effectiveMap[name]; exists {
				...
				result[name] = compiler.CloneSemanticValue(value)
			}
		}
		return result
	}
	if effectiveItems, ok := effective.([]any); ok {
		...
		for index, value := range presentedItems {
			...
			if exemplar == nil {
				result[index] = compiler.CloneSemanticValue(value)
			} else {
