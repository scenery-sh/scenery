...
// PositionIndex returns the source's byte-to-line/column index.
func (source *Source) PositionIndex() *PositionIndex {
	if source == nil {
		return nil
	}
...
func Parse(root, path string) (*Source, []Diagnostic) {
...
func ConvertExpression(sourceID string, source []byte, positions *PositionIndex, expr hclsyntax.Expression) Expression {
	rng := expr.Range()
	raw := ""
...
func expressionValueRanges(sourceID string, positions *PositionIndex, expression hclsyntax.Expression) map[string]Range {
...
func ConvertRange(sourceID string, positions *PositionIndex, rng hcl.Range) Range {
	return Range{
		SourceID: sourceID,
...
func DiagnosticsFromHCL(sourceID string, positions *PositionIndex, diagnostics hcl.Diagnostics) []Diagnostic {
...
func SourceID(relative string) string {
	...
	sum := sha256.Sum256(append([]byte("scenery.source-id\x00"), framed...))
...
func (index *PositionIndex) position(offset int) Position {
	if index == nil {
...
// Position returns the zero-based source position for a byte offset.
func (index *PositionIndex) Position(offset int) Position {
...
func SourceFiles(dir string, root bool) ([]string, error) {
	...
	for _, entry := range entries {
		...
		if entry.Type()&os.ModeSymlink != 0 || !info.Mode().IsRegular() {
			return nil, fmt.Errorf("source file must be a regular non-symlink file: %s", path)
		}
		if entry.Name() == AppLockFilename {
			continue
		}
		if root || entry.Name() == PackageFilename || entry.Name() != AppFilename {
