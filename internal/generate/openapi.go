...
func GenerateOpenAPIArtifact(result *Result, selector string) (OpenAPIArtifact, error) {
	...
	descriptor := addGeneratedArtifactIdentity(map[string]any{
		...
		"content_digest":  "sha256:" + hex.EncodeToString(digest[:]), "generator": "scenery.generate.openapi",
	}, openAPIDescriptorKind, openAPISchemaDescriptor, result.Manifest.SpecRevision)
...
func selectHTTPGateway(resources []Resource, selector string) (Resource, bool) {
	...
	for _, resource := range resources {
		if resource.Kind == "scenery.http-gateway" && (selector == "" || selector == resource.Name || selector == resource.Address) {
...
func renderOpenAPIDocument(manifest *Manifest, gateway Resource, httpRevision, openAPIRevision string) map[string]any {
	...
	return map[string]any{
		...
		"info":    map[string]any{"title": manifest.Application.Name, "version": manifest.ContractRevision},
