...
func TestPackageABIIsStableAcrossModuleAliasesWithTypeOnlyDependency(t *testing.T) {
	resourcesFor := func(instance string) []Resource {
		return []Resource{
			{Address: "app/module/" + instance, Module: "app", Kind: "scenery.module", Name: instance, Spec: map[string]any{"package": map[string]any{"name": "parent", "go_contract": map[string]any{"import_path": "example.test/cross/parent"}}}},
			{Address: instance + "/module/geometry", Module: instance, Kind: "scenery.module", Name: "geometry", Spec: map[string]any{"package": map[string]any{"name": "geometry"}}},
			{Address: instance + "/geometry/record/point", Module: instance + "/geometry", Kind: "scenery.record", Name: "point", Spec: map[string]any{"field": map[string]any{"name": "x", "type": map[string]any{"$ref": "float64"}}}},
			{Address: instance + "/record/shape", Module: instance, Kind: "scenery.record", Name: "shape", Spec: map[string]any{"field": map[string]any{"name": "point", "type": map[string]any{"$ref": instance + "/geometry/record/point"}}}},
			{Address: instance + "/operation/inspect", Module: instance, Kind: "scenery.operation", Name: "inspect", Spec: map[string]any{"input": map[string]any{"$ref": instance + "/record/shape"}, "result": map[string]any{"name": "ok", "type": map[string]any{"$ref": instance + "/geometry/record/point"}}, "handler": map[string]any{"method": "Inspect"}}},
		}
	}
