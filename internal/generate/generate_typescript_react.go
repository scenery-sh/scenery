...
func renderTypeScriptReact(result *Result, target Resource, root string, bindings []Resource) ([]generatedFile, []string, error) {
	...
	files = append(files,
		generatedFile{Path: filepath.Join(reactRoot, "pages.generated.ts"), Bytes: []byte(renderReactPages(pages, splitPages, contentPages))},
...
func renderReactPages(pages []reactTablePage, splitPages []reactSplitPage, contentPages []reactContentPage) string {
	...
	for _, page := range contentPages {
		fmt.Fprintf(&b, "import { %sPage } from %q;\n", goName(page.content.Name), "./"+page.content.Name+".generated.js")
	}
	...
	for _, page := range contentPages {
		fmt.Fprintf(&b, "  { path: %s, component: %sPage },\n", strconv.Quote(stringValue(page.content.Spec["path"])), goName(page.content.Name))
	}
...
func renderReactContentPage(result *Result, target Resource, reactRoot string, page reactContentPage, bindings []Resource) (string, error) {
	...
	b.WriteString("import { Page, defineContentPageSlots } from \"./scenery-ui/index.js\";\n")
	...
	b.WriteString("  const [state, setState] = useState<ContentPageState<" + resultType + ">>({ kind: \"loading\" });\n")
