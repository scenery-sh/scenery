...
func renderTypeScriptReact(result *Result, target Resource, root string, bindings []Resource) ([]generatedFile, []string, error) {
	...
	files, err := renderUICatalog(catalogRoot)
	if err != nil {
		return nil, nil, err
	}
	...
	files = append(files,
		generatedFile{Path: filepath.Join(reactRoot, "pages.generated.ts"), Bytes: []byte(renderReactPages(pages))},
...
func renderReactPages(pages []reactTablePage) string {
...
func renderReactTablePage(result *Result, target Resource, reactRoot string, page reactTablePage, bindings []Resource) (string, error) {
	...
	for _, kind := range []string{"column", "filter", "toolbar", "empty"} {
		for _, item := range namedChildren(page.table.Spec, kind) {
	...
	fmt.Fprintf(&b, "import { %sClient, SceneryClientError, dateTime, url } from \"../index.js\";\n", goName(target.Name))
	...
	b.WriteString("import type { TablePageCellProps, TablePageFilterProps, TablePageQuery, TablePageResult } from \"./scenery-ui/index.js\";\n")
	...
	for _, column := range namedChildren(page.table.Spec, "column") {
	...
	for _, filter := range namedChildren(page.table.Spec, "filter") {
	...
	for _, slot := range []string{"toolbar", "empty"} {
		for _, value := range namedChildren(page.table.Spec, slot) {
	...
	fmt.Fprintf(&b, "  const client = useMemo(() => new %sClient({ baseUrl: url(globalThis.location.origin), authentication: { credentials: \"include\" } }), []);\n", goName(target.Name))
	...
	b.WriteString("  ]} sorts={[\n")
	for _, sortSpec := range namedChildren(page.table.Spec, "sort") {
...
func reactClientMethod(page reactTablePage, bindings []Resource) string {
	count := 0
	for _, binding := range bindings {
		if lastRef(refString(binding.Spec["operation"])) == page.operation.Name {
			count++
		}
	}
	method := tsName(page.operation.Name)
	if count > 1 {
		method += "Via" + goName(page.binding.Name)
	}
	return method
}
