...
func renderReactTablePage(result *Result, target Resource, reactRoot string, page reactTablePage, bindings []Resource) (string, error) {
	...
	inputType := tsType(page.operation.Spec["input"])
	...
	for _, kind := range []string{"column", "filter", "toolbar", "footer", "empty", "row_detail", "row_action"} {
	...
	prefetchExport, prefetchComponentAddress := "", ""
	for _, kind := range []string{"row_action", "row_detail"} {
		for _, slot := range orderedChildren(page.table.Spec, kind) {
			if name := stringValue(slot["prefetch_export"]); name != "" {
				prefetchExport = name
				prefetchComponentAddress = resolveResourceRef(page.table, refString(slot["component"]), "react_component")
			}
		}
	}
	...
	if inputType != "" && inputType != rowType {
		fmt.Fprintf(&b, ", %s", inputType)
	}
	if len(page.metadataFields) > 0 {
	...
	for index, address := range componentAddresses {
		...
		if address == prefetchComponentAddress {
			prefetchAlias = alias + "Prefetch"
			fmt.Fprintf(&b, "import { %s as %s, %s as %s } from %s;\n", stringValue(component.Spec["export"]), alias, prefetchExport, prefetchAlias, strconv.Quote(module))
		} else {
			fmt.Fprintf(&b, "import { %s as %s } from %s;\n", stringValue(component.Spec["export"]), alias, strconv.Quote(module))
		}
	}
	...
	if len(componentAddresses) > 0 {
		...
		for _, slot := range []string{"toolbar", "footer", "empty"} {
			for _, value := range orderedChildren(page.table.Spec, slot) {
				if value["component"] != nil {
					alias := aliases[resolveResourceRef(page.table, refString(value["component"]), "react_component")]
					if slot == "toolbar" {
						alias = goName(page.table.Name) + "Toolbar"
					}
					fmt.Fprintf(&b, "  %s: %s,\n", slot, alias)
				}
			}
		...
		for _, value := range orderedChildren(page.table.Spec, "row_action") {
			alias := aliases[resolveResourceRef(page.table, refString(value["component"]), "react_component")]
			fmt.Fprintf(&b, "  rowAction: %s,\n", alias)
		}
		if prefetchAlias != "" {
			fmt.Fprintf(&b, "  rowIntent: %s,\n", prefetchAlias)
		}
		b.WriteString("});\n\n")
	...
	if page.pagination == "page" {
		b.WriteString("function reactTableSafeTotal(value: bigint | number): number {\n")
		b.WriteString("  const total = Number(value);\n")
		b.WriteString("  if (!Number.isSafeInteger(total) || total < 0) throw new RangeError(\"table_page total must be a non-negative safe integer\");\n")
	...
	searchInput := reactTableMappedName(queryMapping, "search", "search")
	var sorts []string
	if page.pagination == "cursor" {
		sorts = stringValues(page.crud.Spec["list"].(map[string]any)["sorts"])
	} else {
		sortField := shape.Fields[reactTableMappedName(queryMapping, "sort", "sort")]
		sorts = enumWireValues(resources, page.operation.Module, sortField.Type)
		if unwrapReactType(typeExpression(sortField.Type)) == "string" {
			for _, sortSpec := range orderedChildren(page.table.Spec, "sort") {
				sorts = append(sorts, stringValue(sortSpec["name"]))
			}
		}
	}
	...
	writeReactLoadWithDependencies(&b, queryParameter+": TablePageQuery, signal?: AbortSignal", "TablePageResult<"+rowType+metadataTypeArgument+">", func(b *strings.Builder) {
		fmt.Fprintf(b, "    const outcome = await client.%s({\n", method)
		if reactTableSearchable(page, shape, searchInput) {
			fmt.Fprintf(b, "      %s: query.search,\n", tsName(searchInput))
		}
		for _, filter := range orderedChildren(page.table.Spec, "filter") {
			field := stringValue(filter["name"])
			input := defaultString(strings.TrimSpace(stringValue(filter["input"])), field)
			fieldType := shape.Fields[input].Type
			values := enumWireValues(resources, page.operation.Module, fieldType)
			listInput := reactTableListInput(fieldType)
			if len(values) > 0 || unwrapReactType(typeExpression(fieldType)) == "string" || listInput {
				expression := fmt.Sprintf("query.filters[%s]", strconv.Quote(field))
				if len(values) > 0 {
					expression = fmt.Sprintf("%s.filter((value): value is %s => %s)", expression, reactTableFilterValueType(fieldType), reactLiteralPredicate("value", values))
				}
				if listInput {
					fmt.Fprintf(b, "      %s: Array.isArray(query.filters[%s]) ? %s : undefined,\n", tsName(input), strconv.Quote(field), expression)
				} else {
					fmt.Fprintf(b, "      %s: Array.isArray(query.filters[%s]) ? %s[0] : undefined,\n", tsName(input), strconv.Quote(field), expression)
				}
			} else {
				fmt.Fprintf(b, "      %sFrom: typeof query.filters[%s] === \"string\" ? dateTime(query.filters[%s]) : undefined,\n", tsName(input), strconv.Quote(field+"_from"), strconv.Quote(field+"_from"))
				fmt.Fprintf(b, "      %sTo: typeof query.filters[%s] === \"string\" ? dateTime(query.filters[%s]) : undefined,\n", tsName(input), strconv.Quote(field+"_to"), strconv.Quote(field+"_to"))
			}
		}
		if len(sorts) > 0 {
		...
		if page.pagination == "cursor" {
			...
			fmt.Fprintf(b, "      %s: %s,\n", tsName(pageSizeInput), reactTableIntegerExpression("query.limit", shape.Fields[pageSizeInput].Type))
		}
	...
	if loadingLabel := stringValue(page.table.Spec["loading_label"]); loadingLabel != "" {
		fmt.Fprintf(&b, " loadingLabel=%s", jsxStringExpression(loadingLabel))
	}
	if errorTitle := stringValue(page.table.Spec["error_title"]); errorTitle != "" {
		fmt.Fprintf(&b, " errorTitle=%s", jsxStringExpression(errorTitle))
	}
	...
	for _, column := range orderedChildren(page.table.Spec, "column") {
		...
		if value := stringValue(column["export_header"]); value != "" {
			fmt.Fprintf(&b, ", exportHeader: %s", strconv.Quote(value))
		}
		if value := stringValue(column["export_format"]); value != "" && value != "display" {
			fmt.Fprintf(&b, ", exportFormat: %s", strconv.Quote(value))
		}
		if column["export_empty"] != nil {
			fmt.Fprintf(&b, ", exportEmpty: %s", strconv.Quote(stringValue(column["export_empty"])))
		}
		if column["export_zero_empty"] == true {
			b.WriteString(", exportZeroEmpty: true")
		}
	...
	for _, sortSpec := range orderedChildren(page.table.Spec, "sort") {
		...
		fmt.Fprintf(&b, "    { field: %s, label: %s", strconv.Quote(tsName(field)), strconv.Quote(defaultString(stringValue(sortSpec["label"]), humanLabel(field))))
	...
	if len(orderedChildren(page.table.Spec, "row_action")) > 0 {
		b.WriteString(" rowAction={slots.rowAction}")
	}
	if prefetchAlias != "" {
		b.WriteString(" onRowIntent={slots.rowIntent}")
	}
	...
	if page.pagination != "" {
		fmt.Fprintf(&b, " pagination=%s", strconv.Quote(page.pagination))
	}
	if page.table.Spec["hide_header"] == true {
		b.WriteString(" hideHeader")
	}
	fmt.Fprintf(&b, " pageSize={%d} queryKey={scopedQueryKey} load={load} /></Page>\n", pageSize)
	for _, dialog := range page.dialogs {
...
func writeReactTableDialogState(b *strings.Builder, page reactTablePage, dialog reactTableDialog, bindings []Resource, resources map[string]Resource, rowType string) {
	...
	b.WriteString("        queryClient.invalidateQueries({ queryKey: scopedQueryKey }),\n")
	if page.stats != nil {
