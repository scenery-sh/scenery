...
func publicHTTPBindings(resources []Resource, target Resource) []Resource {
	...
	for _, r := range resources {
		...
		if r.Origin.Kind != "authored" && r.Origin.Kind != "framework" && !reactPageBindings[r.Address] {
			continue
		}
...
func renderTSClient(target Resource, bindings, resources []Resource, assistantSurfaces ...[]Resource) string {
	className := goName(target.Name) + "Client"
	retry := tsRetryTarget(target)
	var assistants []Resource
	if len(assistantSurfaces) > 0 {
		assistants = assistantSurfaces[0]
	}
	methods := tsClientMethods(bindings, resources)
	var b strings.Builder
	b.WriteString("// Code generated by Scenery. DO NOT EDIT.\n")
	...
	if len(methods) > 0 {
		b.WriteString(renderTSBindingsTable(methods))
	}
	fmt.Fprintf(&b, "\nexport class %s {\n", className)
	b.WriteString("  readonly #baseUrl: string;\n  readonly #fetch: typeof globalThis.fetch;\n  readonly #headers: Readonly<Record<string, string>>;\n  readonly #authentication?: Runtime.AuthenticationOptions;\n")
	if len(assistants) > 0 {
		b.WriteString("  readonly assistants: AssistantClients;\n")
	}
	if retry.Enabled {
		b.WriteString("  readonly #retryRuntime: Runtime.RetryRuntime;\n")
	}
	if len(methods) > 0 {
		b.WriteString("  readonly #transport: Runtime.InvokeTransport;\n")
	}
	...
	if len(assistants) > 0 {
		b.WriteString("    const assistantOptions: AssistantTransportOptions = { baseUrl: this.#baseUrl, fetch: this.#fetch, defaultHeaders: this.#headers, authentication: this.#authentication };\n    this.assistants = createAssistantClients(assistantOptions);\n")
	}
	if retry.Enabled {
	...
	for _, method := range methods {
		fmt.Fprintf(&b, "  async %s(%s: Types.%sInput, options: Runtime.CallOptions = {}): Promise<Types.%sOutcome> {\n", method.name, method.inputName, method.operationName, method.operationName)
		fmt.Fprintf(&b, "    return Runtime.invoke(this.#transport, bindings.%s, %s, options, typeRegistry) as Promise<Types.%sOutcome>;\n  }\n", method.name, method.inputName, method.operationName)
	}
	b.WriteString("}\n")
	return b.String()
}
