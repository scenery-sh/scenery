...
func TestOpenAPIArtifactRecordsSmallestGatewayProjection(t *testing.T) {
	gateway := Resource{Address: "app/http_gateway/public", Module: "app", Kind: "scenery.http-gateway", Name: "public", Spec: map[string]any{"base_path": "/api", "exposure": "internet"}}
	input := Resource{Address: "house/record/input", Module: "house", Kind: "scenery.record", Name: "input", Spec: map[string]any{"field": map[string]any{"name": "id", "type": map[string]any{"$ref": "int64"}}}}
	output := Resource{Address: "house/record/output", Module: "house", Kind: "scenery.record", Name: "output", Spec: map[string]any{"field": map[string]any{"name": "status", "type": map[string]any{"$ref": "string"}}}}
	operation := Resource{Address: "house/operation/get", Module: "house", Kind: "scenery.operation", Name: "get", Spec: map[string]any{"input": map[string]any{"$ref": "record.input"}, "result": map[string]any{"name": "ok", "type": map[string]any{"$ref": "record.output"}}}}
	...
	manifest := &Manifest{Application: graph.ApplicationIdentity{Name: "demo"}, ContractRevision: graph.RevisionHash("contract\x00", "demo"), Resources: []Resource{gateway, input, output, operation, binding}}
	httpRevisions := map[string]string{gateway.Address: graph.RevisionHash("http\x00", "demo")}
	openAPIRevisions := map[string]string{gateway.Address: graph.RevisionHash("openapi\x00", "demo")}
