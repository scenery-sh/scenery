...
func PlanDeployment(ctx context.Context, root string, request DeploymentPlanRequest, providers DeploymentProviderRegistry) (DeploymentPlan, error) {
	...
	projection, diagnostics := compiler.ResolveDeployment(result.Manifest, request.Deployment)
	...
	revisions := compiler.ComputeDeploymentRevisions(result.Manifest, implementation, map[string][]string{projection.Deployment: digests})
	...
	plan := DeploymentPlan{
		ArtifactIdentity: machine.NewArtifactIdentity(deploymentPlanKind, deploymentPlanSchemaDescriptor), Application: result.Manifest.Application.Name,
	...
	if len(plan.Capabilities) == 0 {
		plan.Capabilities = []string{"scenery.deployment"}
	}
	...
	if err := evolution.RetainIssuedPlan(root, evolution.IssuedDeploymentPlan, plan.PlanID, plan); err != nil {
		return DeploymentPlan{}, err
	}
	return plan, nil
}
...
func ApplyDeploymentPlan(ctx context.Context, root string, plan DeploymentPlan, options DeploymentApplyOptions, providers DeploymentProviderRegistry) (DeploymentReceipt, error) {
	...
	resolved, diagnostics := compiler.ResolveDeployment(current.Manifest, plan.Deployment)
	...
	revisions := compiler.ComputeDeploymentRevisions(current.Manifest, plan.ImplementationRevision, map[string][]string{plan.Deployment: digests})
	...
	receipt := DeploymentReceipt{
		ArtifactIdentity: machine.NewArtifactIdentity(deploymentReceiptKind, deploymentReceiptSchemaDescriptor), PlanID: plan.PlanID, Application: plan.Application,
	...
	stateBytes, err := json.MarshalIndent(deploymentState{
		ArtifactIdentity: machine.NewArtifactIdentity(deploymentStateKind, deploymentStateSchemaDescriptor),
		Plan:             plan,
		Receipt:          receipt,
	}, "", "  ")
	if err != nil {
...
func deploymentProviderGroups(projection DeploymentProjection, resources map[string]Resource) (map[string][]string, []string, error) {
	...
	for address, projected := range projection.Resources {
		...
		case "scenery.data-source", "scenery.execution-engine", "scenery.event-bus", "scenery.secret-store":
			...
			if resources[providerAddress].Kind != "scenery.provider" {
		...
		case "scenery.fixture":
			...
			if entity.Kind != "scenery.entity" || dataSource.Kind != "scenery.data-source" || resources[providerAddress].Kind != "scenery.provider" {
...
func normalizeDeploymentProviderPlan(plan DeploymentProviderPlan) DeploymentProviderPlan {
	if plan.Kind == "" {
		plan.ArtifactIdentity = machine.NewArtifactIdentity(providerDeploymentPlanKind, providerPlanSchemaDescriptor)
	}
	if plan.ProviderABI == "" {
...
func deploymentProviderPlanDigest(plan DeploymentProviderPlan) string {
	plan.Digest = ""
	return revisionHash("scenery.provider-deployment-plan\x00", plan)
}
...
func validateDeploymentProviderPlan(plan DeploymentProviderPlan) error {
	if machine.ValidateArtifactIdentity(plan.ArtifactIdentity, providerDeploymentPlanKind, providerPlanSchemaDescriptor, "re-plan") != nil || plan.ProviderAddress == "" || plan.ProviderSource == "" || plan.ProviderABI == "" || len(plan.Instances) == 0 {
...
func deploymentPlanID(plan DeploymentPlan) string {
	plan.PlanID = ""
	return revisionHash("scenery.deployment-plan\x00", plan)
}
...
func validateDeploymentApprovals(plan DeploymentPlan, options DeploymentApplyOptions) error {
	...
	for _, token := range options.ApprovalTokens {
		...
		payload, err := evolution.ApprovalTokenPayload(token)
